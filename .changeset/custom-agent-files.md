...
Support custom agents defined as Markdown files with frontmatter — a system-prompt body rendered as a template with `${var}` context variables and `${base_prompt}` for wrapping the default prompt, plus tool allow/deny lists — selected with exactly one of `--agent <name>` or `--agent-file <path>` (v2 engine only: `KIMI_CODE_EXPERIMENTAL_FLAG=1 kimi -p`). See "Custom Agents" in the Agents docs for the file format, discovery directories, and binding semantics.
