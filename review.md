...
### 2. `prompt-refresh` 决策用了刷新后的 manifest，但 telemetry 仍用旧 cache manifest
...
1. **避免 update preflight 抢先创建 telemetry `device_id`，恢复 `first_launch` attribution。**
2. **修复 prompt-refresh telemetry 使用 stale manifest。**
...
第一条 `first_launch` 丢失问题已在本次修复。合并前仍建议至少处理：

- prompt-refresh telemetry 的 stale manifest 问题；
