...
describe('CustomEditor mode-aware history navigation', () => {
  const UP = '\u001B[A';
  const DOWN = '\u001B[B';

  it('in bash mode, Up only recalls shell (!-prefixed) entries', () => {
    const editor = makeEditor();
    editor.addToHistory('prompt1'); // older
    editor.addToHistory('!cmd1'); // newer → history index 0
    editor.inputMode = 'bash';

    editor.handleInput(UP);
    expect(editor.getText()).toBe('cmd1');
    expect(editor.inputMode).toBe('bash');

    // No more shell entries beyond index 0, so a second Up is a no-op.
    editor.handleInput(UP);
    expect(editor.getText()).toBe('cmd1');
    expect(editor.inputMode).toBe('bash');
  });

  it('in prompt mode, Up recalls all entries and switches to bash for shell entries', () => {
    const editor = makeEditor();
    editor.addToHistory('!cmd1'); // older
    editor.addToHistory('prompt1'); // newer → history index 0

    editor.handleInput(UP);
    expect(editor.getText()).toBe('prompt1');
    expect(editor.inputMode).toBe('prompt');
  ...
  it('notifies onInputModeChange when a recalled entry changes the mode', () => {
    const editor = makeEditor();
    const modes: Array<'prompt' | 'bash'> = [];
    editor.onInputModeChange = (mode) => modes.push(mode);
  ...
  it('does nothing on Up when no history entry matches the current mode', () => {
    const editor = makeEditor();
    editor.addToHistory('prompt1');
    editor.addToHistory('prompt2');
    editor.inputMode = 'bash';
  ...
  it('restores the pre-browse mode when returning to an empty draft', () => {
    const editor = makeEditor();
    editor.addToHistory('!cmd1');
    editor.inputMode = 'bash';
  ...
  it('strips only the leading ! from a multi-line shell command', () => {
    const editor = makeEditor();
  ...
  it('strips only the marker ! from a command that itself starts with !', () => {
    const editor = makeEditor();
    editor.addToHistory('!!ls'); // marker '!' + command '!ls'
    editor.inputMode = 'bash';
