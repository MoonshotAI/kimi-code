...
function makeHost(
  ...
  const appState = {
    ...
    thinkingEffort: 'off',
    availableModels: {
      k2: { model: 'kimi-k2', provider: 'managed:kimi-code', maxContextSize: 200_000 },
    },
  };
  const session =
...
describe('activateModelSelection', () => {
  it('reports an engine-tracked switch when the pick changes the bound alias', async () => {
    ...
    const engineTrackedSwitch = await authFlow.activateModelSelection('k2', 'high');
