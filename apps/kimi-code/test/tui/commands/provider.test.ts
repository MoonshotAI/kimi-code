...
function makeHost(
  ...
  const host = {
    ...
    authFlow: {
      ...
      activateModelSelection: vi.fn(async () => options.activateReachedLiveSession === true),
    },
    track: vi.fn(),
    showStatus: vi.fn(),
    ...
    authFlow: {
      ...
      activateModelSelection: ReturnType<typeof vi.fn>;
    };
...
describe('setDefaultModel', () => {
  it('applies an above-default pick to the runtime when the gate keeps it session-only', async () => {
    ...
    expect(host.authFlow.activateModelSelection).toHaveBeenCalledWith('opus', 'xhigh');
    ...
    expect(
      host.authFlow.activateModelSelection.mock.invocationCallOrder[0]!,
  ...
  it('does not re-apply the effort when the pick persists', async () => {
    ...
    expect(host.authFlow.activateModelSelection).not.toHaveBeenCalled();
  });
