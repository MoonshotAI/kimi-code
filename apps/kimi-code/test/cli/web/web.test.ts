...
describe('`kimi web` foreground shutdown', () => {
  it('closes both telemetry pipelines within one shared deadline', async () => {
    vi.resetModules();
    ...
    let resolveHostTelemetry: (() => void) | undefined;
    shutdownTelemetry.mockImplementation(
      () =>
        new Promise<void>((resolve) => {
          resolveHostTelemetry = resolve;
        }),
    );
    ...
    try {
      ...
      expect(shutdownTelemetry).toHaveBeenCalledWith({ timeoutMs: 3_000 });
