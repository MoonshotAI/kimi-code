...
/**
 * fsync a directory so a rename inside it is durable (inlined from
 * agent-core `utils/fs.ts` — this package must not depend on v1).
 * Windows: directory handles cannot be fsynced — noop.
 */
