export default {
  ...
  tui: {
    ...
    statusMessages: {
      ...
      selectProviderToLogout: '选择要退出的提供商',
      feedbackNoAttachment: '无附件',
      feedbackNoAttachmentDesc: '仅发送反馈消息。',
      feedbackLogsOnly: '日志',
      feedbackLogsOnlyDesc: '包含最近的日志。',
      feedbackLogsAndCodebase: '日志 + 代码库',
      feedbackLogsAndCodebaseDesc: '包含最近的日志和活跃代码库信息。',
      shareDiagnosticInfo: '共享诊断信息',
      apiKeySavedTo: 'API 密钥已保存至 {provider}。',
      ...
      knownThirdPartyProvider: '已知第三方提供商',
      customRegistryOption: '自定义注册表',
      fetchingCatalog: '正在从 {url} 获取目录...',
      catalogLoaded: '目录已加载。',
      catalogAborted: '已取消。',
      catalogFailedToLoad: '加载目录失败。',
      catalogFetchFailed: '获取目录失败{hint}: {error}',
      providerNoUsableModels: '提供商 "{providerId}" 在此目录中没有可用模型。',
      providerUnsupportedWire: '提供商 "{providerId}" 的传输类型不受支持。',
      providerAdded: '已添加 {providerName} ({providerId})。',
      setDefaultModelFailed: '设置默认模型失败: {error}',
      defaultModelSet: '默认模型已设置为 {alias} ({effort})。',
      failedToImportRegistry: '导入注册表失败: {error}',
      failedToApplyRegistry: '应用注册表失败: {error}',
      registryNoProviders: '注册表中无提供商。',
