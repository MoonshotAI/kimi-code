...
export interface EditorKeyboardHost {
  ...
  /**
   * The host's harness (KimiTUI always has one). Hosts without one skip the
   * paste-time daemon upload and keep the submit-time inline base64 form.
   */
...
export class EditorKeyboardController {
  ...
  private async finishClipboardImagePaste(
    ...
    // The edge cap resolves from the engine's `[image]` config: the in-process
    // v2 bridge pushes it into the compressor's default, so a config reload
    // applies immediately.
