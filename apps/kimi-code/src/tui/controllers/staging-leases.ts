...
import type { TurnEndedEvent, TurnStartedEvent } from '#/core/index';
...
export class StagingLeaseTracker {
  ...
  handleTurnStarted(event: TurnStartedEvent): void {
    ...
    // The v2 turn.started payload carries no promptId field yet; read it
    // structurally so the exact binding engages the moment the engine starts
    // echoing it.
    const promptId = (event as { readonly promptId?: string }).promptId;
