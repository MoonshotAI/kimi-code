...
/**
 * Resolve the plugin-management API. Plugin state is app-global on the v2
 * engine, so `/plugins` always goes through the harness's global facade —
 * including a session-less startup.
 */
...
/**
 * Resolve the capability API. Capability state is app-global on the v2
 * engine, so it is always served through the harness's global facade —
 * including a session-less startup.
 */
