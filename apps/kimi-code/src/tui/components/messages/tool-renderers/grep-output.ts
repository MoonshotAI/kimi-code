...
export interface GrepStats {
   ...
   * `content`, the summed per-file counts in `count_matches`.
   */
...
const NOTICE =
  /^(?:No matches found|No non-sensitive matches found|Found \d+ total (?:non-sensitive )?occurrences? across |Filtered \d+ sensitive file|Results truncated to \d+ lines|\[Output truncated at \d+ bytes|Grep timed out after )/;
...
export function parseGrepOutput(toolCall: ToolCallBlockData, output: string): GrepStats {
  ...
  // matches; without them every row is one, and the path ends at its first
  // colon.
  ...
  return { mode, entries, matches: entries.length, files: paths.size };
}
