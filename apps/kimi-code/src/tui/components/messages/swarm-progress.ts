...
export interface SwarmProgressMember {
  readonly id: string;
  agentId?: string;
...
export interface SwarmProgressLegendItem {
  readonly label: string;
  readonly color?: keyof ColorPalette | string;
}

export interface SwarmProgressCellLabel {
  readonly text: string;
  readonly color?: string;
}

export interface SwarmProgressMemberRenderInput {
  readonly member: SwarmProgressMember;
  readonly snapshot: SwarmProgressSnapshot;
  readonly index: number;
...
export interface SwarmProgressFooterBarInput {
  readonly members: readonly SwarmProgressMember[];
...
export interface SwarmProgressOptions {
  readonly title?: string;
  readonly description?: string;
  readonly legend?: readonly SwarmProgressLegendItem[];
  readonly statusLabels?: Partial<Record<TotalStatus, string>>;
  readonly formatMemberId?: (input: {
    readonly member: SwarmProgressMember;
    readonly index: number;
    readonly count: number;
  }) => string;
  readonly cellLabel?: (input: SwarmProgressMemberRenderInput) => SwarmProgressCellLabel | string | undefined;
  readonly memberProgress?: (input: SwarmProgressMemberRenderInput & {
    readonly capacityTicks: number;
  }) => SwarmProgressMemberProgress | undefined;
  readonly footerBar?: (input: SwarmProgressFooterBarInput) => string | undefined;
...
export class SwarmProgressComponent implements Component {
  private members: SwarmProgressMember[];
  private readonly progressEstimator = new AgentSwarmProgressEstimator();
  private readonly title: string;
  private readonly legend: readonly SwarmProgressLegendItem[];
  private readonly statusLabels: Partial<Record<TotalStatus, string>>;
  private readonly formatMemberId: NonNullable<SwarmProgressOptions['formatMemberId']>;
  private readonly cellLabel: SwarmProgressOptions['cellLabel'];
  private readonly memberProgress: SwarmProgressOptions['memberProgress'];
  private readonly footerBar: SwarmProgressOptions['footerBar'];
  ...
  constructor(options: SwarmProgressOptions) {
    this.title = options.title ?? 'Agent Swarm';
    this.description = options.description ?? '';
    this.legend = options.legend ?? [];
    this.statusLabels = options.statusLabels ?? {};
    this.formatMemberId = options.formatMemberId ?? (({ member }) => member.id);
    this.cellLabel = options.cellLabel;
    this.memberProgress = options.memberProgress;
    this.footerBar = options.footerBar;
  ...
  setPromptTemplateText(promptTemplateText: string): void {
    if (promptTemplateText.length > 0 || this.promptTemplateText.length === 0) {
      this.promptTemplateText = promptTemplateText;
    }
  }

  markItemsStarted(): void {
    this.itemsStarted = true;
  }

  setMemberCount(count: number): void {
    this.ensureMemberCount(count);
  }

  setMemberItemTexts(fullItems: readonly string[], partialItems: readonly string[] = []): void {
  ...
  registerMember(input: {
    ...
    const member = this.findMemberForSubagent(input.runtimeId, input.memberIndex);
    if (member === undefined) return;
    member.agentId = input.runtimeId;
    if (member.phase === 'pending') member.phase = 'queued';
    this.startAnimationIfNeeded();
  }
  ...
  appendModelDelta(input: {
    ...
    const runtimeId = input.runtimeId ?? input.agentId;
    if (runtimeId === undefined) return;
    const member = this.findMemberByAgentId(runtimeId);
  ...
  markMemberCompleted(memberIndex: number, completedText?: string): void {
    const member = this.memberAt(memberIndex);
    if (member === undefined || member.phase === 'failed' || member.phase === 'cancelled') return;
  ...
  markMemberFailed(memberIndex: number, failureText?: string): void {
    const member = this.memberAt(memberIndex);
    if (member === undefined) return;
    this.failMember(member, Date.now(), failureText);
    this.startAnimationIfNeeded();
  }
  ...
  markMemberCancelled(memberIndex: number): void {
    const member = this.memberAt(memberIndex);
    if (member === undefined) return;
    this.cancelMember(member, Date.now());
  }
  ...
  render(width: number): string[] {
    ...
    const nowMs = Date.now();
    const snapshots = this.members.map((member): SwarmProgressSnapshot => ({
      phase: member.phase,
      ticks: member.ticks,
  ...
  private renderLegend(width: number): string[] {
    if (this.legend.length === 0 || width <= 0) return [];
    const rows: string[] = [''];
    const separator = chalk.hex(this.colors.textMuted)('  ');
    let current = '';
    for (const item of this.legend) {
      const rendered = this.renderLegendItem(item);
      const next = current.length === 0 ? rendered : `${current}${separator}${rendered}`;
      if (current.length > 0 && visibleWidth(next) > width) {
        rows.push(truncateToWidth(current, width));
        current = rendered;
      } else {
        current = next;
      }
    }
    if (current.length > 0) rows.push(truncateToWidth(current, width));
    return rows;
  }

  private renderLegendItem(item: SwarmProgressLegendItem): string {
    const color = item.color === undefined
      ? this.colors.textDim
      : colorValue(item.color, this.colors);
    return chalk.hex(color)(item.label);
  }
  ...
  private renderProgressStatusLine(width: number, status: TotalStatus): string {
    const label = renderStatusLabel(
      this.statusLabels[status] ?? totalStatusLabel(status),
    ...
    const customBar = this.footerBar?.({
      members: this.members,
      width: barWidth,
      colors: this.colors,
    });
    if (customBar !== undefined) {
      return truncateToWidth(`${label}${' '.repeat(TOTAL_STATUS_BAR_GAP)}${customBar}`, width);
    }
  ...
  private renderGrid(
    ...
    snapshots: readonly SwarmProgressSnapshot[],
    nowMs: number,
  ): string[] {
    ...
    for (let row = 0; row < rows; row += 1) {
      ...
      for (let col = 0; col < columns; col += 1) {
        ...
        cells.push(padAnsi(this.renderCell(member, snapshot, layout, nowMs, index + 1), layout.cellWidth));
      }
  ...
  private renderCell(
    member: SwarmProgressMember,
    snapshot: SwarmProgressSnapshot,
    ...
    if (snapshot.phase === 'pending') {
      return renderPendingCell(member, this.memberDisplayId(member, index), width, this.colors);
    }
    if (snapshot.phase === 'cancelled' && snapshot.ticks <= 0) {
      return renderCancelledUnstartedCell(member, this.memberDisplayId(member, index), width, this.colors);
    }
    if (!layout.renderText) {
      return this.renderCompactCell(member, snapshot, layout.barCells, nowMs, index);
    }
    if (snapshot.phase === 'queued' && snapshot.ticks <= 0) {
      return renderQueuedCell(member, this.memberDisplayId(member, index), width, this.colors);
    }

    const capacityTicks = layout.barCells * BRAILLE_LEVELS.length;
    ...
    const progressOverride = this.memberProgress?.({
      member,
      snapshot,
      index,
      count: this.members.length,
    ...
    const progressPhase = progressOverride?.phase ?? snapshot.phase;
    const id = chalk.hex(this.colors.primary)(this.memberDisplayId(member, index));
    const bar = brailleBar(
      progressOverride?.displayTicks ?? estimate.displayTicks,
      progressPhase,
      layout.barCells,
      this.colors,
      snapshot.phaseElapsedMs,
      progressOverride?.color ?? cancelledProgressColor(member, progressPhase, this.colors),
    );
    const prefix = `${id} ${bar} `;
    const labelWidth = Math.max(1, width - visibleWidth(prefix));
    const label = this.renderCellLabel(member, snapshot, index, labelWidth);
    return prefix + label;
  }
  ...
  private renderCompactCell(
    ...
    const capacityTicks = barCells * BRAILLE_LEVELS.length;
    const estimate = this.progressEstimator.estimate({
      memberKey: member.id,
      phase: estimatePhase,
      capacityTicks,
    ...
    const progressPhase = progressOverride?.phase ?? estimatePhase;
    const id = chalk.hex(this.colors.primary)(this.memberDisplayId(member, index));
    const bar = brailleBar(
      progressOverride?.displayTicks ?? estimate.displayTicks,
      progressPhase,
  ...
  private renderCellLabel(
    ...
    if (custom !== undefined) {
      const label = typeof custom === 'string' ? { text: custom } : custom;
      return truncateWithColor(label.text, width, label.color ?? phaseColor(snapshot.phase, this.colors));
    }
    return renderCellLabel(member, snapshot, width, this.colors);
  }

  private memberDisplayId(member: SwarmProgressMember, index: number): string {
    return this.formatMemberId({
      member,
      index,
      count: this.members.length,
    });
  ...
  private maxMemberIdWidth(): number {
    let width = 0;
    for (let index = 0; index < this.members.length; index += 1) {
      const member = this.members[index];
      if (member === undefined) continue;
      width = Math.max(width, visibleWidth(this.memberDisplayId(member, index + 1)));
    }
    return width;
  }
  ...
  private findMemberByAgentId(agentId: string): SwarmProgressMember | undefined {
    return this.members.find((member) => member.agentId === agentId);
  }

  private memberAt(memberIndex: number): SwarmProgressMember | undefined {
    if (!Number.isInteger(memberIndex) || memberIndex <= 0) return undefined;
    this.ensureMemberCount(memberIndex);
    return this.members[memberIndex - 1];
  }
  ...
  private promoteToRunning(member: SwarmProgressMember, nowMs?: number, setTicks = false): void {
  ...
  private completeMember(member: SwarmProgressMember, nowMs: number, completedText?: string): void {
  ...
  private failMember(member: SwarmProgressMember, nowMs: number, failureText?: string): void {
  ...
  private cancelMember(member: SwarmProgressMember, nowMs: number): void {
...
function createMembers(count: number, phase: AgentSwarmPhase): SwarmProgressMember[] {
...
function clearMemberState(member: SwarmProgressMember, ...keys: ClearableMemberKey[]): void {
...
function terminalPhaseElapsedMs(member: SwarmProgressMember, nowMs: number): number {
...
export function calculateAgentSwarmGridLayout(
  ...
  const idWidth = input.idWidth === undefined
    ? agentSwarmGridIdWidth(count)
    : Math.max(1, Math.floor(input.idWidth));
...
function summarizeSnapshots(snapshots: readonly SwarmProgressSnapshot[]): AgentSwarmSummary {
  let completed = 0;
  let failed = 0;
  let cancelled = 0;
...
function renderStatusPipBar(
  members: readonly SwarmProgressMember[],
  width: number,
  colors: ColorPalette,
): string {
...
function statusBarCounts(members: readonly SwarmProgressMember[]): StatusBarCount[] {
...
function runningCellLabelText(member: SwarmProgressMember): string {
...
function renderPendingCell(
  ...
  const id = chalk.hex(colors.primary)(displayId);
  const prefix = `${id} `;
  const itemText = collapseWhitespace(member.itemText);
...
function colorValue(color: keyof ColorPalette | string, colors: ColorPalette): string {
  return color in colors ? colors[color as keyof ColorPalette] : color;
}
