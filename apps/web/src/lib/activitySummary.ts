...
// the `activity-run` block in chatTurnRendering.ts). Pure string building on
// top of the shared i18n instance — no Vue reactivity here; the component
// recomputes when its items change.
//
...
export function summarizeActivity(
  ...
  for (const kind of order) {
    ...
    if (!acc) continue;
    const fragments: SummaryFragment[] = [{ text: doneClauseText(kind, acc.count), tone: 'normal' }];
    if (acc.errors > 0) {
      hasError = true;
      fragments.push(failedFragment(acc.errors));
    }
...
function currentClause(current: ActivitySummaryItem): SummaryClause {
  if (current.kind === 'thinking') {
    return { fragments: [{ text: t('thinking.streaming'), tone: 'normal' }] };
  }
  const kind = clauseKind(current.tool.name);
  let subject = toolSummary(current.tool.name, current.tool.arg);
...
export function summarizeLive(items: ActivitySummaryItem[], current: ActivitySummaryItem | null): LiveSummary {
  ...
  for (const kind of order) {
    const acc = byKind.get(kind);
    if (!acc) continue;
    const fragments: SummaryFragment[] = [{ text: `${prefix}${doneClauseText(kind, acc.count)}`, tone: 'faint' }];
    if (acc.errors > 0) fragments.push(failedFragment(acc.errors));
    done.push({ fragments });
  }
  const currentC = current === null ? null : currentClause(current);
