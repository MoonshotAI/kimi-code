...
describe('lib/log', () => {
  it('mirrors to the console and forwards level/message/detail to the bridge', () => {
    ...
    logWarn('[kimi-web] dev backend switch failed:', 'beta');
    expect(warnSpy).toHaveBeenCalledWith('[kimi-web] dev backend switch failed:', 'beta');
    expect(bridgeLog).toHaveBeenCalledWith('warn', '[kimi-web] dev backend switch failed:', 'beta');

    logError('[kimi-web] operation failed: x', new Error('kaput'));
    expect(errorSpy).toHaveBeenCalledOnce();
    expect(bridgeLog).toHaveBeenCalledWith('error', '[kimi-web] operation failed: x', expect.any(Error));
