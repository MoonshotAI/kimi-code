...
- **Before changing any component, style, layout, or theme, read the canonical design system at `apps/desktop/src/renderer/views/DesignSystemView.vue`** (desktop single source — the web copy and the in-app overlay were removed in P22; open it in the desktop app by long-pressing the sidebar logo). It is the canonical design system and visual spec for both ends (tokens §02, primitives §03, chat §04, theme rules §05, style rules §06). New and modified UI must match it.
...
- **The chat working indicator (小蓝 mascot + phase label) is reserved** for the chat working state after a prompt is sent — the main conversation's sending placeholder, the BTW side chat's send → first-token loading, and the inspected sub-agent's live turn in the right-side detail panel — and is rendered solely by `chat/WorkingIndicator.vue`; every other loading state uses the plain `Spinner`.
...
- `composables/` — app-side composables that remain here. The client singletons (`useKimiWebClient`, `useWorkspaceState`, `client/*`) live in `@moonshot-ai/app-client/client`; their platform seams (api / translator / tracer / optional desktop hooks) are registered via `setKimiClientDeps` in `main.ts`. Shared, non-diverged composables (`useIsMobile`, `useFilePreview`, `useDetailPanel`, …) live in `@moonshot-ai/app-client/composables` and take api / translator / tracker by injection — new shared composables go there, not here. (`useDialogFocus` moved to `@moonshot-ai/app-components` in P17.)
- `lib/` — app-side helpers that remain here. Shared pure helpers live in `@moonshot-ai/app-core/lib`; rendering types and hot-path pure modules (turns projection, diff builders, event batching) in `@moonshot-ai/app-core/client`; the icon registry + `kimi` SVG assets and browser-layer helpers (`mediaPreview`, `openFileAttachment`) in `@moonshot-ai/app-client` (`./icons` / `./lib`); the former `toolMeta` / `activitySummary` shells now live in `@moonshot-ai/app-components` (P17) — do not recreate them here.
