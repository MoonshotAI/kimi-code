  ...
  return (
    <div className="rounded-lg border border-zinc-200/50 bg-zinc-50/30 dark:border-zinc-800/50 dark:bg-zinc-900/10 overflow-hidden">
      <button
        ...
        className={cn("w-full flex items-center gap-2 pr-8 hover:bg-zinc-100/50 dark:hover:bg-zinc-800/20 transition-colors", compact ? "px-2 py-1.5" : "px-3 py-2")}
      >
        <div className="inline-flex items-center gap-2 min-w-0">
          <IconBrain className={cn("text-zinc-500 shrink-0", compact ? "size-3" : "size-3.5")} />
          <span className={cn("font-medium text-zinc-700 dark:text-zinc-300", compact ? "text-[0.75rem]" : "text-xs")}>Thinking</span>
          {isStreaming && <IconLoader3 className={cn("text-zinc-400 ml-auto animate-spin shrink-0", compact ? "size-3" : "size-3.5")} />}
        </div>
        <IconChevronDown className={cn("text-zinc-400 ml-auto transition-transform shrink-0", compact ? "size-3" : "size-3.5", expanded && "rotate-180")} />
      </button>
