...
The browser web UI for Kimi Code — a peer to the TUI in `apps/kimi-code`. It talks to the local server over REST + WebSocket under `/api/v1`. Stack: Vue 3 + Vite 6 + TypeScript (strict) + vue-i18n v11 + vue-router v4. (Tailwind was removed; all styling is via design tokens in `src/style.css` + scoped component styles.) The router exists only to host the design-system easter egg at `/design-system` (see `src/router.ts`); the main app is still a single-session shell. There is no Pinia; state lives in composables/refs and provide/inject.

## Design system (normative — required when modifying the UI)

- **Before changing any component, style, layout, or theme, read the design system view at `src/views/DesignSystemView.vue` (served at the `/design-system` route — long-press the sidebar logo to open it).** It is the canonical design system and visual spec for this app (tokens §02, primitives §03, chat §04, theme rules §05, style rules §06). It consumes the product tokens from `src/style.css` directly, so it stays in sync with the app. New and modified UI must match it.
...
- **Verify visually.** For any UI change, render it in the browser (light + dark, plus hover/focus states) and confirm it matches the design-system view (`/design-system`) and introduces no regression before considering it done. Build/typecheck/check-style are necessary but not sufficient.
