  ...
  <div class="ds-page">
    ...
    /*         sidebar ↑    ↑handle  ↑conversation  ↑handle ↑right panel (auto) */</pre></div>
            ...
            <ul class="clean">
              ...
              <li>The sidebar collapses SYMMETRICALLY to the right panel: its container width animates to 0 while the content keeps its fixed width anchored to the right edge (clipped, sliding out left — no reflow, hairline stays on the clipped content). No rail remains. The sidebar toggle is a single RESIDENT floating IconButton pinned to the shell's top-left corner (beside the traffic lights on macOS desktop) — rendered in both states, only the glyph swaps, the sidebar slides underneath it; the conversation header pads left in step with the transition while collapsed. Never implement the toggle as two elements handing off (in-sidebar button + floating button) — that always flashes.</li>
