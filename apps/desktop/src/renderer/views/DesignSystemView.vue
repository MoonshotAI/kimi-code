    ...
    /*         ↑app chrome  ↑terminal panel (desktop-only) */</pre></div>
            ...
            <table class="dt">
              ...
              <tbody>
                ...
                <tr><td>Flat-style variant (flat list + pinned section)</td><td>the sidebar's flat list rows AND — always, regardless of view mode — the pinned section's rows differ from the grouped row in three ways (all keyed off the facade projecting <code>cwdLabel</code>): ① no leading status slot — the title is left-aligned at the row's content edge; ② a second line under the title: <code>folder-closed</code> icon sm + the cwd's final directory name (<code>-</code> when the session has no cwd), xs faint like the time — except the icon, which takes <code>--color-text-muted</code> (one rung stronger, the same optical compensation as the group head's folder; the open-folder glyph's thin back-flap washed out at 14px) — rest-width tail mask fade; when the session has an associated PR (v2 git domain), a quiet chip (<code>git-pull-request</code> icon + #number) sits at the line's right edge, state-colored the GitHub way (open = <code>--color-success</code>, merged = <code>--color-done</code> purple, closed = faint) and opens the PR on click; ③ the first line's right side shows status — attention Badges anchored to the row's right edge, running Spinner, unread dot — INSTEAD of the time, which only renders when there is nothing to report (the Spinner yields to the attention pills: a session waiting for approval/answer never shows both); on hover the actions cross-fade IN as the whole status cluster fades OUT — pills and pin/archive never co-exist (grouped rows keep pills visible on hover). Height stays font-driven — the pill just grows the line. Grouped rows never set <code>cwdLabel</code> and keep the classic structure. The flat ↔ grouped switch lives in a dropdown on the SESSIONS section label (fixed <code>list-settings</code> icon + hover tooltip; the menu opens with a muted group label, per-view icons, and the current view checked at the row's right edge; mode persisted per device)</td></tr>
          ...
          <section id="dialogs">
            ...
            <table class="dt">
              ...
              <tbody>
                ...
                <tr><td class="tk">App update (desktop)</td><td>padded · lg · auto</td><td>version title + quiet meta line (release date · current version) + height-capped scrolling what's-new list / progress bar + right-aligned action row (skip → download, later → restart) with the auto-download checkbox right-aligned on its own foot row below (a pure preference for future checks)</td></tr>
