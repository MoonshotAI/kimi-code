  ...
  <div class="ds-page">
          ...
          <section id="tokens">
            ...
            <table class="dt">
              ...
              <tbody>
                ...
                <tr><td class="tk">--color-sidebar-bg</td><td class="val"><span class="swatch" style="background:#fbfaf9"></span>#fbfaf9</td><td class="val"><span class="swatch" style="background:#13181e"></span>#13181e</td><td>Sidebar surface — one step off <code>--color-bg</code> so the session column reads as its own plane</td></tr>
    ...
    <span class="c">// size:    sm | md | lg</span></pre>
            ...
            <ul class="clean check">
              ...
              <li><b>Grouping</b>: consecutive quiet activity (thinking + quiet-line tool calls) folds into ONE activity-run row — a smart summary sentence that expands into the items in order; text and the richer blocks (Todo, Goal, Question, Swarm, sub-agent identity cards, inline media) never fold and break the run (see §04).</li>
      ...
      return verify(token, process.env.JWT_SECRET!);
          ...
          <section id="chat">
            ...
            <ul class="clean check">
              ...
              <li>Consecutive quiet activity — thinking segments and quiet-line tool calls (Read / Grep / Search / Glob / Ls / Fetch / Bash / Edit / Write) — <b>folds into ONE activity-run row</b>: a smart summary sentence that aggregates the run per tool kind in first-appearance order (<code>Read 2 files · Ran 5 commands (1 failed) · 26.8s</code>), the failure clause hanging on its kind in danger red, the total span faint at the tail — one line, ellipsis-truncated, the full sentence in the title tooltip. Thinking items fold into the run but are not narrated in the sentence. The row shares the thinking row's language (borderless faint text row, text-colour hover only, one whole-row button with a rotating chevron) but rides a roomier 8px vertical padding — 30px against the quiet lines' 22px, so the turn-level summary keeps its presence between prose paragraphs; while the turn streams through the run the row stays expanded and the summary turns live (current action + cumulative per-kind stats + ticking whole seconds), and once every item settles it folds itself back — even if the user expanded it mid-run (the thinking block's vocabulary); a settled → running transition (the stream appending to the same run) reopens it. The glyph carries the state: the current step's own icon breathing while running, green ✓ / red ✕ once settled. A run needs <b>≥ 2 steps</b> — a lone step renders as the plain quiet line it always was. <b>Text never folds</b> (it breaks the run), and neither do the blocks that keep their own richer rendering: Todo and Goal tools narrate progress standalone, a sub-agent delegation keeps its own identity card, Question / Swarm keep their cards, successful media tools render inline media, and unrecognized kinds stay standalone — any of these breaks a run on both sides. The expanded run is the items flat in order (thinking rows + tool quiet lines), each with its own in-row details intact — the lines keep their own 4px row rhythm but breathe 8px apart, with a small inset below the head.</li>
