...
<template>
  <div class="ds-page">
    ...
    <div class="layout">
      ...
      <aside class="sidebar">
        <div class="brand">
          ...
          <div class="brand-name">Kimi Code</div>
        </div>
        ...
        <div class="content-inner">
          ...
          <section id="overview">
            <div class="hero">
              ...
              <h1>Kimi Code <span class="grad">Design System</span></h1>
              <p class="lead">
                This document defines the visual language and component specification for Kimi Code — design tokens, component primitives, the chat interface, theming, and style rules.
                All UI work is grounded in it: unified, restrained, token-driven, and themeable.
              </p>
              <div class="hero-meta">
                <span class="meta-chip"><span class="dot"></span> Scope <b>apps/web</b></span>
          ...
          <section id="principles">
            ...
            <p class="sec-desc">
              Every UI decision traces back to the following principles. Kimi Code is a local Agent tool for developers: quick scanning, long stretches of staring, often in the dark — the design serves the task, and is restrained, clinical, and density-first.
            </p>
          ...
          <section id="tokens">
            ...
            <p>The text-selection color uses <code>--p-selection</code> uniformly (light <code>rgba(23,131,255,.18)</code> / dark <code>rgba(88,166,255,.32)</code>), applied by the global <code>::selection</code> rule; do not set a separate highlight background. The ONE exception is code surfaces (code blocks, diff rows): there the selection uses <code>--color-code-selection</code> — an accent-derived fill tuned per theme so the major shiki token colours stay at WCAG 1.4.3 (≥4.5:1 against it, ink NOT flattened; light: a soft blue passing base/string/number/variable, dark: a quiet navy passing everything but comment) while the fill sits at ≈1.5:1 against the well — a product visibility floor, not WCAG (the 1.1–1.3:1 macOS/VS Code norm was judged too subtle; an earlier pass at WCAG 1.4.11 was mis-anchored — it does not cover text selection). Documented residuals: light comment/keyword/function and dark comment cannot reach 4.5 at any fill that also keeps the others and the visibility floor.</p>
            ...
            <p>Kimi Code uses two font tokens: <b>--font-ui</b> (UI and body, with Schibsted Grotesk for Latin and Noto Sans SC for Simplified Chinese) and <b>--font-mono</b> (code and monospace). Components always reference the variables; do not hard-code font names.</p>
    ...
    <span class="c">// size:    sm | md | lg</span></pre>
            ...
            <p>Inline code, code blocks, and diff contents use the monospace font (<code>--p-font-mono</code>); diff change counts and branch summaries use the UI font. Code blocks have a filename title bar and a copy button; the action edge uses a compact 6px inset. The code column's left inset aligns with the header's language-icon inset (<code>--space-3</code>, right inset to match), lines use <code>--leading-normal</code> (1.5 — the HighlightedCode rhythm in the file preview / diff detail), and block padding is <code>--code-pad-block</code> (half the inter-line gap). Diffs use <code>+</code> / <code>-</code> row colors to express additions and deletions — additions use a success light background, deletions use a danger light background, with no gradients.</p>
            <p><b>Word-wrap toggle</b>: every chat code block (markstream-rendered fences and the local <code>```diff</code> renderer) carries two per-block toggles (line numbers, word wrap) at the right of its header action row, just before the copy button — same compact action-button recipe (muted 14px glyph, the <code>--color-hover</code> f1 hover wash, <code>--radius-sm</code>, the shared focus ring), with <code>aria-pressed</code> and two tabler glyphs (<code>text-wrap</code> / <code>text-wrap-disabled</code>) for the off/on states; the on state is the NEUTRAL <code>--color-selected</code> fill with full-colour ink (never accent — accent is the primary-action hue, not a state colour), hovering a pressed toggle steps one rung deeper to <code>--color-selected-hover</code>, and long lines fold (<code>pre-wrap</code> + <code>overflow-wrap: anywhere</code>) instead of scrolling horizontally. The markstream-header instance is a <b>documented structural exception</b> to the component-primitive rule (like the mention tooltip): its anchor lives inside markstream-vue's runtime-rendered header — no host template mount point, and the highlighted body sits in pierre's shadow root — so it is injected as raw DOM (the <code>codeWrap</code> module, same pattern as the widen-table chip) and re-implements the Button contract by copying the sibling copy button's <code>.code-action-btn</code> class chain. The local <code>```diff</code> bar instance is plain template markup on the §03 <code>IconButton</code> primitive (sm, <code>label</code> + <code>tooltip</code>, <code>aria-pressed</code> on the root). Tooltips are real bubbles, not native <code>title</code>: every header button (numbers, wrap, AND the copy button) carries a localized <code>data-md-tip</code> attribute served by one document-level singleton (<code>codeTooltip</code> — the mention-tooltip pattern: raw-DOM delegation, 150ms show delay, dark <code>--color-text</code>-ink bubble on <code>--color-bg</code>, top placement with flip, hidden while any menu surface is open); the text is read at show time, so locale switches re-stamp the attribute and the next hover speaks the new language. State is per block, default off, and the toggle never remounts the block, so streaming and highlight state are undisturbed. The local diff renderer keys its wrap state by block content + same-content occurrence index — a best-effort identity: keys follow each block through inserts / removals / reorders and identical blocks stay independent, but a structural edit among identical-content blocks may reset or migrate the state (self-healing — one click restores it); keys whose block no longer exists are pruned on render. Line numbers are a SEPARATE per-block header toggle (list-numbers glyph; row order: numbers, wrap, copy), default OFF and fully independent of wrap: when on, numbers show in BOTH scroll and wrap modes (pure CSS counter, muted --color-text-faint, never selectable or copied), and a wrapped continuation still carries no number and aligns with the code column.</p>
            <div class="stage-wrap">
    ...
    export function auth(token: string) {
      return verify(token, process.env.JWT_SECRET!);
            ...
            <p><b>Conversation chrome</b>: Header labels are not selectable; the rename input remains selectable and editable. Branch names start with a 14px branch icon. The overflow trigger is a compact 24px control with a 14px icon. Below a 720px header container, hide the workspace prefix and give the conversation title the available width. On macOS desktop the header doubles as the window-drag region and interactive controls opt out with no-drag; while one of its menus or a dock work panel is open every window-drag strip (chat header, sidebar header, panel header) drops the drag region so an outside press anywhere reaches the page and dismisses the overlay (window dragging is simply paused). On Windows desktop a separate global titlebar sits above the product headers: its height follows <code>env(titlebar-area-height)</code> with a 40px fallback so page zoom stays aligned with the native Window Controls Overlay; full brand with a white tile in light mode and dark tile in dark mode + resident sidebar toggle + File/Edit/View/Help + conditional update pill sit at left, native controls at right, and the remaining safe area is draggable. Opening a native menu preserves the renderer's current editing focus; a standalone Alt enters keyboard menu access only on keyup so Alt-based app shortcuts keep their original scope, access restores that focus when a submenu opens or the user exits with Alt/Escape, and clears without stealing focus when Tab or a pointer moves elsewhere; moving across or activating another top-level trigger closes the current popup and switches directly. Hovered and open triggers retain a system outline in Windows forced-colors mode. Because the custom menu omits Window/minimize, <code>Ctrl+M</code> remains assignable on Windows.</p>
            <p><b>Open In pill</b> (<code>OpenInMenu.vue</code>, shared from <code>@moonshot-ai/app-components</code>; desktop-only entry, data via the injected <code>OpenInService</code> — empty catalog hides the control): a 26px segmented pill — left segment (30px) shows the current target's colored app icon and opens with one press (never persists), right caret (22px) expands the app menu where picking an item both opens and pins it as the default (same storage key the settings 打开方式 select writes, so pill and settings stay in sync; unset = first available). Menu items carry the colored icons with a generic external-link glyph fallback, plus a copy-path row at the end. The two segments are a <b>documented structural exception</b> to the component-primitive rule (like the dock overlay and the mention tooltip): the IconButton primitive's sm size is square with its own radius and hover tile, which cannot assemble a single-bordered segmented control, so the segments re-implement the IconButton contract by hand (transparent rest, the <code>--color-hover</code> wash, the shared <code>:focus-visible</code> ring).</p>
            ...
            <p><b>Session status (<code>/status</code>)</b>: a default-size <code>md</code> Dialog (<code>StatusPanel.vue</code>, shared by both ends) rendering the active session's state as a plain <code>dl</code> of label/value rows — model, thinking, permission, plan mode, swarm mode, context usage, cost. Every row is read-only text EXCEPT Thinking, which reuses the composer's own thinking control verbatim: the same <code>segmentsFor()</code>-driven §03 SegmentedControl (off + declared efforts, or on/off for boolean models — a static "not supported" note when the model declares neither), sitting inside the row's <code>dd</code> so it keeps the list's label/value pairing and the row's flex layout rules; a pick goes through the identical <code>setThinking</code> path as the composer/mobile-sheet pickers (session-scoped, not the daemon-wide default), and carries the same cache-invalidation caption those pickers show beneath it. No new visual language — this is the existing thinking-picker vocabulary transplanted into a read-mostly info panel, so switching thinking mid-session no longer requires leaving <code>/status</code> for the composer.</p>
            ...
            <ul class="clean">
              ...
              <li><b>WorkingIndicator (小蓝 mascot · brand signature)</b> —— used <b>only</b> for the chat working state after a prompt is sent (the sending placeholder in ChatPane, the send → first-token loading in SideChatPanel, and the inspected sub-agent's live turn in the right-side detail panel). The label follows the phase: "Requesting…" until the assistant's reply starts, then "Working…".</li>
            </ul>
            ...
            <!-- ===== SelectionActionBubble (selection-anchored actions) ===== -->
            <h3 class="sub">SelectionActionBubble · selection-anchored actions</h3>
            <p>Transient action bubble anchored to a LIVE TEXT SELECTION inside read-only content (today: assistant message markdown AND the detail panel's file preview — comment / add to chat / add to side chat; <code>components/chat/SelectionActionBubble.vue</code>, shared by both ends). Reach for this pattern whenever an overlay must anchor to a selection rather than to a trigger element. The surface is the §03 Menu verbatim (Menu / MenuItem primitives — the glass token pair and <code>--shadow-menu</code> stay single-sourced) on the <code>--z-dropdown</code> rung, teleported to <code>&lt;body&gt;</code> with <code>position: fixed</code> and anchored to the selection range's rect (horizontal center): BELOW the selection's bottom edge by default — the hand is already there after a drag — flipping above only when below doesn't fit but above does, and clamped to the viewport margin at both ends when neither fits (the maxHeight cap keeps every action reachable). Detection is ONE delegated mouseup AND keyup on the transcript root (never per-block listeners — the keyup entry covers keyboard selections, Shift+Arrows / cursor browse mode), PLUS a document-level <code>selectionchange</code> listener debounced ~250ms — the touch path, where long-press and native selection handles fire nothing else; the debounced refresh is ownership-gated to the pane's own root (a live selection made inside an embedded pane — side chat, agent detail, which renders the same <code>.a-msg .msg</code> markup — only ever CLOSES ours, never re-anchors), skipped entirely while focus sits inside the bubble (comment input / menu interaction must not be disturbed), and transcript-only embeds never register the listener (their <code>selectionActions</code> prop is false). A pointerdown restores the source semantics so a sticky keyboard flag can't misread a later touch-handle selection as keyboard-driven. The bubble shows only for a non-collapsed selection whose range sits ENTIRELY inside ONE <code>.a-msg .msg</code> (both boundary containers, resolved through shadow-host chains so code-block selections count), and a collapsed selection closes it. A SECOND mount covers the detail panel's file preview: <code>App.vue</code> delegates mouseup/pointerdown on the <code>.global-preview</code> aside (the keyboard entry is a document-level keyup inside the composable — the aside is not focusable, so a delegated keyup would miss caret-browsing gestures whose focus sits on <code>&lt;body&gt;</code>) and mounts another instance whose container selector is <code>.file-preview .fp-body</code> (every preview content kind — markdown / code / json / csv / text — mounts under it; iframe kinds never leak a selection out of their frame). The shared trigger logic lives in <code>useSelectionQuoteBubble</code> (app-client) with the container selector parameterized; the two mounts never pop for the same selection — each one's ownership gate only ever CLOSES its own bubble — and the preview's actions ride the transcript's quote pipeline unchanged. Because the keyboard entry is document-level, an outside dismiss (Esc / scroll / outside click) latches the bubble CLOSED until the next <code>selectionchange</code>: the dismissed DOM selection survives, and a stray keyup (Tab, a shortcut) must not revive the bubble from it (a genuine new gesture — a collapse or growth — always fires selectionchange first, so re-selection reopens it immediately). On MOBILE, where the preview is a full-screen overlay, a quote/comment also closes the panel on success — <code>insertQuote</code> focuses the composer, and it must be VISIBLE (sidechat needs no help: it switches the panel itself). A keyboard-driven open focuses the first item and gives the menu the §08 keyboard model (↑/↓ wrap, Enter activates, Esc closes); a mouse open never steals focus. Dismissal: outside mousedown; OUTSIDE scroll (capture — scrolls inside the surface, like the comment input's autoscroll, are ignored); window resize closes the MENU mode but only REPOSITIONS in comment mode (the mobile soft keyboard must not close mid-composition — iOS Safari's visualViewport resize/scroll is handled the same way, and positioning clamps against the visual viewport when present); and a capture-phase Escape on WINDOW (firing before App's document-capture panel close, IME-latch filtered) the bubble SWALLOWS so one Esc closes only the topmost layer — the following Esc keyup is itself suppressed so the kept DOM selection can't reopen it. Positioning reads its gap (<code>--space-1-5</code>) and viewport margin (<code>--p-mention-tip-vmargin</code>) from the tokens, anchors below the range's BOTTOM edge by default, flips above only when below doesn't fit but above does, clamps both axes to the margin at both ends, and caps the surface (max-width/height + overflow) so every action stays reachable at high zoom. "Comment" swaps the items for an inline Input + primary Button inside the same surface (the role flips to <code>dialog</code>; Enter confirms, Esc cancels) — one surface, two states. Nothing persists: no highlight, no stored range — the owner clears the document selection once an action is taken. Comment and add-to-chat insert the excerpt into the composer doc as an inline quote PILL (the rich-text vocabulary's third element — full spec in §05 <b>Quote pill</b>; the the comment rides INSIDE the pill as its second segment), serialized back to a markdown <code>&gt; </code> block on send; the side-chat exit opens the <code>/btw</code> panel with the quote as a DRAFT (nothing is sent).</p>
            ...
            <p>App-icon picker (settings → General → Appearance, last row; macOS desktop only, <code>components/settings/DockIconPicker.vue</code> — deliberately NOT this §03 SegmentedControl: its options are the real app-icon tiles, not text labels). The two options (Default / Black) render the shipped icon composition via <code>BrandLogo</code> forced variants — the light tile is the static icon baked into the .icns, the black tile is the runtime alternative; there is deliberately no system-following option, keeping the row free of theme vocabulary so it cannot be confused with the light/dark appearance setting. Anatomy follows the segmented-control language: 0.5px hairline sunken container with 2px padding, one raised pill sliding and resizing between options on the standard motion tokens, equal-width items stacking a 32px tile over an 11px/450 caption (<code>--text-2xs</code> / <code>--weight-caption</code>). Each tile drops BrandLogo's surface border and is clipped to the shipped icon's true squircle (superellipse n=4.5, the same shape baked into the .icns); its hairline ring is two nested squircles (32px line-colour wrapper + 31px tile), and a text-colour wash well keeps the white tile readable in light mode and the black tile in dark mode. ARIA: <code>radiogroup</code> of <code>radio</code> buttons with the WAI-ARIA keyboard model — roving tabindex (only the selected item is tabbable) and Arrow/Home/End keys moving selection and focus together.</p>
            <div class="stage-wrap">
            ...
            <p><b>Default Thinking</b> (settings → Agent Defaults, next to the Default Model select): the daemon-wide <code>config.thinking</code> counterpart to the picker above, but single-model — the row renders the same §03 SegmentedControl the composer uses for a session (off + the default model's declared efforts, or on/off for boolean models; a static "not supported" note when the default model declares neither). A default model configured but absent from the loaded <code>/models</code> catalog (a config-only entry, still a valid pick per the Default Model select) degrades to the plain on/off segments rather than losing the control — only a genuinely UNSET default model replaces the control with a "No default model" note; the row itself always renders. Writes go through <code>thinkingLevelToConfig</code>, the same mapping the TUI's <code>thinkingEffortToConfig</code> uses: a model's HIGHEST declared tier persists only <code>enabled</code>, never <code>effort</code>, so it can never leak into the single global slot every other model/client also reads — picking it shows selected for the current render but falls back to the model's own default after a reload, a deliberate cross-client protection, not a bug. Switching the Default Model select does NOT also touch thinking — the two rows write independently, same as before this control existed; POST /config's merge semantics mean a stale effort from the previous model can persist under the new one until it's explicitly changed again (a pre-existing limitation of the single global <code>config.thinking</code> slot, not something this row can fix on its own).</p>
          ...
          <section id="chat">
            ...
            <p class="sec-desc">
              The message stream is the core of Kimi Code. Tool calls render as <b>quiet activity lines</b> — one borderless line per call,
            ...
            <p><b>Plan and Goal are the primary work modes — mutually exclusive, at most one armed at a time.</b> Arming happens only through the slash commands (<code>/plan</code>, <code>/goal</code>) or the add menu; an armed mode renders as the input row's leading pill — a neutral chip at the fills ladder's <code>--color-selected</code> rung (one step above the composer card: dark's <code>--color-surface</code> equals <code>--color-composer-bg</code>, so a surface fill vanished there) with the mode's 14px icon, its label, and a × that disarms. The pill is NOT an overlay but a ProseMirror widget decoration pinned to the document head — an in-flow chip on the first line that scrolls with the text, can never sit mid-document or be pushed there by a paste, and never enters the serialized text. Its × is a plain button hand-built to the IconButton contract (Vue primitives can't reach widget DOM — the same exception as the mention tooltip's), sized <code>--wm-x-size</code> (below the sm default, so its hover wash never outgrows the pill's rounded end) with a <code>--wm-x-ring</code> hit reserve; on touch the pill takes its own lane above the text, so the ×'s full <code>--touch-target-min</code> hit area fits inside the lane's reserved footprint instead of covering an editable line — and the armed editor reserves <code>--space-1</code> of scrollport headroom plus a <code>--space-05</code> inline reserve, so the ×'s focus ring and the pill's own rounded cap never clip at the scrollport's edges. On phones the pill stays visible (it used to hide below 640px), pinned to the pinned mobile input line (<code>--composer-mobile-input-size</code>) so it never outgrows the first line at any font scale. The empty-doc placeholder is a decoration too, so the two share the first line rather than colliding; arming Goal also swaps the placeholder to its objective prompt. Sending with Goal armed creates the goal and the pill hands off to the dock's goal pill; a live goal does NOT lock the primary modes — <code>/goal</code> then just focuses the goal's panel, and <code>/plan</code> still arms (the only hard rule: one message cannot both create a goal and enter plan — the goal write carries the plan disarm atomically — and a failed mode write is treated as a send failure: the optimistic message rolls back, the error toasts). Swarm is deliberately NOT a mode: an orthogonal toolbar chip with its own enable confirmation, disarmed from the chip's ×.</p>
          ...
          <section id="richtext">
            ...
            <h3 class="sub">Attachment pill</h3>
            <p>The attachment is the second rich-text element: a file or folder carried <b>with</b> the message — an upload tracked by a fileId, or a folder reference — as opposed to a mention, which only points at a path. It shares the mention's pill geometry (baseline-flush inline-flex, no fill, strong weight, the 2px inset, the leading kind glyph in the same sm icon slot, the 32-char middle-ellipsis label) <b>and its ink</b>: muted, deepening toward <code>--color-text</code> on hover — pills never use the accent color. The distinction from a mention is the leading glyph alone: the paperclip (Remix <code>attachment-2</code>, one glyph for both kinds); the label is the file name, a folder keeping its trailing <code>/</code> — that slash doubles as the kind marker on the wire. On the message stream the pill is <b>click-to-open</b>: a bubble pill whose attId (the submit-time 1..N index) resolves against the message's file attachments is stamped with <code>data-attachment-url</code> plus the file id, media type and size (metadata rides whenever the target resolves — an url-less inline-base64 pill keeps it too), gains button semantics (pointer cursor, hover underline, tabindex + the shared <code>:focus-visible</code> ring), and its click or Enter/Space routes through the <code>mentionTooltip</code> singleton to the fileId open chain (the whitelisted inert-type new-tab preview — an unsupported type toasts); the composer keeps editing semantics (click places the caret — the editor's pills never carry the url and the <code>.ProseMirror</code> override pins the I-beam), and a pill without a resolvable url (an inline-base64 upload with no fileId, an out-of-range index, a queue row whose row is the edit button) stays inert with no pointer affordance. The pill exists in the same two synchronized forms as the mention: an atom in the composer's ProseMirror document and a pill in the message stream, both produced by ONE shared builder (<code>attachmentPill</code>) — same classes, same <code>data-attachment-*</code> attributes — so the editor and the bubble render the same mark. The atom carries only <code>{attId, name, kind}</code>: the heavier metadata (real path, size, upload state, fileId) lives in the composer editor's attachment registry, keyed by attId, so the wire text never exposes a local path. A pill-flow bubble shows its files <b>only</b> as inline pills; a legacy message without attachment links (pre-pill, chip-era history) instead lists its files — the recovered "Attached file …" notices — as a row of these same attachment pills at the very top of the body — pills which follow the same rule: a url-backed one is clickable through the same singleton route (the retired AttachmentChip's behavior carried over), a url-less one stays inert. Media (image/video) thumbnails are unaffected. The legacy row's actionable pill is a native <code>&lt;button type="button"&gt;</code> — a documented structural exception to the component-primitive rule, same family as the dock rows' full-cover open layer and the tooltip singleton above: it is a label-less, size-less, variant-less pure interaction layer that must look byte-identical to the editor's NodeView pill (shared builder, shared classes), so the Button primitive's chrome and <code>:active</code> scale do not apply; the UA chrome is reset in <code>ChatPane</code>'s scoped sheet, Enter/Space fire the native click, and the singleton's capture-phase routing suppresses the double-fire. Inline body pills stay text-atom spans (the mention's pattern) — the exception covers only the row's clickable form.</p>
            <p><b>Wire form and submit rewrite</b>: the pill serializes as a Markdown link with a composer-private scheme — <code>[name](kimi-code-composer://attachments/&lt;attId&gt;)</code> (full spec in app-composer's <code>docs/wire-format.md</code>). The composer document and the persisted draft keep this attId form; on submit the outgoing text rewrites it — a file attachment's attId becomes its 1-based index in first-mention order (the model and the attachments payload correlate by position), and a folder, which is never uploaded, becomes a plain folder mention link <code>[name](path/)</code> carrying the real path. The scheme sits in its own namespace: every mention consumer (the revive parser, the message-side classifier) rejects schemed hrefs on sight, so the two link families are disjoint by construction and a hand-typed or model-emitted file link can never be read as an attachment reference.</p>
            <p><b>Hover tooltip</b>: served by the same <code>mentionTooltip</code> singleton — the documented structural exception above; the attachment adds no new one — as a variant of the path tooltip led by the paperclip glyph. The composer's registry resolves the metadata: a path-backed attachment shows the full path in the mention's wrapping recipe (muted separators, bold basename) and the copy button copies the path; a pathless one — pasted bytes, or a pill in the message stream where no registry resolves it — degrades to the bold name with a muted size tail (<code>· 12 KB</code>; the message side reads the size back from the pill's <code>data-attachment-size</code>, stamped by <code>attachmentTargetAttrs</code> whenever the target resolves), and the copy button copies the name. A composer pill the registry no longer knows (an undo resurrected it without its entry, a revived draft whose sidecar is gone) takes the dead state <code>.attachment-missing</code> — the shared pill ink fades to 55% and strikes through, the mention's not-found recipe verbatim; message-bubble pills never strike (they simply have no registry behind them). A resolved path also runs the existence probe, striking on a definitive not-found — keyed by attId, since the pill carries no path. A composer pill whose upload failed (or was interrupted by a reload) takes the error state <code>.attachment-error</code> — the pill ink goes <code>--color-danger</code>, the tooltip adds a danger line naming the reason (localized, with the delete-or-re-drop remedy), and the send gate refuses to submit while any referenced entry is errored, so a failed attachment can never leave the composer silently; re-dropping the same file restarts the upload and clears the state.</p>
            <p><b>Clipboard</b>: copying out of the composer or a bubble writes three flavors — the composer-internal <code>application/x-kimi-composer</code> (carrying only a process-local vault ref; the flavor itself — the selection as a composer slice plus the referenced registry entries — stays in memory, so a paste back into a composer restores the pills with metadata, deduped by entry key, without local paths or fileIds ever touching the OS clipboard), <code>text/html</code>, and a <code>text/plain</code> that deliberately degrades an attachment pill to its bare name (a folder's keeps the trailing <code>/</code>) — the composer-private attId link never leaves the app as plaintext, so external targets only ever see plain names.</p>
            <p><b>Implementation map</b>: atom, serialization, and the dual-class link parser in app-composer's <code>composerTextDoc</code>; the doc-reconciled metadata registry, the clipboard flavor, and the paste-merge arbitration in <code>attachmentRegistry</code>; the shared pill builder in <code>attachmentPill</code>; the attId → open-target mapping in <code>attachmentTarget</code>; the tooltip variant and the attachment click routing in <code>mentionTooltip</code>; styling next to <code>.mention-pill</code> in app-ui's global sheet. Both composers wire entries and uploads through their <code>Composer.vue</code> + the shared <code>composerAttachments</code> pure layer (<code>@moonshot-ai/app-client/lib</code>); every bubble renders the same pills from the wire text.</p>
            <h3 class="sub">Quote pill</h3>
            <p>The quote is the third rich-text element — and the first composer-only one: the selection quote actions (划词, §03 <b>SelectionActionBubble</b>) insert the selected excerpt into the composer doc as an inline atom instead of a <code>&gt; </code>-prefixed text block. It shares the pill vocabulary's geometry and ink logic — baseline-flush inline-flex, strong weight, the leading glyph in the same sm icon slot, muted ink deepening toward <code>--color-text</code> on hover — but is the family's <b>ONE filled member</b>: user feedback was that a quoted excerpt reads indistinguishable from body text at the bare-ink recipe, so the quote pill carries an <b>accent-soft ground</b> (the same accent link in the reference visual chain as the text-selection highlight and the blockquote's leading bar) with the glyph in accent at rest, hover deepening the ground to <code>--color-accent-bd</code>; the selection-paint rule still wins over the ground. The glyph is the chat-quote (Remix <code>chat-quote-line</code>), the label the quote's first non-empty line, end-ellipsized at 24 graphemes. Several pills <b>accumulate</b>, each one atom (Backspace removes it whole); the 评论 flow's comment rides <b>INSIDE the pill</b> as its second segment (hairline divider, muted regular-weight excerpt) — quote and comment read as one bundled annotation, never as pill plus loose editable text. The composer keeps pure editing semantics everywhere (I-beam, click places the caret — the pill never carries click affordances). The pill's submit form closes the loop on the MESSAGE side: a wire-level <code>&gt; </code>blockquote block — a blank-line-bounded run of canonical <code>&gt; </code>lines, exactly what the submit rewrite emits (and what a hand-typed quote can only imitate) — renders in the user bubble as a quote CHIP: the same pill geometry and accent-soft ground, the same 24-grapheme excerpt label, the full quote on <code>data-quote-text</code> — and the inline chunk that FOLLOWS a quote block rides as the chip's comment segment (<code>data-quote-comment</code>), so the message shows the same two-segment bundle as the composer and the singleton's hover bubble shows the annotated content (quote + comment) there too. The segmentation (<code>splitQuoteBlocks</code>) is conservative — only a chunk made ENTIRELY of canonical quote lines chips, everything else (hand-typed mixes) stays verbatim text — and byte-reversible: copy re-serializes a chip to its <code>&gt; </code>lines, and a paste back into the composer revives the live pill from the chip's self-contained link form. Hover shows the <b>mentionTooltip singleton's</b> path-tooltip variant led by the quote glyph — the full quoted text, whitespace preserved and scroll-capped, with a copy button copying it.</p>
            <p><b>Wire form and draft persistence</b>: the pill serializes as a Markdown link with a composer-private scheme — <code>[excerpt](kimi-code-composer://quote/&lt;encoded&gt;)</code> — whose destination IS the full quote (canonically percent-encoded, the mention dest alphabet; newlines ride as <code>%0A</code>), with independent <code>?comment=&lt;encoded&gt;</code> / <code>?source=&lt;encoded&gt;</code> parameters for the bundled comment and the preview provenance (each '?' is literal ONLY at its own parameter marker — the text's own are percent-encoded). Self-contained by design: no registry, no sidecar — draft restore, history recall, queue reloads and session stash/restore all revive the pill (with its comment and source) from its link form alone (the mention pill's mechanism verbatim). The submit rewrite folds every quote link into its blockquote block on the outgoing copy (app-client's <code>rewriteQuoteLinks</code>) — byte-identical to the text-era wire format (pinned by the quote-selection tests) — and a preview quote's source leads its block as a <code>from: path[:Lx-Ly]</code> header line (single <code>\n</code>, shared chunk with the quote lines; the message-side parser splits it back off). A quote link slipping onto the message surface degrades to its plain quoted text, never rendering the scheme. Implementation: atom, codec, parser and insertion in app-composer's <code>composerTextDoc</code>; the shared pill builder in <code>quotePill</code>; the tooltip variant in <code>mentionTooltip</code>; styling next to <code>.mention-pill</code> in app-ui's global sheet.</p>
          </section>
          ...
          <section id="themes">
            ...
            <p class="sec-desc">
              Kimi Code uses <b>one unified theme</b>: the same components, fonts, radii, shadows, and surfaces — theming only swaps color values.
          ...
          <section id="rules">
            ...
            <div class="callout good"><span class="ico">✓</span><div>
              The chat working state ("prompt sent, turn unfinished") is a brand signature of Kimi Code, rendered uniformly by the <code>WorkingIndicator</code> component: the 小蓝 mascot plus a phase label — "Requesting…" until the assistant's reply starts, "Working…" once it is streaming.
            ...
            <div class="footer">
              <span>Kimi Code Design System · v1.0</span>
          ...
          <section id="shell">
            ...
            <p>On desktop it is a 5-track × 2-row grid: the sidebar and the right panel each occupy a permanent <code>auto</code> column track, with the conversation column in the middle; two 0-width tracks are for the ResizeHandles. Row 1 holds all app chrome; row 2 is the bottom terminal panel's <code>auto</code> track (desktop-only, see below).</p>
...
grid-template-rows: minmax(0, 1fr) auto;
    /*         ↑app chrome  ↑terminal panel (desktop-only) */</pre></div>
            <table class="dt">
              <thead><tr><th>Token</th><th>Value</th><th>Usage</th></tr></thead>
              <tbody>
            ...
            <ul class="clean">
              ...
              <li>The sidebar collapses SYMMETRICALLY to the right panel: its container width animates to 0 while the content keeps its fixed width anchored to the right edge (clipped, sliding out left — no reflow, hairline stays on the clipped content). No rail remains. The collapse control differs by platform: on <b>macOS desktop</b> the toggle is a single resident floating IconButton pinned beside the traffic lights; on <b>Windows desktop</b> it is a resident control immediately after the full brand in the 40px global titlebar, the empty sidebar brand header is omitted entirely, the action group receives <code>--space-2</code> top breathing room, and no floating expand/new-chat controls render; on <b>web</b> the collapse button lives inside the sidebar header and a floating expand button appears only while collapsed. The conversation header uses a 0.5px bottom hairline; only web needs extra left padding for its collapsed floating controls.</li>
            ...
            <p>A session row is an inset rounded pill, structured as: <code>state slot → title → status cluster (attention Badge / Spinner / unread dot) or time → hover actions (pin / complete)</code>.</p>
            <table class="dt">
              <thead><tr><th>Part</th><th>Rule</th></tr></thead>
              <tbody>
                ...
                <tr><td>State slot (lead)</td><td>fixed <code>--sb-gutter</code> width, exactly aligned with the workspace folder icon; an EMPTY gutter that keeps the title start aligned with the group head's name. The status-view Open/Done state glyphs (<code>state-open</code> / <code>state-done</code>) used to ride here (and inline before the title in flat rows) — removed: the 进行中/已完成 tab above already labels the list's state. Running spinner, unread dot and attention Badges never appear here — they report on the trailing side, one status at a time (the shared <code>SessionDisplayStatus</code> enum, app-core <code>sessionDisplayStatus.ts</code>: approval › question › running › aborted › unread)</td></tr>
                ...
                <tr><td>Time</td><td>mono xs, <code>fg-faint</code>; renders only when the row has nothing to report — any status (attention Badge, running Spinner, unread dot) replaces it on the trailing side; yields to the hover actions on hover</td></tr>
                <tr><td>Attention Badge</td><td><code>Badge</code> sm: info (needs answer) / warning (needs approval) / danger (aborted); the Spinner yields to them (a session waiting for approval/answer never shows both). On hover the Badge fades out with the rest of the status cluster as the actions fade IN — pills and pin/complete never co-exist</td></tr>
                <tr><td>Hover actions</td><td><code>IconButton</code> sm × 2 — pin + complete (the status view's PR-merge relabel of archive; done rows show a single reopen instead) — cross-faded over the status cluster / time on row hover (no kebab button). Right-clicking the row opens the full menu (copy ID / rename / emoji / fork / export / pin / complete + timestamp) anchored to the cursor, except over the inline rename input, where the native text-editing menu stays</td></tr>
                <tr><td>Flat-style variant (flat list + pinned section)</td><td>the sidebar's flat list rows AND — always, regardless of view mode — the pinned section's rows differ from the grouped row in two ways (all keyed off the facade projecting <code>cwdLabel</code>): ① no leading state slot — the title starts at the row's content edge; ② a second line under the title: <code>folder-closed</code> icon sm + the cwd's final directory name (<code>-</code> when the session has no cwd), xs faint like the time — except the icon, which takes <code>--color-text-muted</code> (one rung stronger, the same optical compensation as the group head's folder; the open-folder glyph's thin back-flap washed out at 14px) — rest-width tail mask fade; when the session has an associated PR (v2 git domain), a small tag (<code>git-pull-request</code> icon + #number, 2xs medium on a soft ground with a hairline edge and radius-sm corners — a mini §03 Badge) sits at the line's right edge, state-colored the GitHub way (open = <code>--color-success-soft</code> ground + <code>--color-success</code> text, merged = <code>--color-done-soft</code> + <code>--color-done</code> purple, closed = neutral sunken) and opens the PR on click. The trailing status cluster (attention Badges / running Spinner / unread dot replacing the time, cross-fading with the hover actions so pills and pin/complete never co-exist) is shared by every row, flat or grouped. Height stays font-driven — the pill just grows the line. Grouped rows never set <code>cwdLabel</code> and stay single-line. The flat ↔ grouped display switch lives in the 列表管理 (list management) dropdown on the section label (fixed <code>list-settings</code> icon + hover tooltip; the menu opens with a muted group label, per-mode icons, and the current mode checked at the row's right edge; mode persisted per device)</td></tr>
                <tr><td>Archive</td><td>no confirm — the hover archive button / menu item archives immediately, then App.vue shows the §03 <code>ActionToast</code> (top-center) with just Undo (restores the session)</td></tr>
              </tbody>
            </table>
          ...
          <section id="a11y">
            ...
            <p class="sec-desc">
              Kimi Code is a local developer tool; it <b>does not target a specific WCAG conformance level</b>, nor maintain a full screen-reader QA matrix.
...
/* =====================================================================
  ...
  .p-code-block pre { margin: 0; padding: var(--code-pad-block) var(--space-3); font-family: var(--p-font-mono); font-size: var(--p-font-size-sm); line-height: var(--leading-normal); color: var(--p-text); overflow-x: auto; }
