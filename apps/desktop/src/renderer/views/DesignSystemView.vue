    ...
    export function auth(token: string) {
      return verify(token, process.env.JWT_SECRET!);
          ...
          <section id="chat">
            ...
            <div class="callout info"><span class="ico">i</span><div>
              <b>Site-wide consistency</b>: the composer uses one 32px superellipse shell and one 32px desktop control height. The add (+), permission, compact, and model controls are all full-round and transparent at rest; hover reveals a neutral wash, open/active may use accent-soft, and Send remains the sole persistent filled control — an inverted <code>--color-text</code> fill with a <code>--color-bg</code> glyph (never the accent), disabled while the input is empty or an upload is in flight. The transparent dock floats over the transcript, while the scrolling content receives bottom padding equal to the live dock height so its final item can still clear the composer. Composer chrome is not selectable; only the message input permits text selection. Each permission mode has its own registry icon — manual <code>hand</code>, yolo <code>shield-question</code>, auto <code>full-access</code> — paired with the label in the pill (collapsing to the accessible icon below a 620px composer container) and leading its dropdown row in the mode's colour, with the current row's check trailing the row's end. The right toolbar is the flexible region: the model pill shrink-wraps its content, then shrinks and truncates internally only when the toolbar runs out of room. The dock's work pills and panels — pill vocabulary, panel shell and motion, the two-tone head, per-kind bodies, filtering, and the open/cancel model — are specified in <b>Dock · work pills &amp; panels</b> below.
            </div></div>
...
grid-template-rows: minmax(0, 1fr) auto;
    /*         ↑app chrome  ↑terminal panel (desktop-only) */</pre></div>
            ...
            <div class="callout info"><span class="ico">i</span><div>
              The session title's starting x = <code>--sb-pad-x + --sb-gutter + --sb-gap</code>. The group head has a folder icon and the session row has a state slot (the Open/Done glyph); both icons are the same width and position, so the titles align naturally.
            </div></div>
            ...
            <ul class="clean">
              ...
              <li>A group with no sessions is NOT rendered in the 进行中 tab (a cleanup leaves no pile of empty folders; the 工作空间 tab is the directory for creating sessions) — EXCEPT the active workspace's group, which stays so the draft state below keeps its head fill. The Done tab's groups filter the same way. When nothing is open at all (and no pinned sessions), the tab shows the "还没有进行中的会话" empty line.</li>
