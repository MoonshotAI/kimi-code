...
// Sessions revealed per in-group "expand" step — both the UI increment and the
// page size of the follow-up fetch when the locally loaded rows can't cover
...
export function useWorkspaceState(rawState: ExtendedState, deps: UseWorkspaceStateDeps) {
   ...
   *  "expand" click that the locally loaded rows can't cover. */
  ...
  async function loadAllSessions(): Promise<void> {
    ...
    const result = await listAllSessionsGlobal().catch((err) => {
      logWarn('[kimi-code] loadAllSessions failed; search covers only loaded sessions', err);
      return null;
    });
  ...
  async function addWorkspaceByPath(root: string): Promise<boolean> {
    ...
    try {
      ...
      logWarn('[kimi-code] addWorkspaceByPath failed for', trimmed, err);
      return false;
    }
  }
        ...
        [sid]: list.map((t) =>
          t.id === taskId
            ? {
                ...
                // Stamped on the cancel response — client-observed, sort-only
