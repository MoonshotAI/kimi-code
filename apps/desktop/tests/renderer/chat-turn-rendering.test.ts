...
describe('notification render blocks', () => {
  ...
  it('never breaks the activity run on a notification — it defers to after the run', () => {
    const rendered = assistantRenderBlocks(
      assistantTurn([toolBlock('a'), toolBlock('b'), ntfBlock('n'), toolBlock('c'), toolBlock('d')]),
    );
    // All four calls stay in ONE fold group; the notification renders right
    // after it instead of splitting the run in two.
    expect(rendered.map((b) => b.kind)).toEqual(['activity-run', 'notification']);
    if (rendered[0]?.kind === 'activity-run') {
      expect(rendered[0].items.map((it) => it.sourceIndex)).toEqual([0, 1, 3, 4]);
    }
  });
