...
- **待办**：上游 0.31.1+ 同步——A 表 25 项已全部定案（A16/A17/A19 已迁移，A20/A24 已核对为 ⚪，见 `上游更新/迁移计划.md`）；剩余 **C3 插件通知（引擎侧 ? 待核对）**；A8/A23 ⚪ 宿主面；kap-server Rust 化 **阶段 4 已完成**（2026-08-02 复核：src+test 零 v2 import、protocol 19/19 本地化、12 路由去 core）；**v2 harness 已删除**（2026-08-02：apps/kimi-code 的 `cli/v2/`、`cli/experimental-v2.ts`、`--agent/--agent-file` 选项及测试全部移除）；**rust-backed 引擎接入层已收进 SDK**（2026-08-02：`@moonshot-ai/kimi-code-sdk/rust` 新入口——SessionEventTranslator/SessionEngineController/wire 翻译层/RustRpcClient（rust-loop 驱动 CoreAPI，80 方法面与基类对齐，缺口 nativeUnavailable，6 单测绿））；**createKimiHarness 已切换 RustRpcClient 并删除 KimiCore/SDKRpcClient**（2026-08-02：sdk-rpc-client.ts 重写为 Rust-backed 装配；auth.ts/kimi-harness.ts 解绑 agent-core（legacy config/errors + 本地 ImageLimits/withTelemetryContext）；vitest alias 补 subpath 映射；SDK typecheck 绿 + 12 关键测试绿）；**剩余**：① **SDK 集成测试迁移（进行中：320/414 = 77% 通过）**——已实现：合成事件（config.update/turn.steer/permission.set_mode/session.closed）、v1 兼容 wire 事件历史（WireEventWriter：session_index.jsonl + wire.jsonl）、configGet/Set 面（setKimiConfig re-read 合并结果、getKimiConfig 默认值）、telemetry withContext 语义；剩余 93 失败按文件分类：create-session-transport(15)/session-prompt-events(15) 为 wire 事件流与 fork/delete 语义、local-logging(9)/mcp-config(9) 需确认是否预存、session-context(7)/auth-facade(6)/config(6) 为 **config wire 形状差异（Rust serde snake_case vs v1 camelCase+raw）**、session-skills(6)/list-sessions(5)/session-plan(5)/session-cancel(4)/rename(3)/export(2)/background(1)；**引擎 RPC 缺口**：forkSession/deleteSession/exportSession/全局 MCP 写操作（nativeUnavailable）；② image compression 已本地化（legacy/image 移植 + 178 测试绿）；剩余 loadMcpServers/PluginManager/profile 移植；③ SDK 49 个 v1 wire 公开类型切 Rust wire 形状 + CLI/TUI 联动（用户已确认彻底路线）；④ klient 新增 Rust 传输（替换 v2 dispatcher）；⑤ agent-core/agent-core-v2 物理移入 retired/；⑥ 发布规划：kimi-agent（private）需随 SDK 发布或 vendor rust-loop
