# 03 · KimiRuntime 逐方法详解：327 行里的会话路由器

> 时效基线：基于 commit `48e80483`（2026-09-01 拉取后复核；上一基线 `68ddf583b`，其间 #3276 给本类加了 viewChains 视图串行化——见第三节开头的"第〇步"）。行号会随代码演进漂移，**本篇每处 `文件:行号` 引用都附带源码原文**——日后行号对不上时，按代码文本搜索即可重新定位。
> 实测基线：`pnpm -C apps/vscode exec vitest run test/kimi-runtime.test.ts` → 29 个测试全部通过（2026-08-29，旧基线）；2026-09-01 拉取新增并发双开测试（test:399 起，共 32 个）后未在本机复跑（shell node 22 < 仓库门槛 24.15），待合适环境验证。

## 全局认知

- **定位**：`apps/vscode/src/runtime/kimi-runtime.ts`，327 行。类上的 JSDoc 一句话："Extension-host owner for one in-process Node SDK harness"——扩展宿主进程里**那一个**引擎实例（KimiHarness）的持有人，同时是"webview ↔ 会话"的路由器。
...
- **构成**：2 个接口＋8 个字段＋构造器＋16 个方法（9 公开 7 私有）＋4 个文件级函数（1 个导出）。
...
- **设计主线一：双表多对多。** 两张 Map 各管一件事：`sessions`（会话 id → SessionRuntime）——引擎里有哪些活会话；`sessionByView`（webviewId → 会话 id）——每个视图当前看哪个会话（视图＝一个 webview 聊天页面实例，[02-BridgeHandler方法详解.md](02-BridgeHandler方法详解.md) 第三节开头有定义与代码）。由此定下两条规则：**一个会话可以被多个视图同看**（侧边栏＋面板同时开同一会话）、**一个视图同一时刻只绑一个会话**。
  ...
  严格说类里还有第三张 Map——`viewChains`（`kimi-runtime.ts:59`），但它**不参与路由**：值是 Promise，作用是让同一视图的 open/attach/detach 排队串行（#3276 加的，见第三节"第〇步"）。
...
- **设计主线三：实例层级——每层各几个。** 从插件启动起是一条单例链：下面三个类，每个在整个插件代码里都只被 new 一处（已用 grep 全代码搜索确认，不是推断）—— `provider = new KimiWebviewProvider()`（`extension.ts:24`）→  `this.bridgeHandler = new BridgeHandler()` （`KimiWebviewProvider.ts:29`）→  `this.runtime = new KimiRuntime({})` （`bridge-handler.ts:42`）。
  ...
  **KimiHarness 一个**（**引擎的门面**。引擎＝真正干活的那套程序：收到你的消息后调用模型、执行工具（读/改文件、跑命令）、把过程用事件流吐回来、把会话存进homeDir——它的实现在本仓库的 packages/agent-core-v2（默认走这条）和 packages/agent-core（`useAgentCoreV1` 回滚时走这条）。KimiHarness 是 SDK 暴露出来的门面，门面后面才是引擎实体——**DI 容器，代码里就是 `this.app` 这个对象**（下面关停链的最后一步 `this.app.dispose()` 销掉的就是它；"DI 容器是什么"见关停链之后那段）。
  ...
  harness 在 **KimiRuntime 的构造函数里创建**（`kimi-runtime.ts:67-77`：`this.harness = options.harness ?? createHarness({ homeDir, identity, uiMode: "vscode" })`）——插件启动时随 KimiRuntime 一起出生，全程只这一个。
  ...
  插件停用（VS Code 停用插件 / 窗口关闭）
   → provider.dispose()（KimiWebviewProvider.ts:40/:44）
     → BridgeHandler.dispose()（bridge-handler.ts:292-295：先关 FileManager，再往下）
       → KimiRuntime.dispose()（kimi-runtime.ts:260-267：置 closed → 并发 close 全部 SessionRuntime → 清空双表）
         → harness.close()（kimi-runtime.ts:266，实现在 sdk-rpc-client-v2.ts:502 起）
  ...
  **SessionRuntime 若干个**——`sessions` 表里每个**活会话**一个。活会话＝当前在内存里开着的会话（用户正在聊的、或从历史恢复出来还没关的）；相对的是躺在磁盘上的历史会话——那不算，要 `resumeSession` 把它恢复成活的才进这张表（openSession 里 `this.sessions.get(requestedId)` 查的就是"表里有没有它"，`kimi-runtime.ts:108`）。造它的是 `wrapSession`（`new SessionRuntime({ session, ... })`，`kimi-runtime.ts:270`；出生链第 2 步细讲）。它从表里消失只有两条路（第五节拆解）：**被动**——看着它的视图陆续退订，最后一个也退了（`runtime.subscribers.length === 0`，`kimi-runtime.ts:213-216`，从表里删并 close）；**主动**——`closeSession`/`deleteSession`/`dispose` 不看视图数直接关。

  会话的出生链——两次构造有先后，都发生在 openSession 的新建分支（第三节分支③）里：

  - **第 1 步，SDK Session 出生，构造它的是引擎**：KimiRuntime 的 openSession 调 `await this.harness.createSession({ workDir, model, ... })`（`kimi-runtime.ts:117`；恢复历史会话则是 `this.harness.resumeSession({ id: requestedId, includeSubagents: true })`，`kimi-runtime.ts:124`）——对象在引擎内部被创建，这次调用一返回，KimiRuntime 手里就拿到了它；

  - **第 2 步，SessionRuntime 出生，构造它的是 KimiRuntime**：紧接着走本类的私有方法 `wrapSession`（`kimi-runtime.ts:269-279`），关键一行是 `new SessionRuntime({ session, ... })`（`kimi-runtime.ts:270`）——把刚拿到的 SDK Session 当构造参数递进去，造完存进 `sessions` 表；

  - **SessionRuntime 从头到尾不构造 SDK Session，只持有它**：构造函数里 `this.session = options.session;`（session-runtime.ts:94）存成字段，之后跑对话都是调 `this.session.prompt(...)`（session-runtime.ts:176）。
...
| `broadcast` | `kimi-runtime.ts:54` | `RuntimeBroadcast` | 从 BridgeHandler 来，只转手递给 SessionRuntime（`kimi-runtime.ts:273`），本类零调用（见 broadcast 链条篇） |
| `captureBaseline` | `kimi-runtime.ts:55` | 回调 | 同上，只在 wrapSession 转手（`kimi-runtime.ts:274`），本类零调用 |
| `log` | `kimi-runtime.ts:56` | 回调 | 错误日志出口，本类 3 处调用（`kimi-runtime.ts:139/:191/:288`） |
...
| `viewChains` | `kimi-runtime.ts:59` | `Map<string, Promise<void>>` | 每 webviewId 一条 promise 链，serializeView 用它把同视图的会话操作串行化（#3276） |
| `closed` | `kimi-runtime.ts:60` | `boolean` | dispose 后置 true，后续 openSession 被 ensureOpen 拒绝 |

**17 个成员（构造器＋16 方法：9 公开 ＋ 7 私有）**（"主要调用方"列附源码原文）。#3276 之后三个公开入口都成了**薄壳**：壳只做一件事——把活儿塞进该视图的串行队列（serializeView），原逻辑整体移进同名的 `*Inner` 私有方法：

| 成员 | 行号 | 可见性 | 一句话作用 | 主要调用方（源码原文） |
|---|---|---|---|---|
| `constructor` | `kimi-runtime.ts:62-78` | 公开 | 选引擎（v1/v2）并造 harness | bridge-handler.ts:42：`this.runtime = new KimiRuntime({`（唯一） |
| `getSessionForView` | `kimi-runtime.ts:80-83` | 公开 | 视图绑的会话 | bridge-handler.ts:166（唯一）：`getSession: () => this.runtime.getSessionForView(webviewId),` |
| `getSession` | `kimi-runtime.ts:85-87` | 公开 | 按 id 查活会话 | 4 处，见第二节全表 |
| `openSession` | `kimi-runtime.ts:89-91` | 公开 | **壳**：排进视图串行队列后转 openSessionInner | bridge-handler.ts:169（唯一）：`const runtime = await this.runtime.openSession({` → chat.handler.ts:94：`runtime = await ctx.getOrCreateSession(` |
| `openSessionInner` | `kimi-runtime.ts:93-149` | 私有 | 开/复用/恢复会话并绑定视图（三分支＋收尾三步） | openSession 壳（唯一）：`return this.serializeView(options.webviewId, () => this.openSessionInner(options));` |
| `attachResumedSession` | `kimi-runtime.ts:151-159` | 公开 | **壳**：排队后转 attachResumedSessionInner | bridge-handler.ts:193（唯一）：`const runtime = await this.runtime.attachResumedSession(` → session.handler.ts:130：`const runtime = await ctx.resumeSession(params.kimiSessionId);` |
| `attachResumedSessionInner` | `kimi-runtime.ts:161-200` | 私有 | 把一个已在手的 Session 绑到视图 | attachResumedSession 壳（唯一） |
| `detachView` | `kimi-runtime.ts:202-204` | 公开 | **壳**：排队后转 detachViewInner | 宿主 3 处，见第五节全表 |
| `detachViewInner` | `kimi-runtime.ts:206-217` | 私有 | 拆视图绑定；末位视图触发关会话 | detachView 壳＋类内 3 处直调（openSessionInner `kimi-runtime.ts:112/:135`、attachResumedSessionInner `kimi-runtime.ts:172`——已在队列里，直调避免重复排队死锁） |
| `serializeView` | `kimi-runtime.ts:219-234` | 私有 | 同一视图的会话操作串行化（promise 链队列） | 三壳各一次；详见第三节"第〇步" |
| `closeSession` | `kimi-runtime.ts:236-247` | 公开 | 主动关某会话（不管视图数；**不排队**——按 sessionId 操作，无同视图竞争） | deleteSession `kimi-runtime.ts:250`（类内）：`await this.closeSession(id);`；测试 test/kimi-runtime.test.ts:668：`await runtime.closeSession(opened.id);` |
| `deleteSession` | `kimi-runtime.ts:249-252` | 公开 | 关会话＋删磁盘数据 | session.handler.ts:185（唯一，DeleteKimiSession RPC）：`await ctx.runtime.deleteSession(params.sessionId);` |
| `setYoloModeForActiveSessions` | `kimi-runtime.ts:254-258` | 公开 | 全局 yolo 设置变化时刷新所有活会话 | KimiWebviewProvider.ts:149：`await this.bridgeHandler.runtime.setYoloModeForActiveSessions(enabled);` ← extension.ts:63：`?.setYoloModeForActiveSessions(VSCodeSettings.yoloMode)` |
| `dispose` | `kimi-runtime.ts:260-267` | 公开 | 全关：所有会话＋引擎 | bridge-handler.ts:294（唯一）：`await this.runtime.dispose();` |
| `wrapSession` | `kimi-runtime.ts:269-279` | 私有 | new SessionRuntime 并入 sessions 表 | openSessionInner `kimi-runtime.ts:136`、attachResumedSessionInner `kimi-runtime.ts:188`：`runtime = this.wrapSession(session, approval);` |
| `readMigratedLegacyApproval` | `kimi-runtime.ts:281-291` | 私有 | 三级恢复链的第二级（读旧 kimi-cli state.json） | openSessionInner `kimi-runtime.ts:129`、attachResumedSessionInner `kimi-runtime.ts:179`：`(await this.readMigratedLegacyApproval(session))` |
| `ensureOpen` | `kimi-runtime.ts:293-295` | 私有 | closed 守卫 | openSessionInner `kimi-runtime.ts:94`（唯一）：`this.ensureOpen();` |

**4 个文件级函数**：

| 函数 | 行号 | 被谁调用（源码原文） |
|---|---|---|
| `applySessionSettings` | `kimi-runtime.ts:298-313` | openSession 三分支各一次：`kimi-runtime.ts:103` `await applySessionSettings(current.session, options, current.legacyApprovalFlags);`、`kimi-runtime.ts:111/:134` `await applySessionSettings(session, options, approval);` |
| `normalizeEffort` | `kimi-runtime.ts:315-317`（**导出**） | openSession `kimi-runtime.ts:120`：`thinking: normalizeEffort(options.effort),`；chat.handler.ts:114：`const effort = normalizeEffort(params.effort ?? (params.thinking === true ? "on" : "off"));` |
| `flagsDiffer` | `kimi-runtime.ts:319-321` | openSession `kimi-runtime.ts:131`、attachResumedSession `kimi-runtime.ts:182`：`if (storedApproval === undefined \|\| flagsDiffer(storedApproval, approval)) {` |
| `assertSessionWorkDir` | `kimi-runtime.ts:323-327` | openSession `kimi-runtime.ts:110`：`assertSessionWorkDir(runtime.session, options.workDir);`、`kimi-runtime.ts:126`：`assertSessionWorkDir(session, options.workDir);` |

## 一、constructor：选引擎，一次定终身

```ts
// kimi-runtime.ts:62-78
...
**引擎选择不支持热切换**——改动要等下次重载窗口、runtime 重建时才生效，这是有意的：中途换引擎等于所有活会话背后的引擎对象被连根换掉，没有平稳迁移的路径。构造函数里还有两处值得注意，各干各的事：
  ...
  - **`options.harness` 是留给测试的口子**（kimi-runtime.ts:67-68 `this.harness = options.harness ?? createHarness({...})`）。先看真引擎时"关一个没打开的会话"这条调用怎么一层层下去，再看测试的假货替换了哪一层。

    **第 1 层·本类的方法**（kimi-runtime.ts:236-241）——查内存表，查不到就交给 harness：
    ...
    **第 2 层·`KimiHarness` 包装类**——`harness` 字段的类型，也是**两个工厂共同返回的同一个类**（kimi-harness.ts:72）。不管 v1 还是 v2，工厂干的事一样：先造一个底层客户端，再用 KimiHarness 包一层：
    ...
    // v2 工厂（sdk-rpc-client-v2.ts:2645-2647）：同一个包装类，只是吃进 v2 客户端
    ...
    **第 4 层·底层客户端的 `closeSession`**——第 3 层 `this.rpc` 的来路（"包"发生在两处）：
    ...
    // v2 版本（sdk-rpc-client-v2.ts:1357-1361）——覆盖且不调 super，所以基类那份在 v2 下永不执行
    ...
    上面基类里的 `getRpc()` 不是没人实现——它是基类的**抽象方法**（rpc.ts:194 `protected abstract getRpc(): Promise<ResolvedCoreAPI>;`），两边各有实现，但含义完全不同：v1 是真实现（sdk-rpc-client.ts:111-113 `return this.ready;`——返回 v1 核心的 API 面）；v2 是一个**故意抛错的桩**（sdk-rpc-client-v2.ts:565-569）：
    ...
    **测试的假货替换的就是第 2 层往下的全部**：塞一个假对象（test/kimi-runtime.test.ts:178-237 的 `createFakeHarness`，末尾 `as unknown as KimiHarness` 冒充类型）。于是第 1 层代码里那句 `await this.harness.closeSession(id);`（kimi-runtime.ts:239，第 1 层代码块的最后一次调用）调到的不再是包装类和它背后的整条链，而是假对象自己的方法——它只把收到的 id 记进数组，往下哪层都不走：
    ...
    测试拿这些记录做断言，真断言长这样（test/kimi-runtime.test.ts:674-682，"delegates deletion after the active session has been closed"）：
    ...
    ② `closeSession` 有两条分支：**查不到会话 → 交给 harness**（kimi-runtime.ts:238-240，就是前面第 1 层代码块讲的那条；你在会话列表里删一个**没打开**的历史会话走的就是它）；**查到了 → 自己关**（kimi-runtime.ts:242-246：从 `sessions` 表删除、清掉它所有视图的绑定、`await runtime.close()`，第五节拆解）。**当前测试文件只覆盖了第二条**：全部测试里 `runtime.closeSession(` 只出现一次（test:668），那时会话刚 openSession 过、还在表里；"交给 harness"那条分支没有直接测试。
    ...
    **`KimiRuntime.sessions` vs `KimiHarness.activeSessions`——两张表是两层各自的“活会话”清单**：
    ...
    | 谁写入 | `wrapSession`（kimi-runtime.ts:277） | createSession / resumeSession 造出 Session 时（kimi-harness.ts:145/:210） |
    ...
    **而你真正常走的场景——删一个从没打开过的历史会话——两边都查不到**：`harness.closeSession` 的 `?.close()` 是**无害空操作**；真正删磁盘数据的是紧随其后的 `await this.harness.deleteSession(id)`（kimi-runtime.ts:251）——刚查了它的实现（kimi-harness.ts:283 起）：第一行也是同样的空操作兜底，第二行 `await this.rpc.deleteSession({ sessionId })` 才是真删。

    所以“交给 harness”那条分支的本质是**兜底**：不管这个 id 处于哪种状态（两边都没有＝空操作 无害；只有 harness 有＝ 关掉漏网的 Session），保证引擎侧活实例被告知关闭。


  - **`identity` 是这个插件报给服务端的身份**（kimi-runtime.ts:71-75：`productName: "kimi-code-vscode"`、`platform: "kimi_code_vscode"`）——它最终进 API 请求头（sdk-rpc-client-v2.ts:452：`requestHeaders: createKimiDefaultHeaders({ homeDir: this.homeDir, ...identity })`），服务端遥测据此区分一次请求来自 VS Code 插件还是 CLI。
...
### 第〇步：壳＋serializeView——同一视图的会话操作排队（#3276）

公开的 `openSession` 现在只是个壳，真活儿在 `openSessionInner`；壳唯一的职责是把活儿排进**本视图的串行队列**：

```ts
// kimi-runtime.ts:89-91 —— 壳：一行，排队后转 Inner
...
队列本体是 `serializeView`（`kimi-runtime.ts:219-234`），它头上那段源码注释就是存在理由的原文：
...
private serializeView<T>(webviewId: string, work: () => Promise<T>): Promise<T> {
  ...
  const run = prev.then(work, work);              // :224 前序无论成败都接上（失败不挡后续任务）
  const next = run.then(() => undefined, () => undefined);  // :225-228 吞掉结果的链尾
  this.viewChains.set(webviewId, next);           // :229 新链尾入表
  void next.finally(() => {
    if (this.viewChains.get(webviewId) === next) this.viewChains.delete(webviewId);  // :231 链尾仍是自己才删
  });
  return run;                                     // :233 调用方拿到的是带真实结果/异常的 promise
}
```

- **修的什么 bug**：webview 恢复（面板重载）时，`openSession` 与 `attachResumedSession` 可能对同一 webviewId **并发**进入。两个调用都在对方写入 `sessions` 表之前查表 miss → 各自把同一个 SDK Session 包成两个 SessionRuntime，后写覆盖先写，**被覆盖的实例没人释放、却还挂着事件监听**——每个流式 delta、每次 tool call 都广播两遍。
- **机制是 promise 链，不是锁**：按 webviewId 取当前链尾 `prev`，`prev.then(work, work)` 串行接上（前序失败也照样执行本任务）；返回的 `run` 保留真实结果/异常给调用方，另存一个吞错的 `next` 作新链尾——一次失败不会毒化整条链；任务结束后若链尾仍是自己则从 map 删除（防内存泄漏、防误删别人的后继）。
- **两个配套细节**：① 已在链内的 `openSessionInner`/`attachResumedSessionInner` 要拆旧绑定时**直调 `detachViewInner`**——再排队就是等自己，死锁；② `closeSession` 不入队——它按 sessionId 操作，没有"同一视图"的竞争维度。
- **对三分支的意义**：同一视图的"查表—写入"从此是原子区，分支①②的命中/miss 判断不会被并发对手插队。测试 test:399 `"does not double-wrap the SDK session when two opens race for it"`：两个 openSession 并发抢同一个会话，断言拿到**同一个** runtime、订阅数恰为 1。

```ts
// kimi-runtime.ts:94-106 —— openSessionInner 起步＋分支①：视图、会话、目录都没变
private async openSessionInner(options: OpenSessionOptions): Promise<SessionRuntime> {
  ...
  const requestedId = options.sessionId ?? current?.id;        // :96 请求的 id，没请求就用当前绑的

  if (
    current !== undefined &&
    requestedId === current.id &&
    areSameFsPath(current.session.workDir, options.workDir)    // :101 大小写/分隔符不敏感比较
  ) {
...
```ts
  // kimi-runtime.ts:108-148 —— 分支②③＋收尾（均在 openSessionInner 内）
  ...
  if (runtime !== undefined) {                                 // 分支②：会话仍在表里活着（必有别的视图正订阅(使用)着它）
    assertSessionWorkDir(runtime.session, options.workDir);    // :110 目录不符直接抛
    await applySessionSettings(runtime.session, options, runtime.legacyApprovalFlags);
    await this.detachViewInner(options.webviewId);             // :112 先拆本视图的旧绑定（已在队列内，直调）
  } else {
    const defaultApproval: LegacyApprovalFlags = { yolo: options.yoloMode, afk: false };
    const session =
      requestedId === undefined
        ? await this.harness.createSession({ workDir, model, thinking: normalizeEffort(effort), ... })  // :117 全新
        : await this.harness.resumeSession({ id: requestedId, includeSubagents: true });               // :124 恢复
    try {
      assertSessionWorkDir(session, options.workDir);          // :126 恢复出来的会话目录也得对
      ...
      if (storedApproval === undefined || flagsDiffer(storedApproval, approval)) {
        await session.updateMetadata(legacyApprovalMetadata(approval));                    // :132 变了才写回
      }
      await applySessionSettings(session, options, approval);
      await this.detachViewInner(options.webviewId);           // :135
      runtime = this.wrapSession(session, approval);           // :136 → 分支③出口
    } catch (error) {
      await session.close().catch((closeError: unknown) => {   // :138 拒收的会话关掉再抛
  ...
  runtime.subscribe(options.webviewId);                        // :145 收尾三步
  this.sessionByView.set(options.webviewId, runtime.id);       // :146
  await runtime.announceStatus(options.webviewId);             // :147
  return runtime;                                              // :148
}
```

分支②处理"会话还活着但不是本视图当前绑的"（典型：两个视图看同一个会话；或本视图还绑着旧会话 A、这次发消息却指名了 B——即**想要切换**：切换正是由分支②＋收尾三步完成的，先 `detachViewInner` 拆 A 再 subscribe 绑 B，等切完，下一条消息就命中分支①、不会再进②），还有一个边缘情况：requestedId === current.id 但 workDir 不一致掉下来——此时 runtime === current，进②后立刻被 assertSessionWorkDir 抛错，不算复用。

注意"从历史列表点开某会话"走的是另一条路（LoadKimiSessionHistory → resumeSession → attachResumedSession，`session.handler.ts:130`），那条路有自己的表内复用（`kimi-runtime.ts:166-173`），不经过 openSession——openSession 全插件唯一入口是发消息（`chat.handler.ts:94`），"切换"在这里表现为这次消息带的 sessionId ≠ 该视图当前绑定的会话。

分支③是真正的新建/恢复，**model/effort 只在 createSession 时生效**（`kimi-runtime.ts:118-120`），恢复的会话保留自己的模型——`kimi-runtime.ts:304-308` 

**1. 代码事实：分支③的两条子路径待遇不同**
```ts
  // kimi-runtime.ts:117-123 —— 新建：model/effort 被当作出生参数传进去
  ? await this.harness.createSession({
        workDir: options.workDir,
        model: options.model || undefined,        // :119 生效
        thinking: normalizeEffort(options.effort), // :120 生效
        ...
      })
  // kimi-runtime.ts:124 —— 恢复：只传 id，model/effort 根本不在参数里
  : await this.harness.resumeSession({ id: requestedId, includeSubagents: true });
```
  
而且不止恢复路径——分支①②复用活会话时调的 `applySessionSettings（kimi-runtime.ts:298-313）` 只同步权限标志，不碰 model/effort。那段“原注释”就写在这个函数里：
...
- 挑选器改动 → `SaveConfig RPC（config.handler.ts:40-76）`：把选择持久化为新的全局默认（:63-66），并且对当前活会话立刻
  `setModel/setThinking（:69-74）`
...
```
    onClick={() => updateModel(model.id)}
      ...
      → 扩展宿主侧 handler：config.handler.ts:40 saveConfig
          ├ 持久化为全局默认：harness.setConfig({ defaultModel, thinking })（:63-66）
          └ 对当前活会话立刻生效：setModel / setThinking（:69-74）★
```
...
**审批标志三级恢复链**（`kimi-runtime.ts:127-133`，attachResumedSession `kimi-runtime.ts:176-184` 同构）：
...
- 第③级默认值 `{ yolo: 全局设置, afk: false }`（`kimi-runtime.ts:114`：`const defaultApproval: LegacyApprovalFlags = { yolo: options.yoloMode, afk: false };`）。
...
**收尾三步**（`kimi-runtime.ts:145-147`）是所有分支共享的出口：`subscribe`（视图进会话的订阅集）→ 双表之二 `set`（绑定）→ `announceStatus`（状态播报）。测试对这条链有四个直接断言（测试标题原文即文件内容）：

- test:484 `it("preserves the resumed session's model instead of reapplying the configured default", async () => {`

- test:494 `it("preserves the resumed session's thinking effort instead of reapplying the configured default", async () => {`

- test:504 `it("announces the session's actual status to the attaching view so the display matches it", async () => {`

- test:551 `it("lets the global yolo setting override a persisted off flag on resume", async () => {`（test:587 `it("keeps the persisted afk flag while applying the global yolo setting on resume", async () => {`）
...
```ts
// kimi-runtime.ts:151-159 —— 壳：同样先排队
...
// kimi-runtime.ts:161-200 —— Inner（节选）
private async attachResumedSessionInner(webviewId: string, session: Session, defaultYoloMode: boolean): Promise<SessionRuntime> {
  const existing = this.sessions.get(session.id);
  if (existing !== undefined && this.sessionByView.get(webviewId) === session.id) {  // :167 已绑给本视图
    existing.subscribe(webviewId);
    await existing.announceStatus(webviewId);
    return existing;                                            // :170 幂等重入
  }
  await this.detachViewInner(webviewId);                        // :172 直调（已在队列内）
  let runtime = existing ?? this.sessions.get(session.id);
  if (runtime === undefined) {                                  // :173-174 会话不在表里 → wrap 一条
    try {
      ...
      const status = await session.getStatus();                 // :185 与 openSession 的差异点：
      const permission = corePermissionForLegacyApproval(approval);
      if (status.permission !== permission) await session.setPermission(permission); // :187
      runtime = this.wrapSession(session, approval);
    } catch (error) { /* 同 :189-194：close＋重抛 */ }
  }
  runtime.subscribe(webviewId);                                 // :196 收尾三步同 openSessionInner
...
与 openSessionInner 分支③的两处差异：这里的权限走 `getStatus + setPermission` 直改（`kimi-runtime.ts:185-187`，因为 Session 是外面 resume 出来的，可能带着旧权限）；openSessionInner 里则统一由 `applySessionSettings` 处理。测试 "reattaches the same resumed session without replacing its handlers"（test:650：`it("reattaches the same resumed session without replacing its handlers", async () => {`）锁的是幂等分支：同一视图重入不换 handler（不重复 setApprovalHandler，避免双份审批回调）。

## 五、拆与关家族：detachView / closeSession / deleteSession / dispose

```ts
// kimi-runtime.ts:202-217 —— 壳(:202-204)＋detachViewInner(:206-217)
...
**谁调用**（最多面的方法，6 处全列；#3276 后分两类——宿主走**壳**（排队），类内走 **Inner**（已在队列里，直调防死锁））：

- bridge-handler.ts:125（setCustomWorkDir 换目录时，壳）：`await this.runtime.detachView(webviewId);`

- bridge-handler.ts:202（context 的 closeSession，"新会话"按钮，壳）：`await this.runtime.detachView(webviewId);`

- bridge-handler.ts:216（disposeView，webview 关闭，壳）：`await this.runtime.detachView(webviewId);`

- 类内 openSessionInner `kimi-runtime.ts:112`/`kimi-runtime.ts:135`、attachResumedSessionInner `kimi-runtime.ts:172`（换绑前先拆旧的，三处都是 `await this.detachViewInner(options.webviewId);` 或 `await this.detachViewInner(webviewId);`——直调 Inner）。

**删除顺序是双表之二先于一**：先解视图→会话的引用，才轮到会话本身被关。末位判定 `subscribers.length === 0` 就是引用计数清理：两个视图同看一个会话，关一个另一个照常（test:627：`it("keeps a shared session open when one of its Webview detaches", async () => {`）；最后一个关掉，会话整个 close（test:639：`it("closes an SDK session when its last Webview detaches", async () => {`）。

```ts
// kimi-runtime.ts:236-252
async closeSession(id: string): Promise<void> {
  const runtime = this.sessions.get(id);
...
`closeSession` 与 `detachView` 方向相反：detachView 从**视图**出发（可能只拆不关），closeSession 从**会话**出发（必关，且把它所有视图的绑定一并清掉，test:663：`it("removes every Webview mapping when a shared session is closed", async () => {`）。它的外部调用方只有 deleteSession（类内 `kimi-runtime.ts:250`：`await this.closeSession(id);`）和测试（test:668：`await runtime.closeSession(opened.id);`）——**生产代码里没有"只关会话不删数据"的入口**，UI 的删除走 `deleteSession`（session.handler.ts:185：`await ctx.runtime.deleteSession(params.sessionId);`）。不在表里的 id 也要能关：harness 里可能有本 runtime 没绑定过的休眠会话（比如 fork 出来还没人看过的），`harness.closeSession` 兜住它们。

```ts
// kimi-runtime.ts:260-267
...
**谁调用**：bridge-handler.ts:294（插件 dispose，`await this.runtime.dispose();`）。顺序：先置 closed（挡住之后的 openSession）→ 并发关所有会话 → 清双表 → 关引擎。test:685 `it("closes every active SDK session when the host runtime is disposed", async () => {` 锁这条链。

## 六、setYoloModeForActiveSessions：设置变化时刷新所有活会话

```ts
// kimi-runtime.ts:254-258
...
全类**只有这里 new SessionRuntime**，openSessionInner `kimi-runtime.ts:136`（`runtime = this.wrapSession(session, approval);`）与 attachResumedSessionInner `kimi-runtime.ts:188`（同款调用）两条路径汇于此。递下去的四样：SDK Session、算好的审批标志、以及从 BridgeHandler 一路转手来的 broadcast/captureBaseline/log（broadcast 这根线的旅程见 [dive-chain-broadcast链条详解.md](dive-chain-broadcast链条详解.md) 第 2-1 站——本类对它们**只转手、零调用**，grep 全文件可证：`broadcast` 只出现在 `kimi-runtime.ts:24`（类型）、`kimi-runtime.ts:54`（字段）、`kimi-runtime.ts:63`（赋值）、`kimi-runtime.ts:273`（转递）四处）。造完立即 `sessions.set` 入表——**入表和造对象在同一段代码里完成**，不存在"造了还没入表"的窗口。

## 八、readMigratedLegacyApproval：三级链的第二级（私有）

```ts
// kimi-runtime.ts:281-291
...
`ensureOpen`（`kimi-runtime.ts:293-295`）就一行：

```ts
// kimi-runtime.ts:293-295
private ensureOpen(): void {
  if (this.closed) throw new Error("Kimi runtime is closed.");
}
```

openSession 的第一道门（`kimi-runtime.ts:94`：`this.ensureOpen();`），dispose 后的迟到请求在这里被拦。

```ts
// kimi-runtime.ts:298-313
...
注意它**不动 model/effort**（注释 `kimi-runtime.ts:304-308` 解释了为什么——见第三节）。三个参数分别来自：openSession 的 options、和那条会话当前生效的审批标志。`corePermissionForLegacyApproval`（legacy-approval.ts:56-59）把两布尔翻译成引擎的 PermissionMode：
...
`normalizeEffort`（`kimi-runtime.ts:315-317`，导出）：

```ts
// kimi-runtime.ts:315-317
...
`flagsDiffer`（`kimi-runtime.ts:319-321`）：两布尔对比（`return a.yolo !== b.yolo || a.afk !== b.afk;`），"变了才写回"的门闸。`assertSessionWorkDir`（`kimi-runtime.ts:323-327`）：

```ts
// kimi-runtime.ts:323-327
...
`areSameFsPath` 大小写/分隔符不敏感比较——**会话与目录的锁死关系**在 openSessionInner 的两个调用点（`kimi-runtime.ts:110/:126`）把关，测试 test:698 `it("does not retain a resumed session when it belongs to a different working directory", async () => {` 锁的就是它。
...
4. **拒收的会话必须 close 再抛。** openSessionInner `kimi-runtime.ts:137-142` 与 attachResumedSessionInner `kimi-runtime.ts:189-194` 的 catch 块。反例：直接抛，harness 里积一个谁也不绑的活会话；它是内存里的 SDK 对象，不关就泄漏到插件停用。
...
7. **同一视图的会话操作互斥串行（#3276）。** "一个视图同一时刻只绑一个会话"这条规则曾只是约定——直到 webview 恢复时 openSession 与 attachResumedSession 并发进入，双双查表 miss，把同一个 SDK Session 包成两个 SessionRuntime，后写的覆盖先写的、被覆盖的还挂着监听双发事件。修复不是加锁，而是 per-view 的 promise 链（serializeView）：壳排队、Inner 干活、链内直调 detachViewInner 防死锁。反例：只靠"调用方不会并发"的假设，规则的例外就是并发 bug 的入口；串行化把"查表—写入"变成原子区，规则才真正成立。
