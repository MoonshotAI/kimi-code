...
- **config domain（KimiConfig 读 / 写 facade）**：daemon / SDK 读写全局
  ...
  domain **自己不拥有真相**，而是经 `ICoreProcessService` 的 in-process
...
- `services/config`（config domain）只做**KimiConfig 投影 + 变更通知**，不
  ...
  `ICoreProcessService`（真相 owner）和 `IEventService`（变更通知）。
...
- **config（全局配置投影）**：读写全局 `KimiConfig`。键是单一的 “the
  ...
  不持真相，经 `ICoreProcessService.getCoreApi()` 委托给 core。生命周期由
...
- config domain **不持有真相**：`KimiConfig` 的真相在 core 进程（
  ...
  `ICoreProcessService.getCoreApi()`（in-process CoreAPI，跳过 JSON 序列
...
IConfigService.get                 → ICoreProcessService.getCoreApi().getKimiConfig + toConfigResponse (query → core 投影)
IConfigService.set                 → ICoreProcessService.getCoreApi().setKimiConfig + IEventService.publish (command → core + 变更通知)
...
- terminal 的 command 不调用 attach/detach 业务方法；runtime 也不回调
  ...
  法”。
- config 的所有读 / 写都经 `ICoreProcessService.getCoreApi()`（in-process
...
- **DR4：config 是 core config 真相之上的投影 + 变更通知 facade，不是
  ...
  `configPath` 文件），经 `ICoreProcessService.getCoreApi()` 委托给
