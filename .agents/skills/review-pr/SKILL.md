...
| 改动类型 | 第三遍深度 |
|---|---|
| 纯内部重构、测试、文档、CI | 只需给出"没有可观测变化"的证据：哪些分支、默认值、契约文件没动 |
...
每一行记录：行为、变化前、变化后、变化类型（新增 / 修改 / 删除 / 默认翻转 / 收紧 / 放宽）、证据（改动前后各一个 `file:line`）。
...
|---|---|
| L0 | 用户无感知 |
| L1 | 可感知，有一键退路（config、env、flag），且 changeset 写明了失去的旧行为 |
| L2 | 可感知，无退路，旧行为不可恢复 |
| L3 | 静默：数据丢失、配置被静默忽略、动作被静默批准或跳过、安全边界放宽、老数据不可读。用户收不到任何错误，无法自行发现 |

广度三档，必须写出人群名字：全体用户 / 某类配置或平台人群 / 边缘环境。

每一行单独定级，PR 的等级取最高。静默优先于响亮：一个静默的 L3 比一个会报错的崩溃更严重，因为用户没有信号可以上报。
...
- changeset：有没有点明用户**失去了什么**，而不只是新默认是什么。
...
- finding 必须同时有：改动前后的 `file:line` 证据，和一个具名人群及其存在证据。两者缺一，降级为"待作者确认"。
