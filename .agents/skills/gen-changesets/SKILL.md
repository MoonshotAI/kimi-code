...
- When a default flips or an existing behavior is removed, name the behavior users lose, not only the new default, and state the escape hatch, e.g. `Stop hot-reloading config, skills, and AGENTS.md by default; set [watch] enabled = true or KIMI_CODE_WATCH=1 to turn it back on.`
