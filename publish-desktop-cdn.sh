...
# 安装包地址，所以指针 yml 里的 path/url 会被改写为带 <version>/ 前缀（yml
# 内的 sha512 是文件内容哈希，与路径无关，改写安全）。
#
...
rclone listremotes | grep -q '^oss:' \
  || { echo "error: rclone 没配 'oss' remote，参 kimi-cli-cdn-sync 仓 README '前置依赖'"; exit 1; }
...
# 把 yml 内 path/url 的文件名改写为 <version>/ 前缀，生成根目录指针。
_rewrite_pointer_yml() {
  local src="$1" dest="$2"
  sed -E \
    -e "s|^([[:space:]]*- url: )([^ ]+)$|\1${VERSION}/\2|" \
    -e "s|^(path: )([^ ]+)$|\1${VERSION}/\2|" \
    "$src" > "$dest"
  grep -q "path: ${VERSION}/" "$dest" \
...
echo "==> [1/2] Uploading versioned artifacts to tos://${TOS_BUCKET}/${DESKTOP_PREFIX}/${VERSION}/"
echo "==> Cache-Control: ${CACHE_CONTROL_VERSIONED}"
rclone copy "$TMP/" "oss:${TOS_BUCKET}/${DESKTOP_PREFIX}/${VERSION}/" \
...
if [ "$ARTIFACTS_ONLY" -eq 0 ]; then
  ...
  rclone copy "$POINTER_DIR/" "oss:${TOS_BUCKET}/${DESKTOP_PREFIX}/" \
  ...
  for suffix in "${DOWNLOAD_SUFFIXES[@]}"; do
    ...
    rclone copyto \
      "oss:${TOS_BUCKET}/${DESKTOP_PREFIX}/${VERSION}/${src}" \
      "oss:${TOS_BUCKET}/${DESKTOP_PREFIX}/download/KimiCode-${suffix}" \
      -M --metadata-set "cache-control=${CACHE_CONTROL_POINTER}" \
      -v
  done
else
...
rclone lsf "oss:${TOS_BUCKET}/${DESKTOP_PREFIX}/${VERSION}/"

echo ""
if [ "$ARTIFACTS_ONLY" -eq 0 ]; then
  echo "✅ Published desktop ${VERSION} 并切换 latest*.yml 指针 + 固定下载入口"
  echo ""
  echo "自查（code.kimi.com 会 302 到 cdn.kimi.com，curl 需 -L 跟随）:"
