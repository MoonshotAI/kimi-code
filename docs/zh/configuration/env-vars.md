...
| `KIMI_CODE_REPEAT_BREAKER` | 同一工具调用连续重复多次时，是否注入提醒并最终强制停止该轮，优先级高于 `config.toml` 的 `repeat_breaker` | 真值：`1`/`true`/`yes`/`on`；假值：`0`/`false`/`no`/`off` |
| `KIMI_CODE_TOOL_DEDUPE` | 同一步骤内相同的工具调用是否合并为一次执行并共享结果，优先级高于 `config.toml` 的 `tool_dedupe` | 真值：`1`/`true`/`yes`/`on`；假值：`0`/`false`/`no`/`off` |
