...
| `KIMI_TOKEN_COUNTING_STRATEGY` | 上下文 token 计数的来源；优先级高于 `config.toml` 的 `[token_counting] strategy`（默认 `measured+estimated`） | `measured+estimated`、`measured`、`estimated`（不区分大小写）；非法值被忽略 |
