...
| `KIMI_CODE_EXPERIMENTAL_AGENT_ENVIRONMENT_TOOLS` | Agent 环境工具，默认开启：main agent 获得 `change_environment`（切换会话环境，从下一次工具调用起生效）和 `connect`（从启动器规格创建临时环境），以及 `Agent` 的 `environment` 参数，详见 [远程环境](../guides/remote-environment.md#agent-环境工具)；设为假值可关闭 | 真值：`1`/`true`/`yes`/`on`；假值：`0`/`false`/`no`/`off`；未设置 = 开启 |
