...
| `KIMI_CODE_EXPERIMENTAL_REMOTE_RUNTIME` | 启用实验性远程运行时：会话可绑定到 `[runtimes]` 配置节中声明的 SSH 主机、Docker 容器或自定义启动器，Agent 的工具即在目标环境中执行，详见 [远程运行时](../guides/remote-runtime.md)；`KIMI_CODE_EXPERIMENTAL_FLAG=1` 也会一并启用 | 真值：`1`/`true`/`yes`/`on`；假值：`0`/`false`/`no`/`off` |
