...
| `KIMI_CODE_EXPERIMENTAL_TUI_TABS` | 启用实验性 TUI 会话标签页：`/new tab`（Alt+T）在新标签页中打开会话且当前会话继续运行，`/sessions` 中按 Ctrl+T 在新标签页恢复会话，`/tab <n>` / Alt+1..9 / Ctrl+Tab 切换标签页，`/tab close`（Alt+W）关闭标签页 | 真值：`1`/`true`/`yes`/`on`；假值：`0`/`false`/`no`/`off` |
