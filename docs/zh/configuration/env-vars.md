...
| `KIMI_SUBAGENT_MODEL` | 子 Agent 的次主力模型；优先级高于 `config.toml` 的 `[subagent] model`。设置后新派生的子 Agent 默认绑定该模型，而不再继承主 Agent 的模型 | 已配置的模型别名，如 `kimi-code/kimi-k2.5`；空白值被忽略 |
| `KIMI_SUBAGENT_EFFORT` | 次主力子 Agent 模型的 thinking effort；优先级高于 `config.toml` 的 `[subagent] effort`，仅与次主力模型同时生效 | effort 取值，如 `low`；空白值被忽略 |
