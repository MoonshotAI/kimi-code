...
`KIMI_CODE_IDENTITY_*` 和 `KIMI_CODE_BUILTIN_PRODUCT_SKILLS` 这三个变量由 `agent-core-v2` 引擎读取，目前 `kimi web` 和开启 `KIMI_CODE_EXPERIMENTAL_FLAG` 的路径使用该引擎；默认的 `kimi` / `kimi -p` 引擎会忽略它们。反过来，`KIMI_CODE_EXPERIMENTAL_SECONDARY_MODEL`、`KIMI_SECONDARY_MODEL` 和 `KIMI_SECONDARY_EFFORT` 仅由默认引擎读取，v2 引擎会忽略它们。
