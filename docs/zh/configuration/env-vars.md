...
| `KIMI_SECONDARY_MODEL` | 次主力模型；优先级高于 `config.toml` 的 `[secondary_model] model`。设置后新派生的子 Agent 默认绑定该模型，而不再继承主 Agent 的模型（仅 `kimi web` 与实验性 `kimi -p` 生效，TUI 不生效） | 已配置 `[models]` 中的模型 id，如 `kimi-code/kimi-k2.5`；空白值被忽略 |
| `KIMI_SECONDARY_EFFORT` | 次主力模型的 thinking effort；优先级高于 `config.toml` 的 `[secondary_model] default_effort`，仅与次主力模型同时生效（仅 `kimi web` 与实验性 `kimi -p` 生效，TUI 不生效） | effort 取值，如 `low`；空白值被忽略 |
