...
单独存在的遗留 `model` 键（由旧版本写入）在未配置池键时仍作为池默认值生效，但在派发时 `default_model` 和 `models` 的优先级高于它。通过 TUI、Web 界面或配置 API 写入时，两族按键之间遵循 "后写生效"：写入 `default_model` 或 `models` 会移除残留的 `model` 键，写入 `model` 会移除 `default_model` 和 `models`，因此最终生效的总是最近一次设置。
