...
| `reasoning_key` | `string` 或 `array<string>` | 否 | 仅 `openai` 供应商；网关用非标准字段名返回推理内容时才需要设置，默认自动识别 `reasoning_content` 等。字符串表示只读该字段；数组表示只在列出的字段中按给定顺序匹配（回显推理内容时，在尚未观测到任何字段前使用数组第一个元素） |
