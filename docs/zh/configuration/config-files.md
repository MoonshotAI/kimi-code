...
| `tui_mode` | `string` | `regular` | 界面渲染模式：`regular`（内联视口）或 `fullscreen`（alternate-screen 全屏界面：transcript 为可滚动视口，支持鼠标选择、滚动条与 Ctrl-Shift-F 搜索；重启后生效） |
...
tui_mode = "regular" # "regular" | "fullscreen"（alternate-screen 全屏界面，重启后生效）
