...
| `default_effort` | `string` | 否 | 模型的默认 Thinking 档位。在 `/model` 或 `/effort` 中选择高于此默认值的档位时，选择仅对当前会话生效，不会保存为全局默认。managed 和 open-platform 刷新可能会改写该字段；如需手动固定，请改用 `[models."<alias>".overrides] default_effort` |
