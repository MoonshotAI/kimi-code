...
`experimental` 存放实验功能 flag 的持久化覆盖。目前 `micro_compaction` 是唯一用户可见的字段，默认值为 `true`；只有在需要关闭自动清理较旧的大型工具结果时，才需要把它设为 `false`。为了兼容旧配置，未知字段也会被接受：它们在写回配置文件时会被保留，但只有当前已注册的实验功能 ID 会影响运行时行为。
