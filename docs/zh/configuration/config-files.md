...
| `[status_line].command` | `string` | `""` | 自定义状态栏命令。其 stdout 第一行替换状态栏第一行，stdin 会收到 JSON 快照（model、cwd、git 分支、permission 模式、plan 模式、上下文用量、托管计划用量、session id、版本）；用量不可用时 `managedUsage` 为 `null`。运行上限 300ms、每秒最多一次；失败时回退内置布局 |
