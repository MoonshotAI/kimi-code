...
- **`tui.toml`**：终端界面客户端偏好。消息时间戳默认显示；设置 `show_timestamp = false` 可隐藏时间戳和 Assistant 回复耗时。运行 `/reload-tui` 即可应用修改，无需重启。该文件还控制 `[upgrade].auto_install`（自动更新，默认开启），可在 `/settings` 关闭，或设置 `auto_install = false`。
