...
- **`credentials/`**：OAuth 凭据目录，权限 `0o700`（目录）/ `0o600`（文件），仅当前用户可读写。托管供应商凭据存为 `credentials/<name>.json`，MCP server 凭据存在 `credentials/mcp/` 子目录下。凭据写入使用原子流程（tmp → fsync → rename）防止写损。启用实验性钥匙串存储（`KIMI_CODE_EXPERIMENTAL_KEYRING=1`，见[环境变量](./env-vars.md#运行时开关)）后，OAuth 凭据改存操作系统钥匙串，已有明文文件会在首次读取时迁移进钥匙串。
