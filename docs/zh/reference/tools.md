...
尾读优先返回请求范围中较新的完整行。如果连一条完整行都无法容纳，结果会给出未读范围的正向 `Next Read` 参数；`column_offset` 不能与负数 `line_offset` 同时使用。续读位置针对文件的当前内容，文件变化后应重新读取。如果尾读提示读取期间文件发生变化，请基于更新后的文件重试。10 MiB 以内的 UTF-16 LE/BE 文件会先尝试严格解码；无 BOM 且字节同时也是合法 UTF-8 的 UTF-16（例如 UTF-16LE 编码恰好落在 ASCII 字节区间的 CJK 文本）无法与 UTF-8 区分，将按 UTF-8 读取；失败后，`Read` 会将损坏序列替换为 `�` 并返回可读文本，同时在每一页提示发生了有损解码、文本可能与原文不同。告警也计入字符额度；有效文件中原本就有的 `�` 不会触发告警。图片和视频请使用 `ReadMediaFile`。

**`Write`** 接受 `path`、`content` 和可选的 `mode`（`overwrite` 或 `append`，默认覆盖）。缺失的父目录会自动创建；`append` 模式将内容追加到文件末尾，不自动添加换行，且追加的内容按字面以 UTF-8 写入（分块追加不会按文件既有编码重新编码——UTF-16 或有换行风格约定的目标请一次性写入完整内容）。写入已存在的文件（无论 `overwrite` 还是 `append` 模式）要求本会话中先用 `Read` 读过该文件——若文件自上次读取后在磁盘上发生变化，写入会被拒绝；新建文件不受此限。覆盖已存在的文件时，`Write` 会保留其字节顺序标记（BOM）与主导换行风格（CRLF、LF 或 CR）——有效的带 BOM UTF-16 会按 UTF-16 重新编码，而畸形的 UTF-16、以及经由 ACP 集成（仅传输文本）桥接的写入，均按 UTF-8 写入；无 BOM 的文件保持无 BOM，无法按文本解码的文件（二进制、UTF-32）回退为按 UTF-8 原样覆盖。新建文件时——包括通过 `append` 创建的文件——`Write` 会遵循仓库约定：优先采用最近的 `.editorconfig`（从文件所在目录向上搜索至工作区边界、遇 `root = true` 停止）中声明的 `end_of_line` 与 `charset`（`utf-8-bom`、`utf-16le`、`utf-16be`）；否则沿用同目录同扩展名兄弟文件的主导风格。完全没有任何信号时，新文件按提供的内容原样写入，不加 BOM。

**`Edit`** 接受 `path`、`old_string`（要替换的精确文本）和 `new_string`（替换后的文本）。默认只替换唯一一处匹配，若文件中存在多处相同内容会报错并提示使用 `replace_all: true`。`old_string` 与 `new_string` 不能相同。目标文件必须在本会话中先用 `Read` 读过；若文件自读取后在磁盘上发生变化，编辑会被拒绝。`Edit` 会保留文件开头的 UTF-8 BOM 与换行风格；带 UTF-16 或 UTF-32 BOM 的文件会被视为非 UTF-8 而拒绝，需先转换编码（例如使用 `iconv`）。
