...
**`Read`** 接受文件路径（`path`）以及可选的 `line_offset`（起始行号，支持负数从末尾倒数）、`n_lines`（请求读取的行数）和 `max_chars`（结果的字符上限，包含行号和状态信息）。省略 `n_lines` 时向文件末尾读取。默认上限为 100,000 字符，调用可申请到 500,000 字符；两个值都可通过 [`read` 配置](../configuration/config-files.md#read) 修改。字符数按 JavaScript 字符串长度计算：常见字母和汉字各计 1，许多 emoji 计 2。

结果保留完整行，不会再被通用工具输出限制缩短。请求范围无法一次返回时，状态信息会给出 `Next Read` 参数，用于继续读取遗漏范围而不重复已返回的行。尾读优先返回请求范围中较新的部分。如果单独一行也无法容纳，工具会明确报错，不会悄悄丢掉行尾；可申请更大的 `max_chars`，或用 `Bash` 提取较小的字符范围。图片和视频请使用 `ReadMediaFile`。
