...
尾读优先返回请求范围中较新的完整行。如果连一条完整行都无法容纳，结果会给出未读范围的正向 `Next Read` 参数；`column_offset` 不能与负数 `line_offset` 同时使用。续读位置针对文件的当前内容，文件变化后应重新读取。10 MiB 以内的 UTF-16 LE/BE 文件会严格解码，损坏的编码会返回读取错误。图片和视频请使用 `ReadMediaFile`。
