...
鼠标定位光标目前是实验功能。通过 `/experiments`、`[experimental].terminal_mouse_input = true` 或 `KIMI_CODE_EXPERIMENTAL_TERMINAL_MOUSE_INPUT=1` 启用后，在主输入框内点击会把光标移动到点击位置。启用这个 flag 时，很多终端会把普通鼠标拖选交给应用；需要选择终端文本时，请按住 `Shift` 再拖选。
