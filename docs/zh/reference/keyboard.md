...
| `Ctrl-C` | 有输入框选区时复制选中文本；无选区时执行原有取消、清空或退出操作 |
| 鼠标左键拖动 | 设置 `KIMI_CODE_EXPERIMENTAL_TERMINAL_MOUSE_INPUT=1` 后选择输入框文本 |

启用实验性终端鼠标输入后，单击可移动输入框光标，按住左键拖动可选择文本，并可使用 `Ctrl-C` 复制、删除键删除或直接输入进行替换。需要使用终端原生文本选择或回滚缓冲区时，请按住 `Shift` 操作。
