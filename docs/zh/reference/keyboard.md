...
| 快捷键 | 功能 |
| --- | --- |
| `Ctrl-O` | 展开或折叠工具输出、Shell 命令输出、被摘要收起的 Assistant 消息和压缩摘要 |

历史中存在折叠的工具调用结果或 Shell 命令输出时，按 `Ctrl-O` 可在折叠和展开之间切换。较早的 Assistant 消息被收进对话记录摘要后，同一个快捷键可以显示或隐藏这些消息。压缩完成后，它也会在压缩块中显示或隐藏压缩摘要。
