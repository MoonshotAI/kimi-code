...
| `/environment` | `/environments` | 管理当前会话的远程环境：列出已声明的环境、添加新条目、切换会话绑定，或重连已断开的环境。切换需要会话处于空闲状态。实验功能，需设置 `KIMI_CODE_EXPERIMENTAL_REMOTE_RUNTIME`，详见 [远程环境](../guides/remote-environment.md) | 否 |
