...
| `/environment` | `/environments` | 管理当前会话的远程环境：列出已声明的环境、添加新条目、切换会话绑定，或重连已断开的环境。切换需要会话处于空闲状态。详见 [远程环境](../guides/remote-environment.md) | 否 |
...
| 命令 | 别名 | 说明 | 随时可用 |
| --- | --- | --- | --- |
| `/new` | `/clear` | 在当前环境开启全新会话，丢弃当前对话 | 否 |
