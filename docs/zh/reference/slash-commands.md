...
| `/runtime` | — | 管理当前会话的远程运行时：列出已声明的运行时、添加新条目、切换会话绑定，或重连已断开的运行时。切换需要会话处于空闲状态。实验功能，需设置 `KIMI_CODE_EXPERIMENTAL_REMOTE_RUNTIME`，详见 [远程运行时](../guides/remote-runtime.md) | 否 |
