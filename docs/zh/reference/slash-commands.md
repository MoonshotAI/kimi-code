...
| `/connect [--refresh] [--url=<catalog-url>]` | — | 从模型 catalog 配置一个供应商和模型。默认 catalog 随 CLI 内置；附加 `--refresh` 可从 models.dev 拉取最新数据，附加 `--url` 可指向自定义 catalog 端点。详见 [平台与模型 — `/connect` 与模型 catalog](../configuration/providers.md#connect-与模型-catalog)。 | 否 |
