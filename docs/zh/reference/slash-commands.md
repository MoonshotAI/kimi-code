...
| `/criticize` | `/critique` | 启动一个专门的批评子 Agent，分析主 Agent 工作中的缺陷、幻觉、遗漏边界情况和替代方案。批评结果会注入回上下文，让主 Agent 审阅、辩驳或接受每一条意见。在 Plan 模式下会先获取当前计划内容供分析。如果尚未配置批评模型，会先弹出模型选择对话框。 | 是 |
| `/goal [...]` | — | 开始或管理目标模式 | 见下文 |
