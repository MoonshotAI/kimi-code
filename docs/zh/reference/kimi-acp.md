...
`kimi acp` 会让 Kimi Code 成为 IDE 或 ACP 客户端驱动的 ACP Agent 进程。它不会让 Kimi Code 变成一个用于向其他 ACP Agent 派发提示词的 ACP 客户端。
...
转发过来的 MCP server 会在会话内作为工具暴露给 Kimi Code。即使某个 MCP 工具包装了另一个本地 Agent 或 CLI，它也不会被提升成 `Agent` / `AgentSwarm` 子 Agent。
