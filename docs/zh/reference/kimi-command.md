...
服务运行时，`GET /openapi.json` 会返回 REST OpenAPI 文档，`GET /asyncapi.json` 会返回本地 WebSocket 协议的 AsyncAPI 文档。用 API 驱动会话的完整流程见[在网页中使用：用 API 驱动一个会话](../guides/web.md#用-api-驱动一个会话)，协议细节见[服务 API](./server-api.md)。
