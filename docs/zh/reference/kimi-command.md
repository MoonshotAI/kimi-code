...
`-r` / `--resume` 是 `--session` 的隐藏别名；`--yes` 和 `--auto-approve` 是 `--yolo` 的隐藏别名，在帮助信息中不显示。隐藏标志 `--environment <id>` 让新会话绑定到已配置的远程环境（实验功能，详见 [远程环境](../guides/remote-environment.md#kimi-environment)）。
...
- `--environment` 不能与 `--session`/`--continue` 组合——环境在会话创建时绑定，恢复会话时会自动还原已绑定的环境
