...
前台服务按 `Ctrl-C` 停止，后台守护进程用 `kimi server kill` 停止；同一 home 目录下多实例并存时，可用 `kimi server kill <server-id>` 只停止指定实例（缺省停止存活最久的实例）。查看活动连接使用 `kimi server ps`，按实例分组列出各自连接的客户端；`--port`、`--log-level`、`--foreground` 等选项与 `kimi server run` 一致；`--background` 仅 `kimi web` 支持。
