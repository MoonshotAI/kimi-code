...
- 在中断的工具调用结果未被记录时，恢复已恢复的会话。
- 停止将恢复版本标记写入持久化的 agent 元数据。
- 在迁移配置文件时，不再携带已废弃的 legacy loop、background、plan、yolo 或未知的实验性 flag。
- 修复 Xcode 26.5 MCP server 为 Moonshot 兼容性发出的不匹配的 JSON Schema 类型。

### 优化

- 通过换行、压缩或截断可能超出渲染宽度的行，使 TUI 组件保持在窄终端宽度内。
- 提示 CLI 在非平凡的工具调用前显示一句简短的用户语言状态说明。
- 将同语言规则扩展到模型的推理过程，使思考内容跟随用户语言，同时保留代码和技术术语的原始形式。
- 在回退到媒体扩展名之前，使用头信息检测的类型读取媒体文件。
