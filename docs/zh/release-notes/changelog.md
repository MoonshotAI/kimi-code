...
- web: 聊天支持附加任意类型文件，可直接将文件拖放到窗口任意位置；发送的文件、图片、视频都会以附件标签显示在消息气泡中。模型无法直接读取的文件（文档、SVG 图片、压缩包等）会上传到服务器，以文件路径形式交给模型按需读取。

### 优化

- web: 模型请求失败时展示完整诊断信息：语义化标题、供应商原始错误信息，以及可展开的详情（错误码、HTTP 状态码、请求 ID），支持一键复制，不再只是简单的 "Connection error" 提示。
- 应用 Anthropic 官方 effort 配置，未知模型回退到 128k 输出上限；兼容供应商的 thinking 历史在会话恢复与模型切换后保留。
- 上报未处理的 Promise rejection 崩溃遥测，此类异常导致的退出不再无迹可寻。

### 修复

- 修复 Web 服务器 bearer token 校验可被百分号编码的 API 路径（如 `/%61pi/v1/…`）绕过的问题，该漏洞导致所有 API 路由可被未认证访问。
- 修复会话文件系统 API 跟随指向工作区外符号链接的问题，该漏洞可通过预置符号链接读取、列出、创建和下载会话目录之外的宿主机文件。
- web: 会话活动指示器现与 Agent 实际工作保持同步；修复会话激活竞争或 LLM 重试后流式内容重复的问题。
- 修复 Anthropic 兼容供应商中自定义命名模型的问题：新会话思考强度被关闭而非使用模型默认值，且 ACP 客户端不显示思考强度控件。
- Anthropic 兼容模型现在正确遵循 `adaptive_thinking = false`：思考强度限定在旧版预算集合内，请求中不再携带 effort 参数。
- web: 修复服务器绑定非回环地址时 CSP 阻止 Web UI 主题初始化脚本与内置字体加载的问题，并为策略补充显式的 form-action、base-uri、frame-ancestors 指令。
- 修复工作区目录通过符号链接给出时会话创建失败的问题，此前 v2 引擎会误报 "not a directory"。
- 修复剪贴板图片读取失败导致 CLI 意外退出的问题，现在会回退为粘贴文本。
- web: 修复已完成的后台子 Agent 在会话重新加载后丢失最终输出的问题，并在临时拉取失败时重试回填。
- web: 修复开发构建中 Enter 键无法确认模态对话框的问题；确认后对话框保持打开并显示加载状态，直到操作（如归档会话）完成。
- web: 修复流式输出期间后台子 Agent 在 agents dock 面板中显示为两行相同记录的问题。
- 修复 CLI 意外退出时诊断日志缺少实际错误信息的问题。

### 重构

- 将服务器 v1 wire schema 定义移入引擎领域与服务器包，v2 服务器栈不再包含共享 schema 包，无行为变化。
