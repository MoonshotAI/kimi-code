...
- web: 模型请求失败时展示完整诊断信息，支持展开详情与一键复制，不再只是简单的 "Connection error" 提示。
...
- 修复会话文件系统 API 跟随指向工作区外符号链接的问题，该漏洞可通过预置符号链接访问会话目录之外的宿主机文件。
...
- web: 修复开发构建中 Enter 键无法确认模态对话框的问题；确认后对话框保持加载状态，直到操作完成。
