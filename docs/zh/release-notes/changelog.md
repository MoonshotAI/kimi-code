...
- web: 新增会话诊断导出，可将会话与限定范围内的仅含元数据的故障排查日志打包为 ZIP 下载。运行 `/export` 或在会话更多菜单中选择 Export session。Web 下载限制为 64 MiB。
- 前台 Bash 命令超时时不再被终止，而是转入后台继续运行，待完成后回报结果。在 config.toml 的 `[background]` 下设置 `bash_auto_background_on_timeout = false` 可恢复超时终止行为。

### 优化

- web: 优化目标模式控件，新增动画条交互、预算感知进度条，以及符合设计系统的取消确认。
- 关闭会话时请求任务所属工作停止，独立进程遵循 `background.keep_alive_on_exit`，并在尝试强制停止前遵循 `background.kill_grace_period_ms`。
- 重写重复工具调用提醒，引导 Agent 采取其他动作，而非禁止调用。
- 优化 TaskOutput 工具提示词，避免在后台任务上阻塞等待。
- 在向 provider registry（api.json）和模型目录发送请求时带上 kimi-code-cli User-Agent，使 registry 可识别客户端版本。
- skill 解析失败时输出警告，不再静默丢弃，并修复 skill 目录使扫描到的 skill 根目录与策略跳过的 skill 都能被正确报告。

### 修复

- 修复超大图片读取污染会话的问题，并可通过从 provider 请求中移除不安全媒体来恢复已有的请求过大失败。
- 修复会话 fork 仅保留对话日志、丢失其他内容的问题：fork 后的会话现在会携带媒体附件、plan 文件、后台任务输出和 cron 任务，且 fork 失败不再留下损坏的半成品。
- web: 修复重新打开、重连或重新同步会话时的多处渲染异常，包括上下文用量指示器归零、User 消息气泡重复，以及多步轮次在运行中渲染出重复文本。
- web: 修复在非回环服务器连接下，已上传和持久化的图片无法显示的问题。
- web: 修复用户从目标控件恢复被阻塞目标后无法继续运行的问题。
- web: 修复子 Agent 仍在运行时刷新页面后 AgentSwarm 成员列表消失的问题。
- web: 修复会话目标活跃时刷新页面后目标卡片消失的问题。
- web: 修复工作区选择器菜单宽度过窄、无法容纳内容的问题。
- web: 修复瞬时的子 Agent 速率限制被当作会话错误暴露的问题。
- 修复 Windows 上 git 来自原生 MSYS2 工具链（ucrt64/clang64/clangarm64）时 Bash 自动检测失败的问题。
- 修复登录过程中 provider 配置变更后，OAuth 登录在浏览器授权完成后卡住的问题。
- 修复 OAuth 托管模型在 token 刷新后持续返回 401 时，错误显示重新登录提示的问题，现在会展示 provider 的实际拒绝原因。
- 修复未配置 `base_url` 的 provider 被拒绝的问题，anthropic/openai 等协议 provider 现在会再次回退到官方默认 endpoint。
- 修复会话启动后首 turn 无法使用 MCP 工具的问题。
- 修复粘贴的媒体与图片在 `/skill` 和插件命令参数中被丢弃的问题，以及使用 `Ctrl-S` 引导时图片被丢弃的问题。
- 保留跨 provider 的空模型推理块，使多步工具调用能够继续。
- 修复自动权限模式下 plan 退出被标记为用户已审阅的问题，现在会标记为自动批准，Agent 不再将自动计划批准误解为用户开始执行的信号。
- 修复后台任务记录未按 Agent 单独存储的问题，旧版本写入的任务可在恢复时被找到，且一个 Agent 的恢复不会再将另一个 Agent 的任务标记为丢失。
- 修复服务器关闭过程中，正在写入的心跳可能重新创建已被移除的实例文件的问题。
