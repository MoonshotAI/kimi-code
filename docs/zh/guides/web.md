...
`kimi web` 的服务同时暴露 REST 与 WebSocket 接口（实验性，字段级协议见 [服务 API](../reference/server-api.md)）。下面用 curl 走一遍最小流程：确认服务状态 → 创建会话 → 订阅事件 → 提交提示词 → 回读历史。示例假设服务跑在默认地址，token 已存入 shell 变量 `TOKEN`。
