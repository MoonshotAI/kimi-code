...
在较长的提示词中，也可以在空白字符后（包括后续行的行首）输入 `/` 打开仅包含 Skill 的补全菜单。这样可以在一条提示词里引用多个 Skill：Kimi Code 会将它们一起激活，与提示词作为同一轮次运行（一次 `/undo` 即可整体撤销），提示词原文保持不变。提示词中的 Skill 引用不携带参数——只按名称激活；参数仍是单独以 `/skill:<name> args` 调用时的概念。内置命令和 plugin 命令仍需放在输入开头。
...
- **`Esc` / `Ctrl-C`**：中断当前轮次
- **`Ctrl-O`**：全局切换工具输出、被对话记录摘要收起的 Assistant 消息和压缩摘要
