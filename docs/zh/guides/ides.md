...
Kimi Code 支持两类编辑器工作流：通过 Kimi Code for VS Code 使用原生 VS Code 插件，或通过 [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) 让 IDE 启动 Kimi Code CLI。两种方式都会复用同一份本地登录状态，所以只要登录一次，就可以留在编辑器里继续编码。
...
Kimi Code for VS Code 提供原生侧边栏视图、编辑器命令和基于 webview 的聊天界面，底层由本地 Kimi Code CLI 驱动。对于 VS Code 用户，这是最直接的接入方式：插件会在 VS Code 内管理 ACP 子进程、会话加载、文件引用、审批请求和文件变更查看。

安装插件后，从 Activity Bar 打开 **Kimi Code** 视图。插件会检查 `kimi` 可执行文件是否可用，并复用终端 CLI 的同一份登录状态。如果 VS Code 找不到 `kimi`，请把 **Kimi Code: Executable Path** 设置为下面命令返回的绝对路径：
...
- **Kimi Code: Focus Input**：从 VS Code 任意位置聚焦聊天输入框。
- **Kimi Code: Insert Current File**：把当前编辑器文件引用到当前提示词里。
- **Kimi Code: New Conversation**：开启一个新的 Kimi Code 会话。
- **Kimi Code: Show Logs**：打开插件日志，用于排查 CLI 启动和 ACP 消息问题。

Kimi Code for VS Code 从 CLI 侧读取 MCP 服务配置。需要新增或编辑 MCP 服务时，请在 CLI 中使用 `/mcp-config`，如果 VS Code 里的服务列表看起来不是最新的，再重新打开或刷新 VS Code 视图。
