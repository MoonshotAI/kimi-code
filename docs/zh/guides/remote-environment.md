...
远程环境让 Agent 的工具——读写文件、执行 Shell 命令——在另一台机器或容器里执行，而 Kimi Code CLI 本身、所有模型请求和你的凭据都留在本机。适合代码在远程服务器上，或希望把工具执行隔离在 Docker 兼容容器里的场景。

## 远程环境的工作原理

Kimi Code 把 Agent 循环、模型请求、凭据、审批和会话状态全部留在本机，目标环境只执行两组 OS 原语：文件系统、进程。目标机器上运行一个小型执行器（`kimi exec-server`），通过一条连接提供这些原语；其余一切——包括每一次 LLM 请求——都留在本机。
...
底部状态栏会在工作目录前显示当前环境标识（例如 `ssh:dev-box`）；`local` 不渲染。`disconnected` 的环境以错误色显示并附带失败原因；`pending` 的环境——尚未连接过——以暗色显示，不带错误色调。远程会话中本机 git 状态槽会隐藏。审批面板会在命令或路径旁显示目标环境标识，`@` 文件补全改由服务端提供，候选来自目标文件系统。
...
- **用 `connect` 创建临时环境**：传入启动器规格——`{ type: "ssh", host: "..." }`、`{ type: "docker", container: "..." }` 或 `{ type: "command", command: "...", args: [...] }`，可选 `id`。环境会立即连接并像声明的环境一样注册到工作区，但不会写入 `config.toml`：临时环境在进程退出时消失，恢复会话时也找不到它。同一进程内连接断开后可以重试。
...
每个环境 id 在整个进程里只有一条连接。绑到这个 id 的工作区共用它；另一个 id，哪怕在同一台机器上，仍是各自的连接。连接是否已死由 SSH 判断：启动参数带了 `ServerAliveInterval`，本地管道关闭就是断线信号。没有协议层心跳。连接断开时——网络中断、容器停止、执行器退出——使用这个 id 的所有工作区同时进入 `disconnected`，它在目标上启动的进程随连接结束。临时 `connect` 是自己的连接，不参与共享。

连接断开后**不会静默回退到本地环境**：本该落在远程机器上的 `rm` 或 `git` 命令绝不能落到你的本机。下一个工具调用会重新打开这条管道——目标持续不可达时，工具调用会以 `environment.unavailable` 错误失败；你也可以在 `/environment` 对话框中显式重连。

显式重连是这个 id 的进程范围行为：无论从哪个工作区发起，都会先断开再建立新连接。别处正在进行的调用会失败，不会在新连接上自动重试。其他环境 id 不受影响。

恢复旧会话也一样：加载时会按还原出的绑定在后台尝试连接，但不会等连接结束才打开会话。目标不可达或握手还在进行时，会话仍然立刻打开，绑定保留，环境保持 `connecting` 或 `disconnected`。首个工具调用会等待这次连接或再次重试，同样绝不会静默回退到 `local`。
...
执行器是 Kimi Code 自身的轻量构建，在目标环境以 `kimi exec-server` 启动。它只响应文件系统、进程请求——不接触模型 API、凭据和会话状态。stdio 是默认且唯一支持的传输方式，显式写法 `kimi exec-server --listen stdio` 与之等价且依然可用。
...
- **目录列表上限**：列举超过 50,000 个条目的远端目录时会明确报错，不会返回部分列表。请改用较小的子目录，或拆分目录。Skill 发现会在日志中记录无法扫描的目录。
