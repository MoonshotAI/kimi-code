...
| 类别 | 覆盖范围 |
|---|---|
| 股票与金融市场 | Wind、S&P Capital IQ、SEC EDGAR 等知名数据库，能力涵盖 A 股、港股、美股等主要市场的行情、技术指标、财报估值、分析师预期，以及 8,000+ 美股上市公司的官方披露文件（2009 年至今） |
...
- **桌面应用操作**：驱动 macOS 上的各类 App，例如让 AI 打开网易云音乐播放指定歌曲
- **搞定没有接口的软件**：操作没有 CLI 或 API的桌面端应用，例如让它把剪映里这段视频的片头剪掉三秒再导出
- **后台任务执行**：AI 在后台静默完成点击、输入等操作，你可以继续正常使用电脑
