...
Kimi Code CLI 会保留因格式或大小限制而无法直接交付的内嵌 MCP 附件。内嵌音视频无论大小也会保存，因为后续供应商协议转换可能省略它们。原件随会话保存在媒体存储中，不会被图片缓存淘汰。保存的原件（包括图片压缩前的原图）均提供绝对路径和会话相对路径，便于 Agent 在 fork 后的会话中找到副本。普通 SVG 保存为 `.svg`，可作为文本读取；其他二进制格式需要合适的本地阅读器或转换工具。

附件路径和压缩说明共用工具输出预算。较长的清单会保存为文本文件，结果中保留简短指针，即使伴随的文本被截短，该指针仍然可见；Agent 可用 `Read` 分页读取完整清单。如果解码或保存失败，结果会明确说明原件未能保留，并保留其他可用输出。资源链接不会被自动下载。
