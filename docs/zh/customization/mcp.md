...
内嵌 MCP 附件因格式不受支持或超过附件大小限制而无法直接交付时，Kimi Code CLI 会将原始字节保存到会话媒体存储，并提供文件路径。即使伴随的文本被截短，该路径仍然可见；原件随会话保留，不会被图片缓存淘汰。结果还会提供会话相对路径，便于在 fork 后的会话中找到副本。Agent 可使用合适的本地阅读器或转换工具处理原件；`Read` 仅接受文本文件。如果解码或保存失败，结果会明确说明附件交付不完整，并保留其他可用输出。资源链接不会被自动下载。
