...
Kimi Code CLI 会保留因格式或大小限制而无法直接交付的内嵌 MCP 附件。内嵌音视频无论大小也会保存，因为后续供应商协议转换可能省略它们。原件随会话保存在媒体存储中，不会被图片缓存淘汰。保存的原件（包括图片压缩前的原图）均提供绝对路径和稳定的 `kimi-file://` 引用。将引用作为 `path` 传给 `Read` 或 `ReadMediaFile`，即可解析到当前会话中的副本，包括 fork 后的会话。对于 `Read` 无法打开的二进制格式，错误信息会提供解析后的本地路径，供转换工具使用。CSV、HTML、JSON 和普通 SVG 等文本附件使用可读取的扩展名。
