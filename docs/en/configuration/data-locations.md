...
- **`credentials/`**: OAuth credential directory, with permissions `0o700` (directory) / `0o600` (files), readable and writable only by the current user. In file-storage mode, managed provider credentials are stored as `credentials/<name>.json` and MCP server credentials under `credentials/mcp/`; credentials are written using an atomic flow (tmp → fsync → rename) to prevent corruption. When the experimental keychain storage is enabled (`KIMI_CODE_EXPERIMENTAL_KEYRING=1`, see [Environment variables](./env-vars.md#runtime-switches)), OAuth credentials live in the operating system keychain instead, and existing plaintext files are migrated into the keychain on first read. The keychain is separate from this directory, so deleting these files does not remove keychain credentials; disabling keychain access does not copy keychain-only credentials back to files.
