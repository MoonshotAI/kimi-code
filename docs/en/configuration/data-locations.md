...
- **`credentials/`**: OAuth credential directory, with permissions `0o700` (directory) / `0o600` (files), readable and writable only by the current user. In file-storage mode, managed provider credentials are stored as `credentials/<name>.json` and MCP server credentials under `credentials/mcp/`; credentials are written using an atomic flow (tmp → fsync → rename) to prevent corruption. When the operating system keychain is available (macOS Keychain, Windows Credential Manager, or Linux Secret Service), OAuth credentials live in the keychain instead, and existing plaintext files are migrated into the keychain on first read. The keychain is separate from this directory, so deleting these files does not remove keychain credentials; if the keychain is unavailable, existing file copies remain usable, while keychain-only credentials stay unavailable until the keychain recovers or you sign in again.
