...
| `KIMI_CODE_EXPERIMENTAL_TERMINAL_MOUSE_INPUT` | Enable click positioning and left-button drag selection in the TUI prompt editor. While enabled, hold `Shift` for the terminal's native text selection or scrollback behavior | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
