...
| `KIMI_CODE_EXPERIMENTAL_AGENT_ENVIRONMENT_TOOLS` | Agent environment tools, on by default: the main agent gains `change_environment` (switch the session environment, taking effect on the next tool call) and `connect` (create a temporary environment from a launcher spec), plus an `environment` parameter on `Agent` — see [Remote environments](../guides/remote-environment.md#agent-environment-tools); set a falsy value to opt out | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off`; unset = enabled |
