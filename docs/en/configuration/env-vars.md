...
| `KIMI_CODE_EXPERIMENTAL_DYNAMIC_WORKFLOWS` | Enable experimental dynamic workflows — user-approved JavaScript scripts that orchestrate subagents in phases; see [Dynamic Workflows](../customization/workflows.md) | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
