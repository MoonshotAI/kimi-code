...
| `KIMI_CODE_EXPERIMENTAL_KEYRING` | Store OAuth credentials (provider logins and MCP server grants) in the operating system keychain — macOS Keychain, Windows Credential Manager, or Linux Secret Service — instead of plaintext files under `credentials/`; existing plaintext credentials migrate into the keychain on first read, and the file store takes over again whenever the keychain is unavailable. The master `KIMI_CODE_EXPERIMENTAL_FLAG=1` does not enable it | `1` opts in; anything else keeps the plaintext file store |
| `KIMI_DISABLE_KEYRING` | Force OAuth credentials back to the plaintext file store even when `KIMI_CODE_EXPERIMENTAL_KEYRING=1` is set | `1` disables the keychain; anything else leaves the decision to `KIMI_CODE_EXPERIMENTAL_KEYRING` |
