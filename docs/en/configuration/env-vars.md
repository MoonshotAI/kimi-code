...
| `KIMI_CODE_EXPERIMENTAL_REMOTE_RUNTIME` | Experimental remote runtimes: bind sessions to SSH hosts, Docker containers, or custom launchers declared in the `[runtimes]` config section so the agent's tools execute in the target environment — see [Remote runtimes](../guides/remote-runtime.md); `KIMI_CODE_EXPERIMENTAL_FLAG=1` also enables it | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
