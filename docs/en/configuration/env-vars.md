...
| `KIMI_CODE_EXPERIMENTAL_TUI_TABS` | Experimental session tabs in the TUI: `/new tab` (Alt+T) opens a session in a new tab while the current one keeps running, Ctrl+T in `/sessions` resumes one in a new tab, `/tab <n>` / Alt+1..9 / Ctrl+Tab switch tabs, `/tab close` (Alt+W) closes one | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
