...
| `KIMI_TOKEN_COUNTING_STRATEGY` | Source of context token counts; takes higher priority than `[token_counting] strategy` in `config.toml` (default `measured+estimated`) | `measured+estimated`, `measured`, `estimated` (case-insensitive); invalid values are ignored |
