...
| `KIMI_DISABLE_KEYRING` | Force OAuth credentials (provider logins and MCP server grants) back to the plaintext file store even when the OS keychain is available; takes higher priority than `credentials_store` in `config.toml` — useful on hosts where keychain access hangs or prompts (containers, headless SSH) | `1` disables the keychain; anything else leaves the decision to `credentials_store` |
