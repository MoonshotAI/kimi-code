...
| `KIMI_CODE_EXPERIMENTAL_AGENT_ENVIRONMENT_TOOLS` | Experimental agent environment tools: the main agent gains `change_environment` (switch the session environment at the turn boundary) and `connect` (create a temporary environment from a launcher spec), plus an `environment` parameter on `Agent` — see [Remote environments](../guides/remote-environment.md#agent-environment-tools-experimental); `KIMI_CODE_EXPERIMENTAL_FLAG=1` also enables it | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
