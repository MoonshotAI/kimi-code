...
| `KIMI_CODE_REPEAT_BREAKER` | Whether repeating the same tool call many times in a row injects reminders and eventually force-stops the turn; higher priority than `repeat_breaker` | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
| `KIMI_CODE_TOOL_DEDUPE` | Whether identical tool calls within the same step are collapsed into a single execution that shares its result; higher priority than `tool_dedupe` | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
