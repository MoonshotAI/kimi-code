...
The three `KIMI_CODE_IDENTITY_*` / `KIMI_CODE_BUILTIN_PRODUCT_SKILLS` variables are read by the `agent-core-v2` engine, which currently backs `kimi web` and the `KIMI_CODE_EXPERIMENTAL_FLAG` paths; the default `kimi` / `kimi -p` engine ignores them. Conversely, `KIMI_CODE_EXPERIMENTAL_SECONDARY_MODEL`, `KIMI_SECONDARY_MODEL`, and `KIMI_SECONDARY_EFFORT` are read by the default engine only, and the v2 engine ignores them.
