...
| `KIMI_CODE_EXPERIMENTAL_WEB_MULTI_SESSION` | Experimental multi-session support in the `kimi web` server: subscribing to a session over WebSocket resumes it on demand, `POST /api/v1/sessions/resume` and `GET /api/v1/sessions/live` become available, and idle sessions are unloaded according to the [`server`](./config-files.md#server) limits | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
...
| `KIMI_CODE_SERVER_MAX_LIVE_SESSIONS` | Upper bound on sessions the `kimi web` server keeps loaded at once; higher priority than `[server] max_live_sessions` (default `16`, `0` = no limit) | Non-negative integer; invalid values are ignored |
| `KIMI_CODE_SERVER_SESSION_IDLE_TIMEOUT_MS` | Idle time after which the `kimi web` server unloads a session nobody is subscribed to; higher priority than `[server] session_idle_timeout_ms` (default `1800000`, `0` = never) | Non-negative integer; invalid values are ignored |
