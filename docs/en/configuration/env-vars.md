...
| `KIMI_CODE_EXPERIMENTAL_KEYRING` | Store OAuth credentials (provider logins and MCP server grants) in the operating system keychain — macOS Keychain, Windows Credential Manager, or Linux Secret Service — instead of plaintext files under `credentials/`; existing plaintext credentials migrate into the keychain on first read. If the keychain becomes unavailable, existing file copies remain usable, while keychain-only credentials stay unavailable until the keychain recovers or you sign in again. The master `KIMI_CODE_EXPERIMENTAL_FLAG=1` does not enable it | `1` opts in; anything else keeps the plaintext file store |
| `KIMI_DISABLE_KEYRING` | Disable keychain access even when `KIMI_CODE_EXPERIMENTAL_KEYRING=1` is set; this does not copy keychain-only credentials back to plaintext files | `1` disables the keychain; anything else leaves the decision to `KIMI_CODE_EXPERIMENTAL_KEYRING` |
