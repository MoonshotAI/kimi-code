...
| `KIMI_CODE_LEGACY_FLAG` | Use the legacy `agent-core` engine for `kimi`, `kimi -p`, `kimi doctor`, and `kimi acp`; these commands use `agent-core-v2` by default | `1`, `true`, `yes`, `on` |
