...
| `KIMI_CODE_EXPERIMENTAL_REMOTE_RUNTIME` | Experimental remote environments: bind sessions to SSH hosts, Docker containers, or custom launchers declared in the `[environments]` config section so the agent's tools execute in the target environment — see [Remote environments](../guides/remote-environment.md); `KIMI_CODE_EXPERIMENTAL_FLAG=1` also enables it | Truthy: `1`/`true`/`yes`/`on`; falsy: `0`/`false`/`no`/`off` |
