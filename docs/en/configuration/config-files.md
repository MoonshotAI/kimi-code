...
| `max_output_size` | `integer` | No | Per-request output token cap (maps to `max_tokens` / `max_output_tokens`). Honored as an explicit cap by every provider: for `anthropic` it overrides the model's built-in server-side maximum; for OpenAI-compatible providers an explicit value bypasses the generic 128k fallback ceiling but still respects the remaining context window, whereas an unset value falls back to an inferred budget capped at 128k |
