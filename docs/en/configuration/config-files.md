...
| `default_effort` | `string` | No | Default thinking effort for the model. A pick above this default in `/model` or `/effort` applies to the current session only instead of being saved as the global default. Managed and open-platform refreshes may rewrite this field; to pin it manually, set `[models."<alias>".overrides] default_effort` instead |
