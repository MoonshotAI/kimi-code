...
| `--swarm` | | Start a new session with swarm mode enabled — the Agent can dispatch multiple sub-agents in parallel |
...
Start with swarm mode enabled so the Agent can dispatch parallel sub-agents:
