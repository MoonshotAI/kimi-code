...
Both web tools always run from the machine running Kimi Code, even when the session is bound to a [remote environment](../guides/remote-environment.md) — requests never egress from the target environment.
...
| Tool | Default Approval | Description |
| --- | --- | --- |
| `change_environment` | Requires approval (only in Always Ask mode) | Switch the session to another environment |
| `connect` | Requires approval (only in Always Ask mode) | Create and connect a temporary environment |
...
The switch itself takes effect as soon as the tool call completes — the next tool call in the same turn already runs on the new environment. When other tool calls are still executing in parallel, the call fails with an error naming the in-flight count; retry once they have finished, and their work is never yanked mid-flight. A reminder with the new environment's OS, shell, and working directory arrives with the next turn. The switch is recorded like a user-driven `/environment` switch, so undo restores the previous binding.
