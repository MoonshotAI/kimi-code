...
`Agent` and `AgentSwarm` manage Kimi Code sub-agents only. If an MCP server exposes another local agent or CLI, Kimi Code can call that server as a tool when the active profile allows it, but the call is not scheduled or resumed as a Kimi sub-agent.
