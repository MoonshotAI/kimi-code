...
| `Skill` | Auto-allow | Invoke a registered Skill of type `prompt` or `inline` |
...
**`Skill`** allows the Agent to actively invoke a registered model-invocable Skill. Accepts `skill` (the Skill name) and optional `args` (additional argument text). Skills with no explicit `type` (the default is `prompt`), `type = "prompt"`, or `type = "inline"` can be called through this tool. Skills with `type = "flow"` can only be triggered manually by the user, and Skills with `disableModelInvocation: true` are also rejected. Maximum nesting depth is 3 levels. See [Agent Skills](../customization/skills.md) for details.
