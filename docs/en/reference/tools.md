...
**`Read`** accepts a file path (`path`) plus optional `line_offset` (starting line number; negative values count from the end), `n_lines` (requested number of lines), and `max_chars` (maximum characters in the result, including line numbers and status). Omitting `n_lines` reads toward the end of the file. The default is 100,000 characters, and calls can request up to 500,000; both values can be changed in the [`read` configuration](../configuration/config-files.md#read). Characters use JavaScript string length: common letters and Chinese characters count as one, while many emoji count as two.

Results contain complete lines and are not shortened again by the general tool-output limit. When a range does not fit, the status includes `Next Read` arguments that continue through the omitted range without repeating the returned lines. Tail reads return the newest part of the requested range first. A single line that cannot fit produces an error instead of silently losing its ending; request a larger `max_chars` or use `Bash` to extract a smaller character range. Use `ReadMediaFile` for images or videos.
