...
The footer shows the active environment's identity (for example `ssh:dev-box`) before the working directory; `local` renders nothing. A disconnected environment is shown in the error color with the failure reason, while a pending one — never connected yet — renders dim with no error tone, and the local git status slot is hidden for remote sessions. Approval prompts display the target environment next to the command or path, and `@` file completion is served by the server so candidates come from the target's filesystem.
