// packages/app-core/src/lib/mergeWorkspaces.ts
 ...
 * derived workspace (id = root = cwd). Real workspaces win on root. "Same
 * root?" is always decided by the folded identity key (Windows casing/slash
 * variants), never by exact string; display strings keep their original
 * spelling.
 */
export function mergeWorkspaces(input: MergeWorkspacesInput): AppWorkspace[] {
  ...
  const byRoot = new Map<string, AppWorkspace>(); // folded root key -> workspace
  ...
  // the registered workspace whose root identity-matches the session cwd;
  // otherwise the daemon-provided workspaceId; otherwise the cwd itself
  // (derived/fallback mode). Resolved through a folded root-key→id map built
  // ONCE here (FIRST entry per key wins, matching `byRoot` above and the
  // sidebar's first-match assignment) — the per-session linear
  // `workspaces.find` this replaces made the merge O(sessions × workspaces).
  const idByRootKey = new Map<string, string>();
  for (const w of workspaces) {
    const key = workspaceRootKey(w.root);
    if (!idByRootKey.has(key)) idByRootKey.set(key, w.id);
  }
  const counts = new Map<string, number>();
  for (const s of sessions) {
    const wid = idByRootKey.get(workspaceRootKey(s.cwd)) ?? s.workspaceId ?? s.cwd;
  ...
  // activity). Both lists hold folded root KEYS. Hidden roots must be excluded
  // here too — `byRoot` skips them, so a hidden real workspace would otherwise
