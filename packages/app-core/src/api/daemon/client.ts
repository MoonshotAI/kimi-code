...
export class DaemonKimiWebApi implements KimiWebApi {
  ...
  async submitPrompt(
    ...
    const startedAt = Date.now();
    this.tracer.traceKeyEvent?.('prompt:start', {
      sessionId,
    ...
    try {
      ...
      this.tracer.traceKeyEvent?.('prompt:failed', {
        sessionId,
        status: 'failed',
        durationMs: Date.now() - startedAt,
        ...errorTraceMetadata(error),
  ...
  async getUsage(): Promise<ManagedUsageResult> {
    const data = await this.http.get<WireUsageResult>('/oauth/usage');
    if (data.kind === 'error') {
