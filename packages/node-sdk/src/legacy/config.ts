/**
 * Local port of agent-core v1's config runtime, copied verbatim so node-sdk
 * keeps the same config reading/writing behavior without the retired
 * `@moonshot-ai/agent-core` package:
 * - `src/config/path.ts` — resolveKimiHome / resolveConfigPath / ensureKimiHome
 * - `src/config/resolve.ts` — env-value resolution helpers
 * - `src/config/model.ts` — effectiveModelAlias
 * - `src/config/env-model.ts` — KIMI_MODEL_* env synthesis
 * - `src/config/toml.ts` — read / parse / write / stringify
 *
 * Import rewrites: `#/errors` → `./errors`, `#/config/schema` →
 * `./config-schema`, `#/utils/fs` → `./fs-utils`, `pathe` → `node:path`.
 */
