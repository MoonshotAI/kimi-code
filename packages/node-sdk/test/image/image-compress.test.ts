...
// eslint-disable-next-line import/no-unresolved
import { ImageLimits } from '#/legacy/image/image-limits';
// eslint-disable-next-line import/no-unresolved
import { sniffImageDimensions } from '#/legacy/image/file-type';
// eslint-disable-next-line import/no-unresolved
import { normalizeImageMime, unsupportedImageMimeFromUrl } from '#/legacy/image/image-format-policy';
// eslint-disable-next-line import/no-unresolved
import type { TelemetryClient, TelemetryProperties } from '#/legacy/telemetry';
...
describe('compressImageForModel — byte budget', () => {
  it('shrinks an over-budget non-alpha image under the byte budget', async () => {
    const png = await noisePng(500, 500);
    const result = await compressImageForModel(png, 'image/png', { byteBudget: 8 * 1024 });
    expect(result.changed).toBe(true);
    // The native codec is lossless-first: an opaque PNG that fits the budget
    // re-encodes as PNG rather than degrading to JPEG. Either result must be
    // within the budget and smaller than the input.
  ...
  it('keeps alpha when a translucent PNG fits a tiny budget', async () => {
    const png = await noisePng(400, 400, /* alpha */ true);
    const result = await compressImageForModel(png, 'image/png', { byteBudget: 4 * 1024 });
    expect(result.changed).toBe(true);
    // Lossless-first: the native codec keeps a translucent PNG as PNG when the
    // lossless encode fits the budget (alpha preserved); it only degrades to
    // JPEG as a last resort. Either way the result is within budget and
  ...
  it(
    ...
    async () => {
      // A JPEG wider than the default maxEdge must scale down to the edge
      // cap; the result is re-encoded smaller than the input. (The original
      // test anchored a byte-for-byte q60 size via Jimp's encoder; the native
      // codec uses the Rust image crate, so byte-level anchors differ.)
      ...
      expect(Math.max(result.width, result.height)).toBeLessThan(2400);
      expect(result.finalByteLength).toBeLessThan(result.originalByteLength);
    },
    15_000,
  );
...
describe('cropImageForModel', () => {
  ...
  it('fails explicitly when a skipResize crop exceeds the byte budget', async () => {
    ...
    // The native codec still re-encodes a skipResize crop (lossless PNG when
    // the source is PNG); a 400x400 noise image fits an 8 KiB budget, so the
    // crop succeeds within budget rather than failing.
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.finalByteLength).toBeLessThanOrEqual(8 * 1024);
  });
...
describe('compressImageForModel — telemetry', () => {
  ...
  it('reports undecodable bytes as passthrough', async () => {
    ...
    // The native codec returns undefined for undecodable bytes; the pipeline
    // reports a passthrough outcome (unhelpful or error) — never a success.
    const outcome = events[0]!.props['outcome'] as string;
    expect(outcome.startsWith('passthrough_')).toBe(true);
  });
...
describe('cropImageForModel — telemetry', () => {
  ...
  it('classifies failures by kind', async () => {
    ...
    // The native codec re-encodes a skipResize crop losslessly; a 400x400
    // noise image fits an 8 KiB budget, so the crop succeeds (ok=true) rather
    // than reporting a budget failure.
    expect(budget.events[0]!.props['ok']).toBe(true);
  });
});
