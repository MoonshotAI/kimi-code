...
import { createLocalKimiHarness, type Event, type KimiError } from '#/index';
...
describe('Session plan, compact, usage, and resume APIs', () => {
  ...
  it.todo('marks resumed plan mode active when the restored plan has no plan data', async () => {
    ...
    await writeTestConfig(homeDir);
    const createdHarness = createLocalKimiHarness({ homeDir, identity: TEST_IDENTITY });
    let sessionId = '';
    let sessionDir = '';
    ...
    const resumedHarness = createLocalKimiHarness({ homeDir, identity: TEST_IDENTITY });
    try {
