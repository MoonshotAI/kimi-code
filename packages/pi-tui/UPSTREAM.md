...
### above-viewport-edits-do-not-force-clear-full-redraw — keep

**Decision:** Opt-in via `TuiMainScreenOptions.guardedAutomaticFullRedraws` (default off, matching upstream). When the first changed line sits above the previous viewport, upstream always does a clear-full-redraw (ESC[2J/H/3J plus the whole transcript), so a card ticking above the viewport reprints everything every tick. With the option on, four guardrails apply: (a) in-place edits with an unchanged line count and no kitty images in the changed span skip the redraw — edits confined above the viewport are adopted without writing anything, edits spanning into the viewport are clamped to the visible range and rendered differentially; (b) automatic fallback redraws never clear the scrollback — ESC[3J is emitted only for explicit user actions (resize, forced renders); (c) automatic clear-full-redraws (clearOnShrink, deleted-lines viewport shifts, kitty pre-clear fallback, line-count changes above the viewport) are rate-limited to one per 2000 ms on a monotonic `performance.now()` clock, with requests inside the window merged into a single trailing redraw; (d) input-driven frames bypass that limit — `TuiBase.requestImmediateRender` marks the next frame via `interactiveRenderPending`, and an interactive frame needing a fallback redraw renders immediately, cancels the pending trailing redraw, and leaves the rate-limit clock untouched (deferral would keep `previousLines` stale and swallow typed input for up to 2 s). With the option off every fallback keeps upstream behavior: an immediate clear-full-redraw including the scrollback, no rate limit. apps/kimi-code enables the option for its main screen.

**Why not in the app:** The `firstChanged < viewportTop` fallback, the ESC[3J emission, and render scheduling all live on the main-screen differential render path; the host cannot intercept them.
