...
- Set background=true when you can keep working without the answer. This starts a background question and returns a task_id immediately. The answer arrives automatically in a later message — do not poll, sleep, or guess. Continue with other work; use WaitFor only if you later cannot proceed without the answer.
