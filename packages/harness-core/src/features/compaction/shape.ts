...
export function buildCompactionSeed(input: {
  ...
  const events: RecordEvent[] = [
    { type: 'turn.started', turnId: input.turnId },
    ...seeded.map((message) => ({ type: 'message.appended', message })),
    { type: 'turn.ended', turnId: input.turnId, outcome: 'done' },
    ...input.queue.map((entry) => ({ type: 'input.submitted', entry })),
  ];
  return {
    events,
    tokensAfter: usedContextTokens(seeded),
