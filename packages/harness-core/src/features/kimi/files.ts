...
export class KimiFiles {
  ...
  async uploadVideo(input: VideoUploadInput, options?: KimiUploadOptions): Promise<MediaFileRef> {
    if (!input.mimeType.startsWith('video/')) {
      throw new Error(`Expected a video mime type, got ${input.mimeType}`);
    }
    const uploaded = await this.upload(input, 'video', options);
    return { url: `ms://${uploaded.id}`, id: uploaded.id };
  }

  async uploadImage(input: ImageUploadInput, options?: KimiUploadOptions): Promise<MediaFileRef> {
    if (!input.mimeType.startsWith('image/')) {
      throw new Error(`Expected an image mime type, got ${input.mimeType}`);
    }
    const uploaded = await this.upload(input, 'image', options);
    return { url: `ms://${uploaded.id}`, id: uploaded.id };
  }
