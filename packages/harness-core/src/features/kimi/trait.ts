  ...
  convertMessage: (message, converted) => {
    ...
    if (message.role === 'assistant' && Array.isArray(convertedToolCalls)) {
      message.toolCalls.forEach((toolCall, index) => {
        ...
        const out: (OpenAINativeToolCall & { extras?: unknown }) | undefined =
