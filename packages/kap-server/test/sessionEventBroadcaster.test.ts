...
describe('SessionEventBroadcaster', () => {
  ...
  it('rebuilds subscriptions and the journal writer after a session is released', async () => {
    ...
    // `turn.started` lands first and is trailed by a durable
    // `work_changed(busy:true)`, so the rebuilt journal's turn.started is the
    // second-to-last durable.
    expect(durable.at(-2)).toMatchObject({
      type: 'turn.started',
      seq: firstSeq + 1,
