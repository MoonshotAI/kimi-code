...
import { SPINE_FLAG_ENV } from '#/agent/spine/flag';
...
describe('AgentTodoService', () => {
  ...
  it('silences the stale-todo reminder while spine is enabled', async () => {
    const todo = ctx.get(IAgentTodoService);
    const reminder = ctx.get(IAgentReminderService);

    await todo.replace([{ title: 'still open', status: 'in_progress' }]);
    appendAssistantTurns(ctx.context, 10);
    await reminder.reconcileWhenIdle(TODO_LIST_REMINDER_VARIANT);
    expect(reminderInjected(ctx)).toBe(true);

    vi.stubEnv(SPINE_FLAG_ENV, '1');
    const spineCtx = createTestAgent();
    try {
      await spineCtx.restorePersisted();
      const spineTodo = spineCtx.get(IAgentTodoService);
      const spineReminder = spineCtx.get(IAgentReminderService);
      await spineTodo.replace([{ title: 'still open', status: 'in_progress' }]);
      appendAssistantTurns(spineCtx.context, 10);
      await spineReminder.reconcileWhenIdle(TODO_LIST_REMINDER_VARIANT);
      expect(reminderInjected(spineCtx)).toBe(false);
    } finally {
      await spineCtx.dispose();
    }
  });
});
