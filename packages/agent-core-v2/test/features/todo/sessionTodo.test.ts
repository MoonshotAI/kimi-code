...
function makeRuntimeAgent(
  ...
  ix.set(IFlagService, makeFlagsStub());
...
describe('TodoAgentRuntime', () => {
  ...
  it('silences the stale-todo reminder while spine is enabled', async () => {
    ...
    const registry = new RuntimeRegistry();
    registry.register(todoRecord());
    const agent = makeRuntimeAgent(registry, 'main', 1, { toolActive: true, history });
    await agent.todo.replace([{ title: 'still open', status: 'in_progress' }]);
    const reminder = agent.reminderProviders.get(TODO_LIST_REMINDER_VARIANT);
