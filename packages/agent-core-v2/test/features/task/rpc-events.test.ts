...
function createAgentTaskService(options: {
  ...
  if (options.wireRecords !== undefined) {
    overrides.push(
      wireRecordPersistenceServices(
        new InMemoryWireRecordPersistence(withMetadata(options.wireRecords)),
      ),
    );
  }
  ...
  return {
    ctx,
    agent,
    manager: ctx.resolve(AgentTask) as TaskServiceTestManager,
    records,
    persistence,
  };
}
...
describe('AgentTaskService — notification delivery', () => {
  ...
  it('TaskStopTool persists stop reason and suppression across reload', async () => {
    ...
    try {
      ...
      readerFixture = createAgentTaskService({
        sessionDir,
        wireRecords: await writerFixture.ctx.persistedWireRecords(),
      });
      const { agent, manager: reader } = readerFixture;
      await readerFixture.ctx.restorePersisted();
  ...
  it('replays restored terminal agent task notifications when undelivered', async () => {
    ...
    try {
      const persistence = createAgentTaskPersistence(sessionDir);
      await writeLegacyTaskFile(sessionDir, persistedAgent());
  ...
  it('references persisted output without reading a tail for restored process notifications', async () => {
    ...
    try {
      ...
      const persistence = createAgentTaskPersistence(sessionDir);
      await writeLegacyTaskFile(sessionDir, persistedProcess({ taskId }));
  ...
  it('does not replay restored notifications already marked delivered', async () => {
    ...
    try {
      ...
      const persistence = createAgentTaskPersistence(sessionDir);
      await writeLegacyTaskFile(sessionDir, persistedAgent({ taskId: 'agent-seen0000' }));
