...
import { TERMINAL_STATUSES } from '#/features/task/types';
...
function createTaskService(homedir: string, wireRecords?: readonly WireRecord[]): TaskServiceFixture {
  const persistence = createAgentTaskPersistence(homedir);
  const ctx = createTestAgent(
    homeDirServices(homedir),
    taskServices(),
    ...(wireRecords === undefined
      ? []
      : [wireRecordPersistenceServices(new InMemoryWireRecordPersistence(withMetadata(wireRecords)))]),
  );
  const manager = ctx.resolve(AgentTask) as TaskServiceTestManager;
  return {
    ctx,
    manager,
    persistence,
...
describe('AgentTaskService — readOutput / getOutputSnapshot', () => {
  ...
  it('readOutput falls back to disk for ghost tasks', async () => {
    ...
    const freshFixture = createTaskService(sessionDir, await ctx.persistedWireRecords());
    const fresh = freshFixture.manager;
    try {
      await freshFixture.ctx.restorePersisted();
