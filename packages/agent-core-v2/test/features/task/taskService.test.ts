...
import { renderNotificationXml } from '#/features/task/internal/notificationXml';
import { TaskLifecycle } from '#/features/task/internal/taskLifecycle';
...
import { type WaitForInput } from '#/features/task/tools/task-wait/task-wait';
import { WaitForTool } from '#/features/task/tools/task-wait/taskWaitTool';
...
function registerAgentEventBus(
  ...
  ix.set(IEventBus, new AgentEventBusView(eventBus, scopeContext));
  eventBus.activateAgent(scopeContext.agentContext);
  return eventBus;
}
...
describe('TaskLifecycle', () => {
  ...
  beforeEach(() => {
    ...
    agentScope = makeAgentScopeContext({
      agentId: 'main',
    ...
    eventBus = registerAgentEventBus(ix, disposables, agentScope);
  ...
  it('releases a reserved task id after a failed registration', async () => {
    stubTaskConfig({ maxRunningTasks: 1 });
    const svc = taskService();
    const first = svc.registerTask(fakeProcessTask());
    const reserved = svc.reserveTaskId('bash');
    expect(() => svc.registerTask(fakeProcessTask(), { taskId: reserved })).toThrow(
      /Too many background tasks/,
    );
    await svc.stop(first);
    svc.releaseTaskId(reserved);
    expect(svc.registerTask(fakeProcessTask(), { taskId: reserved })).toBe(reserved);
    expect(svc.getTask(reserved)?.status).toBe('running');
  });
  ...
  function stubLoop(): StubLoop {
    return getLoopControl(agentScope.agentContext) as unknown as StubLoop;
  }
  ...
  it('unwinds a nested wait chain leaf-first without deadlocking', async () => {
    ...
    const mainSvc = buildAgentIx('main', docs, bytes).get(ITaskLifecycle);
    const childSvc = buildAgentIx('child-1', docs, bytes).get(ITaskLifecycle);
    const mainTool = new WaitForTool(mainSvc as unknown as TaskRuntime, noopTelemetryService, stubFlag(true));
    const childTool = new WaitForTool(childSvc as unknown as TaskRuntime, noopTelemetryService, stubFlag(true));
  ...
  function abortObservingTask(onAbort: (reason: unknown) => void): TaskExecution {
    return {
      ...fakeProcessTask(),
      start: ({ signal }) => {
        if (signal.aborted) {
  ...
  it('stopAllOnExit suppresses and persists terminal state for detached tasks', async () => {
    ...
    for (const taskId of [first, second]) {
      ...
      const terminated = records.filter(
        (record) =>
          record['type'] === 'task.terminated' &&
          (record['info'] as AgentTaskInfo).taskId === taskId,
      );
      expect(terminated.at(-1)).toMatchObject({
        info: {
          status: 'killed',
  ...
  it('stopAllOnExit does not persist a foreground-only task', async () => {
    ...
    expect(
      records.filter(
        (record) => record['type'] === 'task.started' || record['type'] === 'task.terminated',
      ),
    ).toEqual([]);
  ...
  it('stopAllOnExit stops ordinary tasks even when keepAliveOnExit is set', async () => {
  ...
  it('stopAllOnExit skips executions that survive session close', async () => {
    const svc = taskService();
    const survivorId = svc.registerTask({
      ...fakeProcessTask(),
      survivesSessionClose: () => true,
    });
    const ordinaryId = svc.registerTask(fakeProcessTask());

    const stopped = await svc.stopAllOnExit('Session closed');

    expect(svc.getTask(survivorId)?.status).toBe('running');
    expect(svc.getTask(survivorId)?.terminalNotificationSuppressed).toBeUndefined();
    expect(svc.getTask(ordinaryId)?.status).toBe('killed');
    expect(stopped.map((info) => info.taskId).toSorted()).toEqual(
      [survivorId, ordinaryId].toSorted(),
    );
  });
  ...
  it('dispose stops ordinary tasks even when keepAliveOnExit is set', async () => {
    stubTaskConfig({ keepAliveOnExit: true });
    const svc = taskService();
    let aborted = false;
  ...
  it('scope disposal releases a surviving execution without stopping it', async () => {
    const svc = taskService();
    let aborted = false;
    const forceStop = vi.fn(async () => {});
    const releaseOnSessionClose = vi.fn();
    svc.registerTask({
      ...abortObservingTask(() => (aborted = true)),
      forceStop,
      survivesSessionClose: () => true,
    ...
    expect(releaseOnSessionClose).toHaveBeenCalledOnce();
  });
  ...
  function buildAgentIx(
    ...
    ix.stub(ILogService, stubLog());
    ix.stub(IWireService, stubWireService());
    const contextMemory = stubContextMemory();
    ix.stub(
      IAgentLifecycleService,
      lifecycleWithUndo(
        stubUndoRuntime(),
        lifecycleWithReminder(createReminderStub(), contextMemory),
      ),
    );
    ix.stub(ITelemetryService, { track: () => {}, track2: () => {} });
  ...
  it('rebuilds wait-delivered keys on restore and skips their re-delivery', async () => {
    ...
    const svc1 = one.get(ITaskLifecycle);
    await one.get(IEventDispatcher).restore();
    await svc1.beginRestore();
    ...
    const svc2 = two.get(ITaskLifecycle);
    await two.get(IEventDispatcher).restore();
    await svc2.beginRestore();
  ...
  it('restore touches only the agent own task records', async () => {
    ...
    const main = buildAgentIx('main', docs, bytes).get(ITaskLifecycle);
    ...
    const sub = buildAgentIx('agent-1', docs, bytes).get(ITaskLifecycle);
  ...
  it('main restore claims a previous v2 session task with its legacy output path', async () => {
    ...
    const main = mainIx.get(ITaskLifecycle);

    await main.beginRestore();
  ...
  it('subagent restore does not claim previous v2 session tasks', async () => {
    ...
    const subagent = subIx.get(ITaskLifecycle);

    await subagent.beginRestore();
  ...
  function agentLikeTask(result: string, description: string): TaskExecution {
    return {
      idPrefix: 'agent',
      kind: 'agent',
      description,
  ...
  function serviceWithAppendCounter(): {
    ...
    return { svc: taskService(), persistedChars: () => persistedChars };
  }
