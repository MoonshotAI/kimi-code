...
describe('AgentTaskService', () => {
  ...
  it('persists graceful process shutdown as killed when stop was requested', async () => {
    ...
    try {
      const writer = createAgentTaskService({ sessionDir });
      const { proc, resolve } = manuallyResolvedProcess();
      ...
      const reader = createAgentTaskService({
        sessionDir,
        wireRecords: await writer.ctx.persistedWireRecords(),
      });
      await reader.ctx.restorePersisted();
