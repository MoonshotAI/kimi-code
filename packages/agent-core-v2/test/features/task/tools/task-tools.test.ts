...
import { type AgentTaskStatus, TERMINAL_STATUSES } from '#/features/task/types';
import { AgentTools } from '#/features/toolExecutor/toolExecutorAgentRuntime';
import { TaskListInputSchema } from '#/features/task/tools/task-list/task-list';
import { TaskListTool } from '#/features/task/tools/task-list/taskListTool';
import { TaskOutputInputSchema } from '#/features/task/tools/task-output/task-output';
import { TaskOutputTool } from '#/features/task/tools/task-output/taskOutputTool';
import { TaskStopInputSchema } from '#/features/task/tools/task-stop/task-stop';
import { TaskStopTool } from '#/features/task/tools/task-stop/taskStopTool';
import { WaitForInputSchema } from '#/features/task/tools/task-wait/task-wait';
import { WaitForTool, startWaitProgress, waitForProgressUpdate } from '#/features/task/tools/task-wait/taskWaitTool';
...
import { type IWaitForTool } from '#/features/task/tools/task-wait/task-wait';
...
class FakeTaskServiceImpl {
  ...
  registerTask(_task: TaskExecution, _options?: RegisterAgentTaskOptions): string {
...
type FakeTaskService = FakeTaskServiceImpl & TaskRuntime;

function fakeTaskService(): FakeTaskService {
  return new FakeTaskServiceImpl() as FakeTaskService;
}

describe('TaskListTool', () => {
  it('has name and accepts the current schema', () => {
    const tool = new TaskListTool(fakeTaskService());
  ...
  it('returns the empty active-task message', async () => {
    const result = await executeTool(
      new TaskListTool(fakeTaskService()),
...
describe('TaskOutputTool', () => {
  it('has name and accepts the current schema', () => {
    const tool = new TaskOutputTool(fakeTaskService());
  ...
  it('returns error for unknown task', async () => {
    const result = await executeTool(
      new TaskOutputTool(fakeTaskService()),
  ...
  it('rejects stale block/timeout args at the validator instead of waiting', () => {
    const validator = compileToolArgsValidator(new TaskOutputTool(fakeTaskService()).parameters);
...
describe('TaskStopTool', () => {
  it('has name and accepts the current schema', () => {
    const tool = new TaskStopTool(fakeTaskService());
  ...
  it('returns error for unknown task', async () => {
    const result = await executeTool(
      new TaskStopTool(fakeTaskService()),
...
describe('WaitForTool', () => {
  ...
  it('has name and accepts the current schema', () => {
    const tool = new WaitForTool(fakeTaskService(), recordingTelemetry([]), stubFlag(true));
  ...
  it('returns error and tracks task_not_found for an unknown task_id', async () => {
    ...
    const result = await executeTool(
      new WaitForTool(fakeTaskService(), telemetry, stubFlag(true)),
...
describe('WaitForTool (harness)', () => {
  ...
  async function waitForTerminal(tasks: TaskRuntime, taskId: string): Promise<void> {
