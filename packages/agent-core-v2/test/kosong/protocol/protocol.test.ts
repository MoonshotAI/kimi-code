 ...
 * registry decorator.
 *
 * Locks the decision-1 invariants: the protocol enum holds only the five
 * real wire formats (a vendor name is not a protocol and is rejected), while
 * `providerType` on the adapter config stays a free-form string so external
 * packages can register vendors without touching this layer.
 */
...
describe('ProtocolSchema', () => {
  it('accepts the five real wire protocols', () => {
