...
describe('IExternalHooksRunnerService integration', () => {
  ...
  it('runs UserPromptSubmit for user-slash skill activations, not for model-tool triggers', async () => {
    const disposables = new DisposableStore();
    let ix: TestInstantiationService | undefined;
    try {
      const fired: unknown[] = [];
      const hookEngine = {
        trigger: async (_event: string, args: { inputData?: unknown }) => {
          fired.push(args.inputData);
          return [];
        },
        triggerBlock: async () => undefined,
        fireAndForgetTrigger: async () => [],
      ...
      ix.set(IAgentExternalHooksService, new SyncDescriptor(AgentExternalHooksService));
      ix.get(IAgentExternalHooksService);
      const prompt = ix.get(IAgentPromptService);

      const skillMessage: ContextMessage = {
        role: 'user',
        content: [{ type: 'text', text: 'rendered skill body' }],
        toolCalls: [],
        origin: {
      ...
      await prompt.hooks.onBeforeSubmitPrompt.run({
        promptMessage: skillMessage,
        isSteer: false,
        block: false,
      });
