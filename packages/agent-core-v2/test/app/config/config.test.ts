...
describe('Agent config', () => {
  ...
  it('keeps turn-start config for later steps and applies updates to the next turn', async () => {
    ...
    expect(await ctx.untilApproval(true)).toMatchInlineSnapshot(`
      ...
      [emit] permission.approval.requested   { "time": "<time>", "id": "<approval-1>", "sessionId": "test-session", "agentId": "main", "turnId": 0, "toolCallId": "call_lookup", "toolName": "Lookup", "action": "Approve Lookup", "display": { "kind": "generic", "summary": "Approve Lookup", "detail": { "query": "original" } }, "permissionRequestId": "<approval-1>", "toolInput": { "query": "original" } }
