...
/**
 * Delayed-instantiation tests ported from VS Code's
 * `instantiationService.test.ts` ("Delayed and events" family).
 *
 * A service registered with `SyncDescriptor(Ctor, [], true)` is handed to
 * consumers as a Proxy: subscribing to its `onDid…`/`onWill…` events does NOT
 * construct it, and the first real property/method access does. Listeners that
 * subscribed before construction are replayed onto the real instance once it
 * materializes.
 */
