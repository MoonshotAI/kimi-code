...
import { TowerProtocolError, TowerStore, parseFrontmatter } from '../../../src/agent/tower/protocol';
import type {
  TowerFindingType,
  TowerMission,
  TowerRosterEntry,
  TowerState,
} from '../../../src/agent/tower/protocol';
...
describe('init', () => {
  ...
  it('is idempotent — a second init reports created:false and preserves state', async () => {
    ...
    expect(second).toEqual({ base: 'main', created: false });
    const state = await store.load();
