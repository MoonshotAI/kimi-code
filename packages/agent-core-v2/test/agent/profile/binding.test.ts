...
describe('AgentProfileService.bind', () => {
  ...
  it('binds a profile + model atomically and becomes runnable', async () => {
    ...
    expect(svc.getSystemPrompt()).toContain("You are Claude Code, Anthropic's official CLI for Claude.");
  });
  ...
  it('persists the complete binding in one journal record', async () => {
    ...
    expect(records[0]).toMatchObject({
      ...
      thinkingEffort: 'on',
      systemPrompt: expect.stringContaining("You are Claude Code, Anthropic's official CLI for Claude."),
