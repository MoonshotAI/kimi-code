...
function createManagerStub(interaction: InteractionApi): IAgentManager {
  return { resolve: () => interaction } as unknown as IAgentManager;
}
...
beforeEach(() => {
  ...
  ix.stub(IAgentManager, createManagerStub(createInteractionStub()));
...
describe('AgentUserToolService (wire-backed)', () => {
  ...
  it('inherits currently registered parent user tools into another agent service', async () => {
    ...
    ixChild.stub(IAgentManager, createManagerStub(createInteractionStub()));
  ...
  it('execute parks under a minted interaction id and keeps the provider toolCallId on the payload', async () => {
    ...
    ixExec.stub(IAgentManager, createManagerStub(interactionStub));
  ...
  it('replay rebuilds the model silently and onDidRestore re-registers tools after replay', async () => {
    ...
    ix2.stub(IAgentManager, createManagerStub(createInteractionStub()));
