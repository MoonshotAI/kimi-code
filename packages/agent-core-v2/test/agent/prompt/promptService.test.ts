...
function stubSessionMetadata(initial?: Partial<SessionMeta>) {
  ...
  const service = {
    ...
    update,
    setTitle: async (title: string) => { data = { ...data, title, titleKind: 'custom' }; },
...
function harness(
  ...
  metaInitial?: Partial<SessionMeta>,
  agentId = 'main',
  providerType?: string,
) {
...
describe('AgentPromptService', () => {
  ...
  it('keeps a prompt image whose format the bound provider accepts', async () => {
    const { prompt, context, loop } = harness({ pendingTurnResult: true }, undefined, undefined, 'kimi');
...
describe('AgentPromptService session metadata sink', () => {
  it('writes title and lastPrompt for the first user prompt', async () => {
    ...
    await prompt.submit({ input: [{ type: 'text', text: 'hello world' }] });
  ...
  it('refreshes lastPrompt on repeated identical prompt text', async () => {
    const { prompt, sessionMeta } = harness();
    await prompt.submit({ input: [{ type: 'text', text: 'same text' }] });
    await prompt.submit({ input: [{ type: 'text', text: 'same text' }] });
    expect(sessionMeta.update).toHaveBeenCalledTimes(2);
    expect(sessionMeta.read().lastPrompt).toBe('same text');
  });

  it('skips metadata writes for non-main agents', async () => {
    const { prompt, sessionMeta } = harness(undefined, undefined, 'sub-1');
    await prompt.submit({ input: [{ type: 'text', text: 'subagent task' }] });
    expect(sessionMeta.update).not.toHaveBeenCalled();
  });

  it('updates lastPrompt while preserving a custom title', async () => {
    const { prompt, sessionMeta } = harness(undefined, {
      title: 'My Session',
      titleKind: 'custom',
    });
    await prompt.submit({ input: [{ type: 'text', text: 'next question' }] });
    expect(sessionMeta.update).toHaveBeenCalledTimes(1);
