...
describe('applyPromptMetadataUpdate', () => {
  ...
  it('treats empty and placeholder titles as untitled', async () => {
    for (const title of ['', 'New Session']) {
      const { target, readMeta } = createTarget({ title });

      await applyPromptMetadataUpdate(target, 'hello');

      expect(readMeta().title).toBe('hello');
      expect(readMeta().titleKind).toBe('replaceable');
    }
  });

  it('leaves a custom title untouched but updates lastPrompt', async () => {
    const { target, readMeta } = createTarget({ title: 'My Session', titleKind: 'custom' });

    await applyPromptMetadataUpdate(target, 'next');

    expect(readMeta().title).toBe('My Session');
    expect(readMeta().lastPrompt).toBe('next');
  });
});
