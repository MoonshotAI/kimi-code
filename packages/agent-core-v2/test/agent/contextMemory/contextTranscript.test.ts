...
describe('reduceContextTranscript — log.cut (wire rewind)', () => {
  it('drops the record range invalidated by a log.cut', () => {
    // Journal: prompt(0), u1(1), a1(2..4), prompt(5), u2(6), a2(7..9), cut(10→5)
    const result = reduceContextTranscript([
      ...
      appendMessage(userMessage('u2')),
      ...assistantStep('s2', 'a2'),
      { type: 'log.cut', target: 5, reason: 'undo' },
    ]);
    expect(texts(result)).toEqual(['u1', 'a1']);
    expect(result.foldedLength).toBe(2);
  });

  it('ignores the metadata envelope without giving it a record index', () => {
    const result = reduceContextTranscript([
      { type: 'metadata', protocol_version: '1.5', created_at: 1 },
      appendMessage(userMessage('u1')),
      appendMessage(userMessage('u2')),
      { type: 'log.cut', target: 1, reason: 'undo' },
    ]);
    expect(texts(result)).toEqual(['u1']);
  });

  it('applies nested cuts and keeps records appended after them', () => {
    // u2 invalidated by the first cut; u3/u4 by the second; cuts compose.
    const result = reduceContextTranscript([
      appendMessage(userMessage('u1')),
      appendMessage(userMessage('u2')),
      { type: 'log.cut', target: 1, reason: 'undo' },
      appendMessage(userMessage('u3')),
      appendMessage(userMessage('u4')),
      { type: 'log.cut', target: 3, reason: 'undo' },
      appendMessage(userMessage('u5')),
    ]);
    expect(texts(result)).toEqual(['u1', 'u5']);
  });
});
