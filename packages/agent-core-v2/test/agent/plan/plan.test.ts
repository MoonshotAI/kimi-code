...
describe('Plan service', () => {
  ...
  function replayedToolCall(toolCallId: string) {
    return reduceContextTranscript(ctx.wireRecord.getRecords() as readonly PersistedRecord[])
      .entries.flatMap((message) => message.toolCalls)
  ...
  describe('plan exit tool', () => {
    ...
    it('stops the turn and stays in plan mode when the user rejects the plan', async () => {
      ...
      expect(replayedToolCall('call_exit_reject')).toMatchObject({
        display: {
          kind: 'plan_review',
          plan: '# Plan\n\n- Inspect\n- Change\n- Verify',
          path: planPath,
  ...
  describe('plan exit tool options', () => {
    it('keeps options for approval when an option omits the optional description', async () => {
      ...
      await ctx.untilTurnEnd();
      expect(replayedToolCall('call_exit_options')).toMatchObject({
        display: {
          kind: 'plan_review',
          plan: '# Plan\n\n- Inspect\n- Change\n- Verify',
          path: planPath,
        },
        approvalResult: { decision: 'approved', selectedLabel: 'Approach A' },
      });
    });
  });
