...
import type { LlmErrorMessage, LlmRemoteErrorMessage } from '#human/llm/errors';
import type { MediaVideoUploader } from '#human/llm/media/upload';
import type { StreamedMessagePart, VideoURLPart } from '#human/llm/message';
import type { LlmModel } from '#human/llm/model';
import type { LlmRequestControl, LlmRequester } from '#human/llm/requester/requester';

import { withAuth, withAuthUpload, type CredentialSource } from '#/llm-adapter/model/request-auth';
...
function createInner(plan: readonly (LlmRemoteErrorMessage | 'ok')[]) {
  const calls: InnerCall[] = [];
  const requester: LlmRequester = {
    generate: (config, _content, { onEvent }) => {
      calls.push({ model: config.model });
      const step = plan[Math.min(calls.length - 1, plan.length - 1)] ?? 'ok';
      if (step === 'ok') {
        onEvent?.({
