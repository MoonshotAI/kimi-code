...
import type { BytePipe, ExecBridge } from '#/remote/client/execBridge';
import { RemoteExecConnection, type ConnectOptions } from '#/remote/client/connection';
import { LineFrameDecoder } from '#/remote/protocol/codec';
import type { InitializeResult, RemoteEnvironmentInfo } from '#/remote/protocol/methods';
import { StdioHost } from '#/remote/server/stdioHost';
...
export async function spawnExecutorBridge(env?: Record<string, string>): Promise<SpawnedExecutor> {
  ...
  const { ExecBridge } = await import('#/remote/client/execBridge');
