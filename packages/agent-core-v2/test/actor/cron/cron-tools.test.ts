...
import type { CronJobOrigin } from '#/actor/contextMemory/types';
...
import { type CronRuntime } from '#/actor/cron/cronAgentRuntime';
import {
  computeNextCronRun,
  parseCronExpression,
} from '#/actor/cron/internal/cron-expr';
import { renderCronFireXml } from '#/actor/cron/internal/format';
import {
  jitteredNextCronRunMs,
  oneShotJitteredNextCronRunMs,
} from '#/actor/cron/internal/jitter';
import {
  MAX_CRON_JOBS_PER_SESSION,
  type CronCreateInput,
} from '#/actor/cron/tools/cron-create/cron-create';
import { CronCreateTool } from '#/actor/cron/tools/cron-create/cronCreateTool';
import type { CronDeleteInput } from '#/actor/cron/tools/cron-delete/cron-delete';
import { CronDeleteTool } from '#/actor/cron/tools/cron-delete/cronDeleteTool';
import type { CronListInput } from '#/actor/cron/tools/cron-list/cron-list';
...
function createToolHarness(options: {
  ...
  const manager = { resolve: () => cron } as unknown as IAgentLifecycleService;
...
describe('cron tools on non-main agents', () => {
  it('CronCreate rejects with the main-agent-only error before any validation', async () => {
    const harness = createToolHarness();
    const tool = new CronCreateTool(harness.manager, subagentScopeContext);
  ...
  it('CronDelete rejects with the main-agent-only error before mutating the store', async () => {
    const harness = createToolHarness();
    harness.store.add({ cron: '*/5 * * * *', prompt: 'ping', recurring: true }, harness.now());
    const tool = new CronDeleteTool(harness.manager, subagentScopeContext);
  ...
  it('CronList rejects with the main-agent-only error', async () => {
    const harness = createToolHarness();
    const tool = new CronListTool(harness.manager, subagentScopeContext);
