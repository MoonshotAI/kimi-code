...
export async function drainSessionLeaseReleases(): Promise<void> {
...
export class SessionLeaseManager extends Disposable {
  ...
  async acquire(sessionId: string, onLost: () => void): Promise<SessionLeaseHolder> {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      if (!(await this.removeIfStillStale(filePath, holderOnDisk, mtimeMs))) {
  ...
  async release(sessionId: string): Promise<void> {
    ...
    const mtimeMs = await this.mtimeOf(entry.filePath);
    if (mtimeMs === undefined) return;
    if (!(await this.removeIfStillStale(entry.filePath, current, mtimeMs))) {
      this.log.warn('session lease release skipped; the lock was replaced meanwhile', { sessionId });
    }
  }
  ...
  override dispose(): void {
    ...
    const release = this.releaseAll().catch((error: unknown) => {
      this.log.error('session lease teardown failed', error);
    });
    pendingReleases.add(release);
    void release.finally(() => pendingReleases.delete(release));
    super.dispose();
  }
  ...
  private async renew(entry: HeldLease): Promise<void> {
    ...
    if (holderOnDisk.leaseId !== entry.holder.leaseId) {
      if (holderOnDisk.acquiredAt < entry.holder.acquiredAt && (await this.retake(entry, holderOnDisk))) {
        return;
      }
      this.lost(entry);
      return;
  ...
  private async retake(entry: HeldLease, zombie: SessionLeaseHolder): Promise<boolean> {
    this.log.warn('session lease clobbered by a stale holder, retaking', {
      sessionId: entry.sessionId,
      zombie,
    });
    for (let attempt = 0; attempt < 2; attempt++) {
      await this.fs.remove(entry.filePath).catch(() => undefined);
      if (await this.fs.createExclusive(entry.filePath, encodeLease(entry.holder))) {
        return true;
      }
      const current = await this.readHolder(entry.filePath);
      if (current === undefined || current.acquiredAt >= entry.holder.acquiredAt) {
        return false;
      }
    }
    return false;
  ...
  private async removeIfStillStale(
    filePath: string,
    observed: SessionLeaseHolder | undefined,
    observedMtimeMs: number,
  ): Promise<boolean> {
    const current = await this.readHolder(filePath);
    if (current?.leaseId !== observed?.leaseId) return false;
    const mtimeMs = await this.mtimeOf(filePath);
    if (mtimeMs !== observedMtimeMs) return false;
