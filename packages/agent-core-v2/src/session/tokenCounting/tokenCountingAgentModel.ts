...
import type { TokenCountingRebaseInput, TokenMeasurement } from './sessionTokenCounting';

type TailEstimator = (messages: readonly ContextMessage[]) => number;
...
export class TokenCountingAgentModel extends AgentModel<TokenCountingState> {
  ...
  get(
    ...
    estimateProjected: TailEstimator = estimateTokensForMessages,
  ): ContextSize {
    ...
    const estimated =
      this.state.anchors.length === 0 && from === 0 && to === context.length
        ? estimateProjected(tail)
  ...
  latestMeasurement(): TokenMeasurement | undefined {
    const latest = this.state.anchors.at(-1);
    if (latest === undefined) return undefined;
    return { length: latest.length, tokens: latest.tokens, measured: latest.measured };
  }
  ...
  statusSize(
    ...
    return Math.max(this.get(undefined, undefined, estimateProjected).size, this.latestMeasured());
  }
  ...
  recordTurn(
    ...
    return this.emit(
      new TokenCountingTurnRecorded({
        ...
        tokens: this.statusSize(strategy, estimateProjected),
      }),
    );
  }
