 ...
 * frozen at construction from the `sessionContext` seed (`cwd`); the
 * additional dirs are a live read view over the handler-shared set, seeded
 * as `ISessionWorkspaceInfo` by the workspace handler — the workspace-level
 * add-dir surface (`workspaceDirs`) and its `local.toml` watch refresh this
 * view through the seed's change event. The plain-data state (`workDir`,
 * `additionalDirs`) is registered into `sessionState`
