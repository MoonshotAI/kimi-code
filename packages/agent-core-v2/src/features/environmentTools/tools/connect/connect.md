...
- Temporary environments are not persisted: they vanish when the process exits, and a session resumed onto one finds it gone — switch back to `local` (or another environment) before ending the session if the work should continue later. A dropped connection can be retried in the same process with `change_environment` or reconnect.
