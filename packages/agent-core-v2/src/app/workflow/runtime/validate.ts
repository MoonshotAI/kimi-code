/**
 * `workflow` domain (L6) — workflow metadata validation (engine-agnostic).
 *
 * Zod schemas and guards for `WorkflowMeta` and workflow names, plus the
 * `WorkflowValidationError` raised by every validation failure in the script /
 * runtime layers. Pure functions — no scoped state.
 */
import { z } from 'zod';
