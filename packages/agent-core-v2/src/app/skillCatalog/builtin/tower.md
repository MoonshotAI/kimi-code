...
1. **Init** — `TowerInit`. It creates `.tower/`, enables the tower tool set, and records the base branch. If the session's permission mode would interrupt long-running background agents with constant prompts, tell the human once that a more autonomous mode fits tower better — then proceed regardless.
