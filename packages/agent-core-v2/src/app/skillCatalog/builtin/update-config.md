...
- **`config.toml`** — agent / runtime settings: `default_model`, `[secondary_model]` (`default_model` / `[secondary_model.models]` subagent model pool / `force` to pin subagents to `default_model`), `[subagent]` (`timeout_ms`), `providers`, `models`, `thinking`, `permission`, `hooks`, `loop_control`, etc.
