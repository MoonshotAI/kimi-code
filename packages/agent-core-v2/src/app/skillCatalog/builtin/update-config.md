...
- **`config.toml`** — agent / runtime settings: `default_model`, `[subagent]` (`default_model` / `[subagent.models]` subagent model pool), `providers`, `models`, `thinking`, `permission`, `hooks`, `loop_control`, etc.
