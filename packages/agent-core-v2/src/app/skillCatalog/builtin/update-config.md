...
- **`config.toml`** — agent / runtime settings: `default_model`, `[secondary_model]` (`default_model` / `[secondary_model.models]` subagent model pool), `[subagent]` (`timeout_ms`), `providers`, `models`, `thinking`, `permission`, `hooks`, `loop_control`, etc.
