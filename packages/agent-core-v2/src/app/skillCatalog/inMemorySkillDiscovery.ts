 ...
 * roots returns the user skills, one seeded with plugin roots returns the plugin
 * skills, and an empty root list (the common test case where the resolved
 * directories do not exist on disk) returns everything the double holds — user
 * skills first, project skills last, so project entries win the within-list
 * collision resolution the same way the workspace source's higher priority wins
 * across sources. App-scoped.
 */
