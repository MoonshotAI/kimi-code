...
const ENVIRONMENTS_SECTION_PROSE =
  'This session can execute tool calls on the environments below. Use `change_environment` to switch the active environment (the switch takes effect at the next turn boundary), or `connect` to create a temporary environment from a launcher spec.';
