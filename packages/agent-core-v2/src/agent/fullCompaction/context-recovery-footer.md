...
- Reading: lines are long JSON (often 10k+ chars). Grep the file for a keyword to get line numbers, then Read exactly that line (line_offset=N, n_lines=1). Read returns whole lines; request a larger max_chars if a record does not fit. To pull one field with real newlines: sed -n 'Np' wire.jsonl | jq -r '.event.result.output'. Prefer individual records over large ranges, and follow the Next Read arguments when a range is incomplete.
