 ...
 *
 * Splits a parsed bash command into the executable unit texts — commands
 * (with their redirections and assignment prefixes attached), standalone
 * variable assignments, redirected groups, and the payloads of command /
 * process substitutions at any depth — so rule evaluation can judge a
 * compound command per unit instead of as one opaque string. A `deny`/`ask`
 ...
 * session-approval shape) — a wildcard pattern must not span operators, and
 * parse failure or budget exhaustion must not fall back to whole-string
 * globbing, since both are exactly the over-match being closed. Quoted
 ...
 * matches through `rule-match`. Pure functions plus a memoizing provider;
 * no scoped service.
 */
...
export function matchesDecomposedCommandRule(
  ...
  // An unknown decision must not expand into per-part matching, since that
  // expansion is only sound once the allow/deny direction is known.
