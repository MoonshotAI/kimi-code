...
export const AgentSwarmToolInputSchema = z
  .object({
    ...
    model: z
      ...
      .optional()
      .describe(
        'Which model to run the item-spawned subagents on. Choose from the slot names listed under "Available models" in this tool description (e.g. "fast", "quality"), or "primary" for the main model. When named slots are absent, the legacy choices are "secondary" and "primary". This explicit choice overrides the selected agent type\'s model_preference. Only effective when subagent model selection is enabled; otherwise subagents inherit your model. Resumed subagents always keep their own model.',
      ),
  })
  .strict();
