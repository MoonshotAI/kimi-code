...
export interface AgentRuntimeProvider<State, Runtime> {
...
): AgentRuntimeProvider<State, Runtime> {
  for (const cls of descriptor.durable?.events ?? []) registerEvent2Class(cls);
  const provider = Object.freeze({ contract }) as AgentRuntimeProvider<State, Runtime>;
...
export function getAgentRuntimeDefinitionId(
  definition: AgentRuntimeDefinition<any> | AgentRuntimeProvider<any, any>,
): string {
...
export function getAgentRuntimeContract(
  contribution: AgentRuntimeDefinition<any> | AgentRuntimeProvider<any, any>,
...
export interface AgentRuntimeDefinitionRecord {
  ...
  readonly provider?: AgentRuntimeProvider<any, any> | AgentRuntimeDefinition<any>;
  readonly generation: number;
