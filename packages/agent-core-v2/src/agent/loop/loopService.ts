 ...
 * `turn.*` events and the transient `turn.*` / delta observables through
 * `state` (`IEventDispatcher`), persists loop events through
 * `contextMemory`, and reads the step budget from `config`. The plain-data
 * loop state
