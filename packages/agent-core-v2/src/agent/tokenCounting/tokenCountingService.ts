...
export class AgentTokenCountingService extends Disposable implements IAgentTokenCountingService {
  ...
  constructor(
    ...
    this._register(this.eventBus.subscribe(ContextSpliced, () => this.publishSizes()));
  }
  ...
  get(start?: number, end?: number): ContextSize {
    ...
    const neverMeasured = this.agentState.get(tokenCountingKey).anchors.length === 0;
    const estimated =
      neverMeasured && from === 0 && to === context.length
        ? this.estimateProjected(tail)
        : this.estimateMessages(tail);
  ...
  latestMeasurement(): TokenMeasurement | undefined {
    const latest = this.agentState.get(tokenCountingKey).anchors.at(-1);
  ...
  private publishSizes(): void {
    ...
    void this.dispatcher.dispatch(new AgentStatusUpdated({ contextTokens, rawContextTokens }));
  }
