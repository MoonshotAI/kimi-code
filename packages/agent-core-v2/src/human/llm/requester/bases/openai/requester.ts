...
export function prepareOpenAIRequest(
  ...
  return encodeOpenAIRequest(finalParams, input.stream !== false);
}
...
async function executeOpenAIRequest(
  ...
  if (!request.stream) {
    const { data: completion, response } = await client.chat.completions
      .create(
        request.params as unknown as OpenAI.Chat.ChatCompletionCreateParamsNonStreaming,
        { signal },
      )
      .withResponse();
    onEvent?.({ type: 'llm.streaming.headers', headers: headersToRecord(response.headers) ?? {} });
    reasoning.observe(completion.choices[0]?.message);
    parse(openAIChatCompletionToChunk(completion), sink);
    if (failed) {
      return;
    }
