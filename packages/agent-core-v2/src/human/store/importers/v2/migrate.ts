...
import type { Branch } from '#/store/storage';
import { SESSION_TREE_NAME } from '#/session/layout';
...
export async function migrateV2Session(dir: string, options?: { treeName?: string }): Promise<V2MigrationResult> {
  const treeName = options?.treeName ?? SESSION_TREE_NAME;
  ...
  return { treeName, agents: agentIds };
}
