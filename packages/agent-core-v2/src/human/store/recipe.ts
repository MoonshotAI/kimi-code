...
export function createStore<S extends object>(name: string, setup: () => S): StoreRecipe<S> {
  const recipe: StoreRecipe<S> = {
    ...
    onMount: (_recipe, node) => {
      const watchers = new Set<(face: unknown) => void>();
      pushCleanup(node, () => watchers.clear());
      bindFaceWatchers(name, node, watchers);
      const parent = node.parent;
      if (parent === null) {
        return;
      }
      const handle = buildStoreHandle(name, node, watchers);
...
function buildStoreHandle(
  name: string,
  node: UnitNode,
  watchers: Set<(face: unknown) => void>,
): StoreResolution<any> {
  const latestFace = (): Record<string, unknown> => {
    if (node.state === 'unmounted' || node.state === 'failed') {
      throw new Error(`store '${name}' is ${node.state}`);
    }
...
function bindFaceWatchers(name: string, node: UnitNode, watchers: Set<(face: unknown) => void>): void {
  const face = node.setupResult;
  ...
  node.scope.run(() => {
    for (const key of keys) {
      watch(
        () => toValue(record[key]),
        () => {
          node.fire({ type: 'store.state', store: name, state: nonFunctionFields(record) });
          for (const listener of Array.from(watchers)) listener(face);
        },
      );
    }
  });
