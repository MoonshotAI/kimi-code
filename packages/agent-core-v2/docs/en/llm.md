...
3. **format masks inter-protocol differences; dialects express provider customizations**. format lives at the protocol layer and handles encoding/decoding of requests, responses, errors, usage, and finish. Each protocol owns a typed dialect interface (`OpenAIDialect` / `OpenAIResponsesDialect` / `AnthropicDialect` / `GoogleGenAIDialect`) exposing only the customization points that protocol actually consumes — a hook a protocol ignores is unrepresentable, never silently dead. format and dialect never import each other: both speak only the neutral wire/chunk types in the protocol's `contract.ts`. The requester is the composition root — `generate` runs a fixed per-protocol pipeline (`planOpenAIRequest` and friends) that alternates pure format stages (lower → assemble → encode → stream parser) with dialect hooks (cacheKey/thinking → convertMessage → mergeHistory → convertTool → buildParams → extractUsage), so customization is explicit data flow instead of a closure captured inside format. Endpoint/env resolution and default headers form the provider `connection`, error classification is a requester option, and model capability is a provider-variant field — none of them are format business. Protocol differences must not leak into the machine or into requester decorators.
...
│                         each with contract / format / lower / patterns / capability / extra-params / dialect / requester
│
├── provider/
...
- DDD domain-method wrapping (Generation Domain, etc.) — use the format/dialect/provider layering instead.
- A single cross-protocol trait bag holding every vendor hook (the old ProtocolTrait) — per-protocol typed dialects, composed by the requester's request pipeline.
- Binding the dialect into the format (a `createOpenAIFormat(dialect)` closure, or dialect hooks passed as formatRequest options) — the requester pipeline alternates format stages and dialect hooks explicitly; the two sides only share the neutral `contract.ts` types.
