...
7. **Every variable capability is a contribution point**. Providers, media upload/degradation, usage, traceId, error recovery (compaction / media degradation), and credentials (a request resolver for injection plus an `LlmRecovery` strategy for refresh) all plug in through extension points; the llm core contains none of these concepts.
...
- Wrapping `generate` in an auth decorator that intercepts the event stream (swallowing `llm.failed.remote` to retry privately) — credentials plug in as a per-attempt `LlmRequestResolver` (injection) plus an `LlmRecovery` strategy (refresh), keeping a single retry path and a fully visible event stream.
