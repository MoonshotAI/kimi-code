...
1. **Command — imperative verb**. Asks the receiver to do something. Examples: `input.submit`, `input.steer`, `input.abort`, `input.remind`, `tool.abort`, `turn.abort`, `turn.drain`, `agent.notify`, `turn.spawn_tools`, `context.reset`.
2. **Fact — past participle**. Reports that something already happened; usually drives transitions or parent-level bookkeeping. Examples: `llm.sent`, `llm.done`, `llm.failed.syntax`, `llm.failed.remote`, `llm.retrying`, `llm.recovering`, `tool.done`, `tool.failed`, `tool.aborted`, `tool.detached`, `todo.used`. Emitted events are facts by definition: `turn.started`, `step.started`, `turn.done`, `turn.failed`, `turn.aborted`, `turn.aborting`, `turn.drained`, `agent.created`, `agent.forked`, `agent.switched`, `agent.stopped`, `agent.failed`, `usage.updated`.
...
- Multi-word segments use `snake_case`: `turn.spawn_tools`, `llm.streaming.message_id`. Never kebab-case or camelCase inside a segment.
