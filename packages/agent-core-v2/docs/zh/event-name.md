...
1. **命令 —— 动词原形**。要求接收方做事。例：`input.submit`、`input.steer`、`input.abort`、`input.remind`、`tool.abort`、`turn.abort`、`turn.drain`、`agent.notify`、`turn.spawn_tools`、`context.reset`。
2. **事实 —— 过去分词**。报告某事已发生，通常驱动转移或父级记账。例：`llm.sent`、`llm.done`、`llm.failed.syntax`、`llm.failed.remote`、`llm.retrying`、`llm.recovering`、`tool.done`、`tool.failed`、`tool.aborted`、`tool.detached`、`todo.used`。emitted 事件天然是事实：`turn.started`、`step.started`、`turn.done`、`turn.failed`、`turn.aborted`、`turn.aborting`、`turn.drained`、`agent.created`、`agent.forked`、`agent.switched`、`agent.stopped`、`agent.failed`、`usage.updated`。
...
- 多词段用 `snake_case`：`turn.spawn_tools`、`llm.streaming.message_id`。段内禁止 kebab-case 与 camelCase。
