...
5. **无状态内核 + 状态机外壳**。`generate(config, content, control)` 是无状态函数，错误走 onEvent 不 throw；llm machine 包装单次请求（request resolver、abort 作用域、事件转发），并借助 retry.ts / recovery.ts 的纯策略函数驱动重试与 recovery：每次 attempt 前，各 `LlmRequestResolver` 可替换 config 和/或 messages（以上一次 attempt 的错误作为 `lastAttemptError` 传入）；recovery 由纯函数 `propose` 产出 proposal 直接重发——可以是替换消息，也可以是只有 action 的 proposal（如凭证刷新，其副作用由 attempt 前的 resolver 完成）（attempt 重置为 1）；重试走 `retrying` 状态的 backoff（尊重 Retry-After），两者分别对外补发 `llm.recovering / llm.retrying` 事件；empty response 由 `withEmptyResponseGuard` 在 requester 边界判定并转为 `llm.failed.remote`，进入同一重试路径；abort 由 turn 持有的 AbortController 承载：controller 经 `LlmInput.signal` 传入 machine 与 request actor，turn 在 `turn.abort` 时直接 abort 它，请求随即以 `llm.failed.remote` 收尾；request actor 不自建 controller、回收时不触碰任何 signal，正常完成的请求绝不可能误 abort 共享 signal。累积器由 turn 持有并随事件流喂入，在 `llm.retrying / llm.recovering` 时 rollback 并重建，每次 attempt 从零累积，从而尽可能保留中断现场（turn 在 `llm.done` 时从累加器 finish 出完整消息）。
6. **不兜底**。配置是什么就是什么；beta 特性、thinking、empty response 等场景先定义明确报错条件，在请求阶段报错并引导用户修正，而不是静默兜底。
7. **一切可变能力都是贡献点**。provider、媒体上传/降级、usage、traceId、错误恢复（compaction/媒体降级）、凭证（request resolver 负责注入、`LlmRecovery` 策略负责刷新）都通过扩展点接入，llm 内核不含这些概念。
...
- 用拦截事件流的 auth 装饰器包裹 `generate`（吞掉 `llm.failed.remote` 私下重试）—— 凭证以每次 attempt 前的 `LlmRequestResolver`（注入）加 `LlmRecovery` 策略（刷新）接入，保证重试只有一条路径、事件流全程可见。
