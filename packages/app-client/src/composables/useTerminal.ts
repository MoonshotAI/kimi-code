...
import type { AppTerminal, KimiEventConnection, KimiWebApi } from '@moonshot-ai/app-core/api';

export function useTerminal(sessionId: Ref<string>, api: KimiWebApi) {
  ...
  async function close(): Promise<void> {
    ...
    try {
      ...
      await api.closeTerminal(current.sessionId, current.id);
    } catch (error_) {
      error.value = error_ instanceof Error ? error_.message : String(error_);
    }
  }
