// packages/app-client/src/lib/openFileAttachment.ts
...
// bare getFileUrl src 401s under daemon auth. The api is injected (first
// parameter); the app shells pass their composed singleton.
...
export async function openFileAttachment(
  ...
  const blob = await api.getFileBlob(fileId).catch(() => null);
  if (blob === null) {
    win?.close();
    return 'failed';
  }
