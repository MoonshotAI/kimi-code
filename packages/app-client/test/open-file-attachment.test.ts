...
const api = { getFileBlob: mocks.getFileBlob };
...
describe('openFileAttachment', () => {
  ...
  it('refuses to preview text/html — an active document would run same-origin', async () => {
    const result = await openFileAttachment(api, 'f_1', 'page.html', 'text/html');
    expect(result).toBe('unsupported');
    expect(windowOpen).not.toHaveBeenCalled();
  });

  it('refuses to preview image/svg+xml — SVG carries script when navigated', async () => {
    const result = await openFileAttachment(api, 'f_1', 'vector.svg', 'image/svg+xml');
    expect(result).toBe('unsupported');
    expect(windowOpen).not.toHaveBeenCalled();
  });

  it('refuses a .html extension even with an empty recorded MIME', async () => {
    const result = await openFileAttachment(api, 'f_1', 'page.html', '');
    expect(result).toBe('unsupported');
  });

  it('renders script source inert: text/* is pinned to text/plain, never executed', async () => {
    const result = await openFileAttachment(api, 'f_1', 'a.js', 'text/javascript');
  ...
  it('refuses non-text xml types outright', async () => {
    expect(await openFileAttachment(api, 'f_1', 'a.xml', 'application/xml')).toBe('unsupported');
    expect(await openFileAttachment(api, 'f_1', 'a.xhtml', 'application/xhtml+xml')).toBe('unsupported');
  });

  it('refuses an extensionless file with no usable MIME', async () => {
    expect(await openFileAttachment(api, 'f_1', 'Makefile', '')).toBe('unsupported');
  });

  it('previews pdf / safe images / media with their whitelisted MIME', async () => {
    expect(await openFileAttachment(api, 'f_1', 'a.pdf', 'application/pdf')).toBe('previewed');
    expect(previewedBlobType()).toBe('application/pdf');
    expect(await openFileAttachment(api, 'f_1', 'a.png', 'image/png')).toBe('previewed');
    expect(previewedBlobType()).toBe('image/png');
    expect(await openFileAttachment(api, 'f_1', 'a.mp4', 'video/mp4')).toBe('previewed');
  ...
  it('previews text but pins the blob to text/plain so it renders inert', async () => {
    ...
    const result = await openFileAttachment(api, 'f_1', 'evil.html', 'text/plain');
    expect(result).toBe('previewed');
    expect(previewedBlobType()).toBe('text/plain;charset=utf-8');
  });

  it('previews text-ish extensions with an empty MIME as text/plain', async () => {
    const result = await openFileAttachment(api, 'f_1', 'notes.md', '');
    expect(result).toBe('previewed');
    expect(previewedBlobType()).toBe('text/plain;charset=utf-8');
  });

  it('severs window.opener on the preview tab', async () => {
    await openFileAttachment(api, 'f_1', 'a.pdf', 'application/pdf');
  ...
  it('closes the tab and reports failure when the byte fetch fails', async () => {
    ...
    const result = await openFileAttachment(api, 'f_1', 'a.pdf', 'application/pdf');
