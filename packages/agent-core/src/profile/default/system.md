...
The current time is not embedded in this prompt. Whenever the real current time matters — web-result freshness, age or expiry checks, anything time-sensitive — get it fresh from the environment, for example by running `date` if you have a shell tool.
