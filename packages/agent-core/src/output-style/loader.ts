...
export async function loadOutputStyles(options: LoadOutputStylesOptions): Promise<readonly OutputStyle[]> {
  ...
  await scanDir(path.join(brandHomeDir, 'output-styles'), 'user');
  await scanDir(path.join(projectRoot, '.kimi-code', 'output-styles'), 'project');
...
async function findProjectRoot(workDir: string): Promise<string> {
  ...
  while (true) {
    try { await fs.stat(path.join(current, '.git')); return current; } catch { /* keep walking */ }
