...
jobs:
  build-test-push:
    ...
    steps:
      ...
      - name: Build image
        run: docker build -t myapp:$ {\1{ github.sha } } .
      - name: Run tests
        run: docker run --rm myapp:$ {\1{ github.sha } } pytest
      - name: Scan image
        uses: aquasecurity/trivy-action@master
        with:
          image-ref: myapp:$ {\1{ github.sha } }
      - name: Push to registry
        run: |
          docker tag myapp:$ {\1{ github.sha } } ghcr.io/org/myapp:$ {\1{ github.sha } }
          docker push ghcr.io/org/myapp:$ {\1{ github.sha } }
```
