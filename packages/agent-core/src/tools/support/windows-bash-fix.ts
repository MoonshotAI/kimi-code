/**
 * Windows Git Bash pre-process fixes for the Bash tool (v1).
 *
 * Ported from kimi-agent's `bash_fix.py` (Python scanner) onto the pure
 * `@moonshot-ai/tree-sitter-bash` parser as a shell-aware front end. Before
 * `bash -c` runs on Windows the fixer rewrites Windows-isms the model tends to
 * emit:
 *
