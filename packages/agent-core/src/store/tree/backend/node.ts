...
import type { BlobBackend, TreeBackend } from './backend';

function isEnoent(error: unknown): boolean {
  return error instanceof Error && 'code' in error && error.code === 'ENOENT';
}

export class NodeTreeBackend implements TreeBackend {
...
export class NodeBlobBackend implements BlobBackend {
  constructor(private readonly dir: string) {}
  ...
  async read(ref: string): Promise<Uint8Array> {
    return new Uint8Array(await readFile(this.path(ref)));
  }

  async write(ref: string, data: Uint8Array): Promise<void> {
...
export class NodeBackend {
  readonly trees: NodeTreeBackend;
  readonly blobs: NodeBlobBackend;
