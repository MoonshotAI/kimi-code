import { encodeHeader, encodeLine, parseHeader, parseLine } from './codec';
...
export class Branch {
  ...
  static create(
    ...
    void branch
      .enqueue(() => ctx.backend.write(tree, name, encodeHeader(header)))
      .catch(() => undefined);
    return branch;
  }
  ...
  static async load(
    ...
    const content = await ctx.backend.read(tree, name);
    ...
    if (!truncated && content.length > 0 && !content.endsWith('\n')) {
      await ctx.backend.append(tree, name, '\n');
    }
  ...
  append(input: AppendInput): Promise<EntryLine> {
    return this.enqueue(async () => {
      ...
      const payload: Payload = { kind: input.kind, size, data: input.data ?? null };
  ...
  repair(): Promise<void> {
    return this.enqueue(async () => {
      ...
      const content = await this.ctx.backend.read(this.tree, this.name);
      const physical = content.split('\n');
      if (physical.at(-1) === '') physical.pop();
  ...
  *walk(): Generator<EntryLine> {
  ...
  private async writeLine(entry: EntryLine): Promise<void> {
    await this.ctx.backend.append(this.tree, this.name, encodeLine(entry));
    if (this.ctx.fsync) await this.ctx.backend.sync?.(this.tree, this.name);
  }
...
function publish(ctx: TreeContext, tree: string, name: string, lines: string[]): Promise<void> {
  return ctx.backend.write(tree, name, `${lines.join('\n')}\n`);
}
