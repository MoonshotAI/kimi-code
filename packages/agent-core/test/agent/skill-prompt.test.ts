...
describe('renderSkillLoadedBlock bundled resources', () => {
  ...
  it('lists bundled resource paths inside the loaded block when provided', () => {
    const text = renderUserSlashSkillPrompt({
      ...base,
      skillResources: ['references/hooks-patterns.md', 'AGENTS.md'],
    });

    const blockStart = text.indexOf('<kimi-skill-loaded');
    const blockEnd = text.indexOf('</kimi-skill-loaded>');
    const block = text.slice(blockStart, blockEnd);
    expect(block).toContain('<bundled-resources>');
    expect(block).toContain('Skill(skill="review", resource="<path>")');
    expect(block).toContain('- references/hooks-patterns.md');
    expect(block).toContain('- AGENTS.md');
    // The section is appended after the skill content, inside the block.
    expect(text.indexOf('body')).toBeLessThan(text.indexOf('<bundled-resources>'));
    expect(text.indexOf('</bundled-resources>')).toBeLessThan(blockEnd);
  });

  it('includes the section for model-tool activations', () => {
    const text = renderModelToolSkillPrompt({
      ...base,
      trigger: 'model-tool',
      skillResources: ['rules/accessibility.md'],
    });

    expect(text).toContain('<bundled-resources>');
    expect(text).toContain('- rules/accessibility.md');
  });
