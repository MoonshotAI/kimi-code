...
import { encodeHeader, encodeLine, parseHeader, parseLine } from '#/store/tree';
import type { BranchHeader, EntryLine } from '#/store/tree';
...
describe('codec line', () => {
  ...
  it('rejects a payload with a ref', () => {
    const result = parseLine(
      '{"kind":"entry","seq":2,"ts":1,"type":"a.b","payload":{"kind":"json","size":1,"ref":"abc123"}}',
      2,
    );
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.error.kind).toBe('schema');
