...
describe('proxyEnvForChild', () => {
  ...
  it('removes inherited bracketed IPv6 loopback for portable child env', () => {
    expect(
...
describe('reconcileChildNoProxy', () => {
  ...
  it('preserves an explicit bracketed IPv6 loopback override', () => {
    const childEnv: Record<string, string> = { NO_PROXY: 'aug', no_proxy: 'aug' };
    reconcileChildNoProxy(childEnv, { NO_PROXY: 'server.local,[::1]' });
    expect(childEnv['NO_PROXY']).toBe('server.local,[::1],localhost,127.0.0.1,::1');
    expect(childEnv['no_proxy']).toBe('server.local,[::1],localhost,127.0.0.1,::1');
  });
