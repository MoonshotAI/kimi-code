...
describe('sanitizeMcpSchema — missing type filling', () => {
  ...
  it('infers type from enum values', () => {
    ...
    // Mixed-type enums become typed anyOf branches (not a single forced type).
    const mixed = prop(result, 'mixed');
    expect(mixed['type']).toBeUndefined();
    expect(mixed['enum']).toBeUndefined();
    const mixedBranches = mixed['anyOf'] as Schema[];
    expect(mixedBranches).toHaveLength(2);
    const byType = Object.fromEntries(mixedBranches.map((b) => [b['type'], b['enum']]));
    expect(byType['integer']).toEqual([1]);
    expect(byType['string']).toEqual(['a']);
  });
...
describe('sanitizeMcpSchema — JSON Pointer escaping', () => {
  it('decodes ~1 and ~0 in local $ref paths', () => {
    const result = sanitizeMcpSchema({
      type: 'object',
      properties: {
        slashy: { $ref: '#/$defs/a~1b' },
        tildy: { $ref: '#/$defs/c~0d' },
      },
      $defs: {
        'a/b': { type: 'integer' },
