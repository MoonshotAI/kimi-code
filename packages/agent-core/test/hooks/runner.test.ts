...
import { reducePermissionDecisionResults } from '../../src/session/hooks/permission-decision';
...
describe('runHook process runner', () => {
  ...
  it('parses a permission decision and its exact request id', async () => {
    const runHook = await importRunHook();
    const cmd =
      "node -e \"process.stdout.write(JSON.stringify({hookSpecificOutput:{permissionRequestId:'approval_1',permissionDecision:'allow'}}))\"";
  ...
  it('allows only when every matched hook returns allow for the current request', () => {
    expect(
      reducePermissionDecisionResults(
        [
          decisionResult('allow', 'approval_1'),
          decisionResult('allow', 'approval_1'),
        ],
        'approval_1',
      ),
