...
beforeEach(async () => {
  tmp = await mkdtemp(join(tmpdir(), 'kimi-mode-bootstrap-'));
...
async function countWireRecordEnters(recordType: string): Promise<number> {
  ...
  return lines.filter((line) => (JSON.parse(line) as { type?: string }).type === recordType)
    .length;
}
...
describe('plan-mode bootstrap from config.defaultPlanMode', () => {
  it('activates plan mode on a new session when config.defaultPlanMode is true', async () => {
    ...
    expect(await countWireRecordEnters('plan_mode.enter')).toBe(1);
  });
  ...
  it('leaves plan mode inactive when config.defaultPlanMode is absent', async () => {
    ...
    expect(await countWireRecordEnters('plan_mode.enter')).toBe(0);
  });
...
describe('swarm-mode bootstrap from config.defaultSwarmMode', () => {
  it('activates swarm mode on a new session when config.defaultSwarmMode is true', async () => {
    ...
    expect(await countWireRecordEnters('swarm_mode.enter')).toBe(1);
  });
  ...
  it('leaves swarm mode inactive when config.defaultSwarmMode is absent', async () => {
    ...
    expect(await countWireRecordEnters('swarm_mode.enter')).toBe(0);
  });
