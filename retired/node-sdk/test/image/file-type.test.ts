...
describe('detectFileType', () => {
  ...
  it('header sniffing picks up extensionless video and refines unknown-suffix MIME', () => {
    ...
    // .bin is in NON_TEXT_SUFFIXES; a sniffed PNG header refines it to an
    // image mime. The native detector reports the coarse kind (image) with an
    // `image/unknown` mime; the TS cross-validation may refine to image/png.
    const detected = detectFileType('sample.bin', pngHeader);
    expect(detected.kind).toBe('image');
    expect(detected.mimeType).toMatch(/^image\//);
