/**
 * Global (user-level) MCP server config store — local port of the retired
 * `agent-core/mcp/global-config.ts`. Reads/writes the user-home `mcp.json`
 * (server entries + optional auth), keeping the SDK's global MCP operations
 * host-side while the engine owns the session-scoped MCP runtime.
 */

import { mkdir, readFile } from 'node:fs/promises';
import { dirname, join } from 'pathe';

import { resolveKimiHome } from './config';
