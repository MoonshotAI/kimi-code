...
- [ ] 排查 `activateAgentServices()` 懒代理“假预热”：审计已完成，裸 `accessor.get(...)` 对 delayed service 不能作为完整预热屏障。已补 `IPermissionRulesService.rules` 强制构造以恢复 session approval replay；已补 `ISwarmMode.isActive` 强制构造以恢复 `AgentSwarm` service-owned tool 注册；restore 预热已补 `IUserToolService.register` 和 `IGoalService.getGoal()`。后续优先核对/修复构造期有 wire handler、hook、tool、event listener 的高风险服务：`IPlanModeService`、`IMcpRuntimeService`、`IFullCompaction`、`IMicroCompactionService`、`IProfileService`、`IBackgroundService`、`IToolStoreService`、`IContextUsageService`、`IPermissionModeService`、`IPermissionService`、`ILoopService`、`IAgentSkillService`。覆盖测试按对应服务单文件收口，不做一次性大改。
...
- [ ] 完成 swarm-mode replay restore parity：manual/task/tool 进入和退出的行为已恢复；task turn 正常完成、失败、取消时 auto-exit 已对齐旧 `src/agent`，`AgentSwarm` silent/tool 模式也已恢复。剩余重点是 replay restore 语义。覆盖测试：`tool.test.ts`、`turn.test.ts`。
