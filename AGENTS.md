...
- `apps/web` 只依赖 `@moonshot-ai/{app-core,app-i18n,app-markdown,app-ui,app-client,app-composer,app-components}` 共享包，不直接 import kimi-code 的包。
...
- **UI 改动必须遵循设计系统**：改组件 / 样式 / 布局 / 主题前，先读 canonical 设计规范 `apps/desktop/src/renderer/views/DesignSystemView.vue`（**desktop 单份正本**——web 副本与 web 应用内 overlay 已随 P22 删除，不再双端同步；desktop 应用内长按侧栏 logo 打开）。新增 / 修改的 UI 必须与之匹配；涉及结构、约束或新组件模式时，同步更新该文档。组件原语（`@moonshot-ai/app-ui`）的结构性例外只有五个：(1) 无标签、无尺寸、无变体外观的纯交互层用原生 `<button>` 而非 Button 原语——Button 的 chrome 与 `:active` scale 不适用；已登记两处：dock 工作面板行/卡片上的全覆盖隐形打开层（`TasksPane` 的 `.tp-open`、`SubagentGrid` 的 `.sg-open`）、消息气泡 legacy 附件行的可点击 attachment pill（`ChatPane` 的 `.u-atts`，须与编辑器内 NodeView pill 同款外观，UA chrome 在 ChatPane scoped 样式里重置；inline 正文 pill 保持文本原子 span，不在此列）；(2) mention / attachment / quote pill 共用的悬停 tooltip（`packages/app-composer` 的 `mentionTooltip`）——document 级裸 DOM 单例，锚点是 ProseMirror NodeView 和 pillify 后的 span（attachment pill 与 quote pill 复用同一单例，无新增例外），Vue 原语（Button/IconButton）够不到，其 open/copy 按钮按 Button 契约（尺寸、hover、`:focus-visible` 环）手工复刻，DesignSystemView §05（mention / attachment / quote 三个小节）已备案；(3) composer 的 work-mode（plan/goal）pill 与空文档 placeholder（`packages/app-composer` 的 `workModePill`，desktop 与 web 两端共用）——ProseMirror widget decoration 裸 DOM（只能在文档首部、不算文档内容），Vue 原语够不到，其 × 按钮按 IconButton 契约（`--wm-x-size`、hover、`--p-focus-ring`）手工复刻，DesignSystemView 的 work modes 段已备案；样式在各端 Composer.vue 以 `:deep()` 承载；(4) markdown code block 头部的换行 toggle（`packages/app-markdown` 的 `codeWrap`）——锚点在 markstream-vue 运行时渲染的 `.code-block-header` 内部（第三方库的组件树，宿主没有可挂 Vue 组件的模板挂载点，高亮内容还在 pierre 的 shadow root 里），只能以裸 DOM 注入（与 tableWide 的 widen chip 同一模式），按钮按 Button 契约手工复刻——class 直接复制同 header copy 按钮，尺寸 / hover / `:focus-visible` 环整条 `.code-action-btn` 样式链随之继承；其 tooltip 同样够不到 Vue 原语，走同包 `codeTooltip` 的 document 级裸 DOM 单例（mentionTooltip 同款模式，`data-md-tip` 属性驱动、TooltipBubble 视觉配方复刻），DesignSystemView 的 Code/Diff 段已备案；(5) OpenInMenu 的分段 pill（`packages/app-components` 的 `OpenInMenu.vue`，P20 下沉）——共享边框容器内的两段 action 区（30px 直开段 + 22px caret 段、26px 总高、共享分隔线），IconButton 原语表达不了：其 sm 是方形固定尺寸且自带圆角与独立 hover 块，拼不出单容器分段几何，按钮按 IconButton 契约（hover wash、`:focus-visible` 环）手工复刻，DesignSystemView 的 chat header 段已备案。
...
- `apps/desktop`：Electron 壳（`kimi-code-app`）；简介见 `apps/desktop/README.md`，原生功能分叉清单见 `apps/desktop/docs/native-todos.md`，Kimi Code Canary 内测通道（构建与使用）见 `apps/desktop/docs/canary.md`。新测试：主进程进 `tests/main/`，renderer 进 `tests/renderer/`。
...
- `apps/auth-login`：Remote Control 鉴权中间页（`kimi-code-auth-login`，单页、移动端优先）：OAuth device flow 经 `@moonshot-ai/kimi-code-oauth/device` CORS 直连 OAuth host（无 dev proxy），host 按页面 hostname 选择——`.kimi.ai` 域连 `auth.kimi.ai`，其余（kimi.com、localhost、LAN 隧道）连 `auth.kimi.com`，映射写在本 app 的 `flow.ts`（不依赖引擎包）；token 不落地，POST 给同源 `../auth/exchange` 换 relay 种的 HttpOnly session cookie（BFF 模式，续期全在 relay 服务端，见 `src/session.ts`），页面加载时 `GET ../auth/me` 探测登录态；`redirect_uri` 可选，有则登录后回跳。方案见 `docs/plans/2026-08-13-rc-auth-login.md`。
- `packages/*`：`@moonshot-ai/{app-core,app-i18n,app-markdown,app-ui,app-client,app-composer,app-components}` + `vite-preset`（exports→src，被 apps/web 与 desktop renderer 复用）——app-core 是无 Vue 依赖的纯层（api 客户端 / lib 纯函数 / client 渲染类型与热路径纯模块），app-client 是 Vue 客户端层：composables（注入 api / t / tracker）+ client 单例（`client/`，注入缝 `setKimiClientDeps`）+ Pinia domain stores（`stores/`，包持有 `clientPinia` 实例，两端 main.ts 安装；规范见 `docs/specs/2026-08-01-renderer-architecture.md` §5），app-composer 是 composer 富文本层（wire codec / ProseMirror editor / 用户消息渲染器 ComposerText / mention DOM / work-mode pill 与 placeholder 的 widget decoration 层 `workModePill`），app-components 是双端单源产品组件层（P17 起下沉：零真实差异的 `components/**` 与其支撑模块，deps 允许 app-ui/app-client/app-core/app-i18n/app-markdown/app-composer，禁止 import app 侧模块）；共享字体产物在 `app-ui/src/assets/fonts`（gitignored），由 `scripts/prepare-fonts.mjs` 自动准备。
...
pnpm dev:desktop:canary  # 桌面端，模拟 Kimi Code Canary 身份（黄徽章等 canary 专属 UI；KIMI_DESKTOP_CANARY=true，不改版本号、不动 userData）
