...
- `packages/agent-core-v2`: the DI × Scope agent engine (the v2 port behind kap-server). Two `LifecycleScope` tiers — `App` / `Session` (`app/scopes.ts`; agent granularity lives in Agent Runtimes hosted by the `AgentRuntimeSet`, with per-agent infrastructure carried by the explicitly-constructed `AgentHost` plain object) — plus the L3 unit layer (`Service`/`Fiber` units, collection contribution points, the Feature seam in `src/features/` — agent-granular domains live in `src/actor/`); there is no App-level session lifecycle facade — callers compose `ISessionIndex` → `IWorkspaceLifecycleService.handlerFor` → the handler. See `packages/agent-core-v2/AGENTS.md` and use the `agent-core-dev` skill (`.agents/skills/agent-core-dev/SKILL.md`) when developing here.
