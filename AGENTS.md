...
- `packages/remote-exec`: the remote execution protocol and bridge — self-contained NDJSON line-framed RPC (codex exec-server dialect) between the local `ExecBridge` (ssh/docker/`command` launchers) and the light `kimi exec-server` executor, plus the fs/process/terminal RPC stubs behind the agent-core-v2 `Runtime` interface. Client and server entries are exported separately; the CLI consumes only the server entry via dynamic import.
