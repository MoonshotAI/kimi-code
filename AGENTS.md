...
- `scripts/merge-mac-update-yml.mjs`：把 mac 双 arch 的 `latest-mac-<arch>.yml` 合并回单个 `latest-mac.yml`（files 含双 arch；release.yml 发布前调用，零第三方依赖的文本级合并）。根目录另有 `publish-desktop-cdn.sh`：desktop 产物的 CDN 发布（GH Release → TOS 版本目录 + latest*.yml 指针改写 + `download/` 固定入口刷新；本地手动，TOS 凭证限内网，配置见 kimi-cli-cdn-sync 仓 README）。
