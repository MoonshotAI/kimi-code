...
- `packages/agent-core`: the unified agent engine, including Agent, Session, profile, skills, tools, plan, permission, background, records, the in-process DI service layer (`src/services/` — domain-runtime-services decomposed into command/query/runtime facades over runtime-owned repositories/indexes, fenced by dependency direction), and other core capabilities.
