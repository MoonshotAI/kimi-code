...
一次简单查询只选一个数据源，不要并行读取其他源的 desc。专用源成功返回且已经覆盖用户问题后，立即回答；不要为了补充字段、重新格式化或交叉验证继续调用其他 API。只有用户明确要求跨源对比时，才能查询第二个数据源。

- **汇率（美元兑人民币等）、各国 CPI/通胀、国际收支、GDP 预测** → 只用 `imf`。禁止并行或兜底调用 `yahoo_finance`；它的外汇历史最多 2 年，`world_bank_open_data` 也只有年度宏观指标
