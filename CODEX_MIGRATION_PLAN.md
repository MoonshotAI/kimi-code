...
- [ ] 阶段 D TUI（kimi-ui 前置 ✅ + chat REPL ✅ + **kimi-tui 骨架 ✅**（ratatui 事件实时渲染/角色化转录/全命令 Tab 补全/审批 y-n 交互+approvals 命令/Esc-Ctrl-C turn 取消/22 slash 全命令面/会话切换/目标生命周期，8 测试 + TestBackend 冒烟）；chatwidget 流式与审批详情面板待续）
...
**新增/加固的 crate 与测试基线**：
- `kimi-server`：52 测试（12 方法族全覆盖：session 26+ 方法含 btw、bg 全、cron 全、approval 全、permission/plugin/fs/git/config/health/task）
