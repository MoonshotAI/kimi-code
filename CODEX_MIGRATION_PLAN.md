...
- [ ] 阶段 E SDK/ACP/OAuth/Config（kimi-sdk 🔶（Session 44/44 方法面）+ **kimi-acp 🔶**（+set_mode/set_model ✅）+ **kimi-oauth ✅**（device flow）+ **catalog ✅**（models.dev）+ provider/login/completions 命令 ✅ + **WS 传输 ✅**（kimi-server-transport））
