...
- [x] 阶段 B 宿主协议层（kimi-server 49 测试 + transport/serve 二进制 + client InProcess/Remote 全链路）✅
...
**新增/加固的 crate 与测试基线**：
- `kimi-server`：49 测试（12 方法族全覆盖：session 20+ 方法、bg 全、cron 全、approval 全、permission/plugin/fs/git/config/health/task）
